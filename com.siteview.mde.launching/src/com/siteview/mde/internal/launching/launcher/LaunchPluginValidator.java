...
public class LaunchPluginValidator {
	...
	private static IMonitorModelBase[] getSelectedWorkspacePlugins(ILaunchConfiguration configuration) throws CoreException {
	...
	public static Set parsePlugins(ILaunchConfiguration configuration, String attribute) throws CoreException {
		...
		if (ids != null) {
			...
			for (int i = 0; i < entries.length; i++) {
				...
				if (entry != null) {
					IMonitorModelBase matchingModels[] = attribute.equals(IPDELauncherConstants.SELECTED_TARGET_PLUGINS) ? entry.getExternalModels() : entry.getWorkspaceModels();
					for (int j = 0; j < matchingModels.length; j++) {
						if (matchingModels[j].isEnabled()) {
							...
							if (version == null || matchingModels[j].getMonitorBase().getVersion().equals(version)) {
	...
	public static void runValidationOperation(final LaunchValidationOperation op, IProgressMonitor monitor) throws CoreException {
		op.run(monitor);
		if (op.hasErrors()) {
			String message = NLS.bind(MDEMessages.PluginValidation_error, op.getInput().toString());
