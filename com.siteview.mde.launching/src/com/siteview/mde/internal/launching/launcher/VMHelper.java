...
public class VMHelper {
	...
	public static IVMInstall getVMInstall(ILaunchConfiguration configuration) throws CoreException {
		...
		if (jre == null) {
			...
			if (vm == null) {
				throw new CoreException(LauncherUtils.createErrorStatus(NLS.bind(MDEMessages.WorkbenchLauncherConfigurationDelegate_noJRE, name)));
			}
		} else {
			...
			if (vm == null) {
				...
				throw new CoreException(LauncherUtils.createErrorStatus(NLS.bind(MDEMessages.VMHelper_cannotFindExecEnv, id)));
			}
		}
		return vm;
	}
	...
	public static IVMInstall createLauncher(ILaunchConfiguration configuration) throws CoreException {
		...
		if (!launcher.getInstallLocation().exists())
			throw new CoreException(LauncherUtils.createErrorStatus(MDEMessages.WorkbenchLauncherConfigurationDelegate_jrePathNotFound));
		return launcher;
	}
