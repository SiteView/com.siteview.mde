...
public class BundleLauncherHelper {
		...
		if (configuration.getAttribute(IPDELauncherConstants.USE_CUSTOM_FEATURES, false)) {
			...
			for (Iterator iterator = featureResolutionMap.keySet().iterator(); iterator.hasNext();) {
				...
				for (int i = 0; i < featurePlugins.length; i++) {
					ModelEntry modelEntry = MonitorRegistry.findEntry(featurePlugins[i].getId());
					if (modelEntry != null) {
						IMonitorModelBase model = findModel(modelEntry, featurePlugins[i].getVersion(), pluginResolution);
						if (model != null)
				...
				for (int i = 0; i < featureImports.length; i++) {
					if (featureImports[i].getType() == IFeatureImport.PLUGIN) {
						ModelEntry modelEntry = MonitorRegistry.findEntry(featureImports[i].getId());
						if (modelEntry != null) {
							IMonitorModelBase model = findModel(modelEntry, featureImports[i].getVersion(), pluginResolution);
							if (model != null)
								launchPlugins.add(model);
						}
					}
			...
			if (!osgi) {
				...
				for (int i = 0; i < applicationIds.length; i++) {
					ModelEntry modelEntry = MonitorRegistry.findEntry(applicationIds[i]);
					if (modelEntry != null) {
						IMonitorModelBase model = findModel(modelEntry, null, defaultPluginResolution);
						if (model != null)
							launchPlugins.add(model);
					}
				}
			...
			while (it.hasNext()) {
				...
				ModelEntry modelEntry = MonitorRegistry.findEntry(id);
				if (modelEntry != null) {
					IMonitorModelBase model = findModel(modelEntry, null, defaultPluginResolution);
					if (model != null)
						launchPlugins.add(model);
			...
			for (Iterator iterator = launchPlugins.iterator(); iterator.hasNext();) {
				...
				if (pluginMap.containsKey(id)) {
					IMonitorModelBase existing = (IMonitorModelBase) pluginMap.get(id);
					if (model.getMonitorBase().getVersion().equalsIgnoreCase(existing.getMonitorBase().getVersion()) || (isSingleton(model) && isSingleton(existing))) {
						if (workspaceModels == null)
							workspaceModels = Arrays.asList(MonitorRegistry.getWorkspaceModels());
	 ...
	 * @return model
	 */
	private static IMonitorModelBase findModel(ModelEntry modelEntry, String version, String location) {
	...
	private static boolean isSingleton(IMonitorModelBase model) {
	 ...
	 */
	private static IMonitorModelBase getBestCandidateModel(IMonitorModelBase[] models, String version) {
	...
	public static IMonitorModelBase[] getMergedBundles(ILaunchConfiguration configuration, boolean osgi) throws CoreException {
		Map map = getMergedBundleMap(configuration, osgi);
		return (IMonitorModelBase[]) map.keySet().toArray(new IMonitorModelBase[map.size()]);
	}
	...
	public static String resolveSystemRunLevelText(IMonitorModelBase model) {
	...
	public static String resolveSystemAutoText(IMonitorModelBase model) {
		BundleDescription description = model.getBundleDescription();
		String modelName = description.getSymbolicName();
	 ...
	 */
	private static void addBundleToMap(Map map, IMonitorModelBase bundle, String sl) {
	...
	public static String writeBundleEntry(IMonitorModelBase model, String startLevel, String autoStart) {
		...
		if (entry != null && entry.getActiveModels().length > 1) {
			...
			buffer.append(model.getMonitorBase().getVersion());
		}
		...
		if (upgrade) {
			...
			if (!usedefault && !useFeatures) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					IMonitorModelBase model = MonitorRegistry.findModel(plugin);
					if (model == null)
						continue;
	...
	public static String writeAdditionalPluginsEntry(IMonitorModelBase model, String pluginResolution, boolean checked) {
		IMonitorBase base = model.getMonitorBase();
		String id = base.getId();
		StringBuffer buffer = new StringBuffer(id);
		buffer.append(':');
		...
		if (userAddedPlugins != null) {
			for (Iterator iterator = userAddedPlugins.iterator(); iterator.hasNext();) {
				...
				if (!onlyEnabled || checked) {
					...
					ModelEntry pluginModelEntry = MonitorRegistry.findEntry(id);
					if (pluginModelEntry != null) {
						if (IPDELauncherConstants.LOCATION_DEFAULT.equalsIgnoreCase(pluginResolution)) {
							pluginResolution = defaultPluginResolution;
						}
						IMonitorModelBase model = findModel(pluginModelEntry, version, pluginResolution);
						if (model != null) {
