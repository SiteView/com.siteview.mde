...
public class RegistryBrowser extends ViewPart {
	...
	private void initializeModel() {
		...
		initializeModelJob = new Job(MDERuntimeMessages.RegistryBrowser_InitializingView) {
	...
	private void makeActions() {
		...
		fGroupByExtensionPointsAction = new GroupByAction(MDERuntimeMessages.RegistryBrowser_ExtensionPoint, EXTENSION_REGISTRY);
			...
			public void run() {
				...
				for (Iterator it = bundles.iterator(); it.hasNext();) {
					...
					if ((problems != null) && (problems.getChildren().length > 0)) {
						dialog = new DiagnosticsDialog(getSite().getShell(), MDERuntimeMessages.RegistryView_diag_dialog_title, null, problems, IStatus.WARNING);
						dialog.open();
					} else {
						MessageDialog.openInformation(getSite().getShell(), MDERuntimeMessages.RegistryView_diag_dialog_title, MDERuntimeMessages.RegistryView_no_unresolved_constraints);
					}
	...
	public String getTitleSummary() {
		...
		switch (getGroupBy()) {
			default :
			case BUNDLES :
				type = MDERuntimeMessages.RegistryBrowser_plugins;
				total = model.getBundles().length;
				break;
			case EXTENSION_REGISTRY :
				type = MDERuntimeMessages.RegistryBrowser_extensionPoints;
				total = model.getExtensionPoints().length;
				break;
			case SERVICES :
				type = MDERuntimeMessages.RegistryBrowser_Services;
				total = model.getServiceNames().length;
				break;
		}

		if (tree == null)
			return NLS.bind(MDERuntimeMessages.RegistryView_titleSummary, (new String[] {"0", "0", type})); //$NON-NLS-1$ //$NON-NLS-2$
		return NLS.bind(MDERuntimeMessages.RegistryView_titleSummary, (new String[] {Integer.toString(tree.getItemCount()), Integer.toString(total), type}));
	}
