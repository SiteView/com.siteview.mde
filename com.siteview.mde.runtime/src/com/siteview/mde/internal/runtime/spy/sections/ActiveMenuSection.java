...
public class ActiveMenuSection implements ISpySection {
	...
	public void build(ScrolledForm form, SpyFormToolkit toolkit, Event event) {
		...
		if (object != null) {
			...
			if (object instanceof IContributionItem) {
				...
				if (id != null) {
					buffer.append(toolkit.createIdentifierSection(text, MDERuntimeMessages.ActiveMenuSection_0, new String[] {id}));
				}
	...
	private void createLocationURI(SpyFormToolkit toolkit, Object object, FormText text, StringBuffer buffer, String id) {
		...
		if (parent instanceof IMenuManager) {
			...
			buffer.append(toolkit.createIdentifierSection(text, MDERuntimeMessages.ActiveMenuSection_7, new String[] {locationURI}));
	...
	private void scan(IContributionItem item, StringBuffer buffer, SpyFormToolkit toolkit, FormText text) {
		...
		if (item instanceof IActionSetContributionItem) {
			...
			buffer.append(toolkit.createIdentifierSection(text, MDERuntimeMessages.ActiveMenuSection_1, new String[] {actionItem.getActionSetId()}));
		}
		if (item instanceof ActionContributionItem) {
			...
			buffer.append(toolkit.createClassSection(text, MDERuntimeMessages.ActiveMenuSection_2, new Class[] {command.getClass()}));
			buffer.append(toolkit.createClassSection(text, MDERuntimeMessages.ActiveMenuSection_3, new Class[] {command.getHandler().getClass()}));
		}
	}
	...
	private void createActionContributionItemText(Object object, StringBuffer buffer, SpyFormToolkit toolkit, FormText text) {
		...
		if (id != null) {
			buffer.append(toolkit.createIdentifierSection(text, MDERuntimeMessages.ActiveMenuSection_4, new String[] {action.getActionDefinitionId()}));
		}
		...
		if (action instanceof PluginAction) {
			...
			buffer.append(toolkit.createClassSection(text, MDERuntimeMessages.ActiveMenuSection_5, new Class[] {clazz}));
	...
	private void createActionContributionItemText(Object object, StringBuffer buffer, SpyFormToolkit toolkit, FormText text, Class clazz, PluginAction pluginAction) {
		try {
			...
			buffer.append(toolkit.createClassSection(text, MDERuntimeMessages.ActiveMenuSection_6, new Class[] {(retargetAction == null) ? delegate.getClass() : retargetAction.getActionHandler().getClass()}));
			PackageAdmin admin = PDERuntimePlugin.getDefault().getPackageAdmin();
			Bundle bundle = admin.getBundle(clazz);
