...
public class ActivePartSection implements ISpySection {
	...
	public void build(ScrolledForm form, SpyFormToolkit toolkit, ExecutionEvent event) {
		...
		section.setText(NLS.bind(MDERuntimeMessages.SpyDialog_activePart_title, part.getSite().getRegisteredName()));
		...
		buffer.append(toolkit.createClassSection(text, NLS.bind(MDERuntimeMessages.SpyDialog_activePart_desc, partType), new Class[] {part.getClass()}));
		if (part instanceof PageBookView) {
			PageBookView outline = (PageBookView) part;
			IPage currentPage = outline.getCurrentPage();
			if (currentPage != null) {
				buffer.append(toolkit.createClassSection(text, MDERuntimeMessages.SpyDialog_activePageBook_title, new Class[] {currentPage.getClass()}));
			}
		}
		...
		try {
			...
			if (list != null && list.size() > 0) {
				...
				buffer.append(MDERuntimeMessages.SpyDialog_activeMenuIds);
				buffer.append("</p>"); //$NON-NLS-1$
