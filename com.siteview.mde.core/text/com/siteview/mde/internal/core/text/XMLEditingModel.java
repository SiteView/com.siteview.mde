...
import com.siteview.mde.internal.core.util.SAXParserWrapper;
...
public abstract class XMLEditingModel extends AbstractEditingModel {
	...
	public void load(InputStream source, boolean outOfSync) {
		try {
			fLoaded = true;
			status = new Status(IStatus.OK, MDECore.PLUGIN_ID, null);
			SAXParserWrapper parser = new SAXParserWrapper();
			parser.parse(source, createDocumentHandler(this, true));
		} catch (SAXException e) {
			fLoaded = false;
			status = new Status(IStatus.ERROR, MDECore.PLUGIN_ID, e.getMessage(), e);
		} catch (IOException e) {
			fLoaded = false;
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.text.AbstractEditingModel#save(java.io.PrintWriter)
	 */
