...
public class MonitorLibraryNode extends MonitorObjectNode implements IMonitorLibrary {
	...
	public String[] getContentFilters() {
		...
		for (int i = 0; i < children.length; i++) {
			MonitorObjectNode node = (MonitorObjectNode) children[i];
	...
	public String getType() {
		...
		return (type != null && type.equals("resource")) ? IMonitorLibrary.RESOURCE : IMonitorLibrary.CODE; //$NON-NLS-1$
	}
	...
	public void addContentFilter(MonitorElementNode node) {
		addChildNode(node);
		if (isInTheModel()) {
	...
	public void removeContentFilter(String filter) throws CoreException {
		...
		for (int i = 0; i < children.length; i++) {
			if (children[i].getXMLTagName().equals(P_EXPORTED) && filter.equals(children[i].getXMLAttributeValue(P_NAME))) {
				removeContentFilter((MonitorElementNode) children[i]);
			}
		}
	}

	public void removeContentFilter(MonitorElementNode node) {
