...
import com.siteview.mde.internal.core.text.AbstractTextChangeListener;
...
public class XMLTextChangeListener extends AbstractTextChangeListener {
	...
	protected void deleteNode(IDocumentElementNode node) {
		...
		if (node.getOffset() > -1) {
			...
			if (fReadableNames != null)
				fReadableNames.put(op, NLS.bind(MDECoreMessages.XMLTextChangeListener_editNames_removeNode, node.getXMLTagName()));
		} else if (old == null) {
	...
	protected void insertNode(IDocumentElementNode node) {
		...
		fOperationList.add(op);
		if (fReadableNames != null)
			fReadableNames.put(op, NLS.bind(MDECoreMessages.XMLTextChangeListener_editNames_insertNode, node.getXMLTagName()));
	}
	...
	protected void modifyNode(IDocumentElementNode node, IModelChangedEvent event) {
		...
		if (node1.getOffset() < 0 && node2.getOffset() < 0) {
			...
			if (fReadableNames != null)
				fReadableNames.put(source, NLS.bind(MDECoreMessages.XMLTextChangeListener_editNames_modifyNode, node.getXMLTagName()));
		} else {
	...
	protected void addAttributeOperation(IDocumentAttributeNode attr, IModelChangedEvent event) {
		...
		if (offset > -1) {
			if (newValue == null || newValue.toString().length() == 0) {
				...
				if (fReadableNames != null)
					name = NLS.bind(MDECoreMessages.XMLTextChangeListener_editNames_removeAttribute, new String[] {attr.getAttributeName(), attr.getEnclosingElement().getXMLTagName()});
			} else {
				op = new ReplaceEdit(offset, attr.getValueLength(), getWritableString(event.getNewValue().toString()));
				if (fReadableNames != null)
					name = NLS.bind(MDECoreMessages.XMLTextChangeListener_editNames_modifyAttribute, new String[] {attr.getAttributeName(), attr.getEnclosingElement().getXMLTagName()});
			}
		}
		...
		if (op == null) {
			...
			if (node.getOffset() > -1) {
				...
				if (fReadableNames != null)
					name = NLS.bind(MDECoreMessages.XMLTextChangeListener_editNames_addAttribute, new String[] {attr.getAttributeName(), attr.getEnclosingElement().getXMLTagName()});
			} else {
				insertNode(node);
				return;
			}
	...
	protected void addElementContentOperation(IDocumentTextNode textNode) {
		...
		fOperationList.add(op);
		if (fReadableNames != null)
			fReadableNames.put(op, NLS.bind(MDECoreMessages.XMLTextChangeListener_editNames_addContent, textNode.getEnclosingElement().getXMLTagName()));
	}
