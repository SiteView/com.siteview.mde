...
public class BundleTextChangeListener extends AbstractKeyValueTextChangeListener {
	...
	public TextEdit[] getTextOperations() {
		...
		if (fReadableNames != null)
			fReadableNames.put(result[ops.length], MDECoreMessages.BundleTextChangeListener_editNames_newLine);
	 ...
	 * 
	 * @see com.siteview.mde.internal.core.text.AbstractKeyValueTextChangeListener#insertKey(com.siteview.mde.internal.core.text.IDocumentKey, java.lang.String)
	 */
		...
		for (int i = 0; i < objects.length; i++) {
			...
			if (object instanceof ManifestHeader) {
				...
				if (header.getValue() == null || header.getValue().trim().length() == 0) {
					String name = fReadableNames == null ? null : NLS.bind(MDECoreMessages.BundleTextChangeListener_editNames_remove, header.fName);
					deleteKey(header, name);
				} else {
					String name = fReadableNames == null ? null : NLS.bind(header.getOffset() == -1 ? MDECoreMessages.BundleTextChangeListener_editNames_insert : MDECoreMessages.BundleTextChangeListener_editNames_modify, header.fName);
