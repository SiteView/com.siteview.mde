...
public class RequiredMonitorsClasspathContainer extends MDEClasspathContainer implements IClasspathContainer {
	...
	static {
		DEBUG = MDECore.getDefault().isDebugging() && "true".equals(Platform.getDebugOption("org.eclipse.pde.core/classpath")); //$NON-NLS-1$ //$NON-NLS-2$
	}

	/**
	 * Constructor for RequiredPluginsClasspathContainer.
	 */
	public RequiredMonitorsClasspathContainer(IMonitorModelBase model) {
		this(model, null);
	}

	public RequiredMonitorsClasspathContainer(IMonitorModelBase model, IBuild build) {
		fModel = model;
		fBuild = build;
	}
	...
	public IPath getPath() {
		return MDECore.REQUIRED_PLUGINS_CONTAINER_PATH;
	}
	...
	public String getDescription() {
		return MDECoreMessages.RequiredPluginsClasspathContainer_description;
	}
	...
	public IClasspathEntry[] getClasspathEntries() {
		...
		if (DEBUG) {
			System.out.println("Dependencies for plugin '" + fModel.getMonitorBase().getId() + "':"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private Rule[] getInclusions(Map map, IMonitorModelBase model) {
	...
	protected void addExtraClasspathEntries(HashSet added, ArrayList entries, String[] tokens) {
		for (int i = 0; i < tokens.length; i++) {
			...
			if (!path.isAbsolute()) {
				...
				if (file.exists()) {
					IFile resource = MDECore.getWorkspace().getRoot().getFileForLocation(new Path(file.getAbsolutePath()));
	...
	private void addExtraLibrary(IPath path, IMonitorModelBase model, ArrayList entries) {
		if (path.segmentCount() > 1) {
			...
			if (model != null) {
				...
				if (MDECore.getWorkspace().getRoot().getFile(candidate).exists())
					srcPath = candidate;
			}
