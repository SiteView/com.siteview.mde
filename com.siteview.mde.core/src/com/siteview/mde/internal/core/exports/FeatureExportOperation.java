...
import com.siteview.mde.internal.core.feature.ExternalFeatureModel;
...
import com.siteview.mde.internal.core.target.TargetMetadataCollector;
...
public class FeatureExportOperation extends Job {
	...
	public FeatureExportOperation(FeatureExportInfo info, String name) {
		...
		fBuildTempLocation = MDECore.getDefault().getStateLocation().append("temp").toString(); //$NON-NLS-1$
		fBuildTempMetadataLocation = MDECore.getDefault().getStateLocation().append("tempp2metadata").toString(); //$NON-NLS-1$
	}
	...
	protected IStatus run(IProgressMonitor monitor) {
		try {
			...
			return new Status(IStatus.ERROR, MDECore.PLUGIN_ID, MDECoreMessages.FeatureBasedExportOperation_ProblemDuringExport, e.getCause() != null ? e.getCause() : e);
		} catch (CoreException e) {
			return new Status(IStatus.ERROR, MDECore.PLUGIN_ID, MDECoreMessages.FeatureBasedExportOperation_ProblemDuringExport, e.getCause() != null ? e.getCause() : e);
		} catch (IOException e) {
	...
	private void createDestination(String os, String ws, String arch) throws InvocationTargetException {
		...
		if (!file.exists() || !file.isDirectory()) {
			if (!file.mkdirs())
				throw new InvocationTargetException(new Exception(MDECoreMessages.ExportWizard_badDirectory));
		}
	}
	...
	protected void doExport(String featureID, String version, String featureLocation, String[][] configs, IProgressMonitor monitor) throws CoreException, InvocationTargetException {
		...
		try {
			...
			monitor.setTaskName(MDECoreMessages.FeatureExportJob_taskName);
			...
			generator.generate();
			monitor.worked(1);
			monitor.setTaskName(MDECoreMessages.FeatureExportOperation_runningBuildScript);
			// compile the classes
			...
			if (publishingP2Metadata()) {
				monitor.setTaskName(MDECoreMessages.FeatureExportOperation_publishingMetadata);
			...
			monitor.setTaskName(MDECoreMessages.FeatureExportOperation_runningAssemblyScript);
			for (int i = 0; i < configs.length; i++) {
			...
			monitor.setTaskName(MDECoreMessages.FeatureExportOperation_runningPackagerScript);
			for (int i = 0; i < configs.length; i++) {
				setArchiveLocation(properties, configs[i][0], configs[i][1], configs[i][2]);
	...
	public void deleteBuildFiles(Object object) throws CoreException {
		...
		if (object instanceof BundleDescription) {
			model = MonitorRegistry.findModel((BundleDescription) object);
		...
		if (model.getUnderlyingResource() != null && !isCustomBuild(model)) {
			String directory = (model instanceof IFeatureModel) ? ((IFeatureModel) model).getInstallLocation() : ((IMonitorModelBase) model).getInstallLocation();
		...
		if (model instanceof IFeatureModel) {
			...
			for (int i = 0; i < plugins.length; i++) {
				IMonitorModelBase plugin = MonitorRegistry.findModel(plugins[i].getId());
				if (plugin != null) {
	...
	protected void runScript(String location, String[] targets, Map properties, IProgressMonitor monitor) throws InvocationTargetException, CoreException {
		...
		runner.addBuildListener("com.siteview.mde.internal.core.ant.ExportBuildListener"); //$NON-NLS-1$
		runner.setExecutionTargets(targets);
	...
	*/
	protected void setP2MetaDataProperties(Map map) {
		if (fInfo.useJarFormat && fInfo.exportMetadata) {
			...
			try {
				...
				map.put(IBuildPropertiesConstants.PROPERTY_P2_METADATA_REPO_NAME, MDECoreMessages.FeatureExportOperation_0);
				map.put(IBuildPropertiesConstants.PROPERTY_P2_ARTIFACT_REPO_NAME, MDECoreMessages.FeatureExportOperation_0);
			} catch (MalformedURLException e) {
				MDECore.log(e);
			}
		}
	...
	protected File createScriptFile(String filename) throws IOException {
		String path = MDECore.getDefault().getStateLocation().toOSString();
	...
	protected void createFeature(String featureID, String featureLocation, String[][] configurations, boolean includeLauncher) throws IOException {
		...
		try {
			...
			if (includeLauncher) {
				IFeatureModel model = MDECore.getDefault().getFeatureModelManager().getDeltaPackFeature();
				if (model != null) {
					IFeature feature = model.getFeature();
					Element includes = doc.createElement("includes"); //$NON-NLS-1$
					includes.setAttribute("id", feature.getId()); //$NON-NLS-1$
					...
					IMonitorModelBase launcherPlugin = MonitorRegistry.findModel(IPDEBuildConstants.BUNDLE_EQUINOX_LAUNCHER);
			...
			List workspacePlugins = Arrays.asList(MonitorRegistry.getWorkspaceModels());
				...
				if (fInfo.items[i] instanceof IFeatureModel) {
					...
					for (Iterator iterator = configs.iterator(); iterator.hasNext();) {
						...
						if (shouldAddPlugin(bundle, environment)) {
							...
							if (fInfo.exportSource && fInfo.exportSourceBundle) {
								if (workspacePlugins.contains(MonitorRegistry.findModel(bundle))) { // Is it a workspace plugin?
								...
								{
									IMonitorModelBase model = MonitorRegistry.findModel(bundle.getSymbolicName() + ".source"); //$NON-NLS-1$
									if (model != null) {
	...
	protected boolean shouldAddPlugin(BundleDescription bundle, Dictionary environment) {
		...
		try {
			return (filterSpec == null || MDECore.getDefault().getBundleContext().createFilter(filterSpec).match(environment));
	...
	protected IStatus testBuildWorkspaceBeforeExport(IProgressMonitor monitor) throws CoreException {
		try {
			...
			if (fInfo.useWorkspaceCompiledClasses) {
				...
				if (!errors.isEmpty()) {
					monitor.worked(5);
					return new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(MDECoreMessages.FeatureExportOperation_workspaceBuildErrorsFoundDuringExport, errors.toString()));
				}
				monitor.worked(5);
			}
