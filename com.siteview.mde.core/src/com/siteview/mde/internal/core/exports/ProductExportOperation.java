...
public class ProductExportOperation extends FeatureExportOperation {
	...
	public static IStatus parseErrorMessage(CoreException e) {
		...
		for (; tokenizer.hasMoreTokens();) {
			...
			if (line.startsWith(STATUS_ENTRY) && tokenizer.hasMoreElements()) {
				...
				if (next.startsWith(STATUS_MESSAGE)) {
					status = new MultiStatus(MDECore.PLUGIN_ID, 0, next.substring(8), null);
				}
			} else if (line.startsWith(STATUS_SUBENTRY) && tokenizer.hasMoreElements() && status != null) {
				String next = tokenizer.nextToken();
				if (next.startsWith(STATUS_MESSAGE)) {
					status.add(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, next.substring(8)));
				}
			}
		}
		if (status != null)
		...
		return new MultiStatus(MDECore.PLUGIN_ID, 0, new IStatus[] {e.getStatus()}, errorMessage, null);
	}
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		try {
			...
			if (hasAntErrors()) {
				return new Status(IStatus.WARNING, MDECore.PLUGIN_ID, NLS.bind(MDECoreMessages.FeatureExportOperation_CompilationErrors, fInfo.destinationDirectory));
			}
	...
	private void createBuildPropertiesFile(String featureLocation, String[][] configurations) {
		...
		boolean hasLaunchers = MDECore.getDefault().getFeatureModelManager().getDeltaPackFeature() != null;
		...
		if (fInfo.exportSource && fInfo.exportSourceBundle) {
			...
			for (int i = 0; i < fInfo.items.length; i++) {
				if (fInfo.items[i] instanceof IFeatureModel) {
					...
					if (workspacePlugins.contains(MonitorRegistry.findModel(bundle))) {
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.exports.FeatureExportOperation#publishingP2Metadata()
	 */
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.exports.FeatureExportOperation#setP2MetaDataProperties(java.util.Map)
	 */
	protected void setP2MetaDataProperties(Map map) {
		if (fInfo.exportMetadata) {
			if (MDECore.getDefault().getFeatureModelManager().getDeltaPackFeature() == null)
			...
			try {
				...
				map.put(IBuildPropertiesConstants.PROPERTY_P2_METADATA_REPO_NAME, NLS.bind(MDECoreMessages.ProductExportOperation_0, fProduct.getProductId()));
				map.put(IBuildPropertiesConstants.PROPERTY_P2_ARTIFACT_REPO_NAME, NLS.bind(MDECoreMessages.ProductExportOperation_0, fProduct.getProductId()));
	...
	private String getExpandedPath(String path) {
		if (path == null || path.length() == 0)
			return null;
		IResource resource = MDECore.getWorkspace().getRoot().findMember(new Path(path));
		if (resource != null) {
	...
	private String createMacInfoPList() {
		...
		URL url = MDECore.getDefault().getBundle().getEntry(entryName);
		if (url == null)
			return null;
