...
public class ExecutionEnvironmentProfileManager {
	 ...
	 */
	private static final IPath PROFILE_PATH = MDECore.getDefault().getStateLocation().append(".profiles"); //$NON-NLS-1$
				...
				if (entry == null) {
					...
					if (properties != null) {
						...
						try {
							...
							MDECore.log(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(MDECoreMessages.ExecutionEnvironmentProfileManager_0, env.getId()), e));
						} finally {
							try {
								if (stream != null)
									stream.close();
