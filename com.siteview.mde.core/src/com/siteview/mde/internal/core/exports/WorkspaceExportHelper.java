			...
			if (markers.length > 0) {
				for (int j = 0; j < markers.length; j++) {
					...
					if (severity != null && severity.intValue() >= IMarker.SEVERITY_ERROR) {
						if (markers[j].getType().equals(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER) || markers[j].getType().equals(MDEMarkerFactory.MARKER_ID)) {
			...
			if (buildFile.exists()) {
				...
				if (javaProject.exists()) {
					...
					if (!modelOutput.isEmpty()) {
						IMonitorModelBase model = MDECore.getDefault().getModelManager().findModel(projects[i]);
						if (model != null) {
	...
	private IProject[] getExportedWorkspaceProjects(Object[] exportedItems) throws CoreException {
		if (fWorkspaceProjects == null) {
			...
			for (int i = 0; i < exportedItems.length; i++) {
				if (exportedItems[i] instanceof IMonitorModelBase) {
					IPath installLocation = new Path(((IMonitorModelBase) exportedItems[i]).getInstallLocation());
					IProject project = MDECore.getWorkspace().getRoot().getProject(installLocation.lastSegment());
					if (project.exists()) {
					...
					for (int j = 0; j < plugins.length; j++) {
						IMonitorModelBase plugin = MDECore.getDefault().getModelManager().findModel(plugins[j].getId());
						if (plugin != null) {
