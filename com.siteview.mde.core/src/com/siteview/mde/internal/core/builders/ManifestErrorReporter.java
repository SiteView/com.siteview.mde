...
public class ManifestErrorReporter extends XMLErrorReporter {
	...
	protected void reportIllegalElement(Element element, int severity) {
		...
		if (parent == null || parent instanceof org.w3c.dom.Document) {
			report(MDECoreMessages.Builders_Manifest_illegalRoot, getLine(element), severity, MDEMarkerFactory.CAT_FATAL);
		} else {
			report(NLS.bind(MDECoreMessages.Builders_Manifest_child, new String[] {element.getNodeName(), parent.getNodeName()}), getLine(element), severity, MDEMarkerFactory.P_ILLEGAL_XML_NODE, element, null, MDEMarkerFactory.CAT_FATAL);
		}
	}

	protected void reportMissingRequiredAttribute(Element element, String attName, int severity) {
		String message = NLS.bind(MDECoreMessages.Builders_Manifest_missingRequired, (new String[] {attName, element.getNodeName()})); //			
		report(message, getLine(element), severity, MDEMarkerFactory.CAT_FATAL);
	}
	...
	protected void reportUnknownAttribute(Element element, String attName, int severity) {
		String message = NLS.bind(MDECoreMessages.Builders_Manifest_attribute, attName);
		report(message, getLine(element, attName), severity, MDEMarkerFactory.P_ILLEGAL_XML_NODE, element, attName, MDEMarkerFactory.CAT_OTHER);
	}

	protected void reportIllegalAttributeValue(Element element, Attr attr) {
		String message = NLS.bind(MDECoreMessages.Builders_Manifest_att_value, (new String[] {attr.getValue(), attr.getName()}));
		report(message, getLine(element, attr.getName()), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
	}
	...
	protected void validateVersionAttribute(Element element, Attr attr) {
		...
		if (status.getSeverity() != IStatus.OK)
			report(status.getMessage(), getLine(element, attr.getName()), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
	}
	...
	private void reportMissingElementContent(Element element) {
		report(NLS.bind(MDECoreMessages.Builders_Feature_empty, element.getNodeName()), getLine(element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
	}
	...
	protected void reportExtraneousElements(NodeList elements, int maximum) {
		if (elements.getLength() > maximum) {
			for (int i = maximum; i < elements.getLength(); i++) {
				...
				report(NLS.bind(MDECoreMessages.Builders_Feature_multiplicity, element.getNodeName()), getLine(element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
			}
		}
	}
	...
	protected void validateURL(Element element, String attName) {
		...
		try {
			...
			report(NLS.bind(MDECoreMessages.Builders_Feature_badURL, attName), getLine(element, attName), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
		}
	}
	...
	protected boolean validatePluginID(Element element, Attr attr) {
		if (!IdUtil.isValidCompositeID3_0(attr.getValue())) {
			String message = NLS.bind(MDECoreMessages.Builders_Manifest_compositeID, attr.getValue(), attr.getName());
			report(message, getLine(element, attr.getName()), CompilerFlags.WARNING, MDEMarkerFactory.CAT_OTHER);
			return false;
		}
		return true;
	}
	...
	protected void reportDeprecatedAttribute(Element element, Attr attr) {
		...
		if (severity != CompilerFlags.IGNORE) {
			report(NLS.bind(MDECoreMessages.Builders_Manifest_deprecated_attribute, attr.getName()), getLine(element, attr.getName()), severity, MDEMarkerFactory.CAT_DEPRECATION);
		}
	}
}
