...
public class FeatureErrorReporter extends ManifestErrorReporter {
			...
			for (int j = 0; j < attributes.getLength(); j++) {
				...
				if (name.equals("version")) { //$NON-NLS-1$
					...
					if (element.getAttributeNode("patch") != null) { //$NON-NLS-1$
						report(NLS.bind(MDECoreMessages.Builders_Feature_patchedMatch, attr.getValue()), getLine(element, attr.getValue()), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					} else {
					...
					if ("true".equalsIgnoreCase(attr.getValue()) && feature == null) { //$NON-NLS-1$
						report(NLS.bind(MDECoreMessages.Builders_Feature_patchPlugin, attr.getValue()), getLine(element, attr.getValue()), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					} else if ("true".equalsIgnoreCase(attr.getValue()) && element.getAttributeNode("version") == null) { //$NON-NLS-1$ //$NON-NLS-2$
						report(NLS.bind(MDECoreMessages.Builders_Feature_patchedVersion, attr.getValue()), getLine(element, attr.getValue()), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					} else {
	...
	private void validatePluginID(Element element, Attr attr, boolean isFragment) {
		...
		if (severity != CompilerFlags.IGNORE) {
			...
			if (model == null || !model.isEnabled() || (isFragment && !model.isFragmentModel()) || (!isFragment && model.isFragmentModel())) {
				report(NLS.bind(MDECoreMessages.Builders_Feature_reference, id), getLine(element, attr.getName()), severity, MDEMarkerFactory.CAT_OTHER);
			}
		}
	}
	...
	private void validateFeatureID(Element element, Attr attr) {
		...
		if (severity != CompilerFlags.IGNORE) {
			IFeatureModel[] models = MDECore.getDefault().getFeatureModelManager().findFeatureModels(attr.getValue());
			if (models.length == 0) {
				report(NLS.bind(MDECoreMessages.Builders_Feature_freference, attr.getValue()), getLine(element, attr.getName()), severity, MDEMarkerFactory.CAT_OTHER);
			}
		}
	}

	protected void reportExclusiveAttributes(Element element, String attName1, String attName2, int severity) {
		String message = NLS.bind(MDECoreMessages.Builders_Feature_exclusiveAttributes, (new String[] {attName1, attName2}));
		report(message, getLine(element, attName2), severity, MDEMarkerFactory.CAT_OTHER);
	}
	...
	private void validateUnpack(Element parent) {
		...
		if (pModel instanceof IBundlePluginModel) {
			...
			if (header != null) {
				...
				if (value != null && !value.equalsIgnoreCase(unpackValue)) {
					String message = NLS.bind(MDECoreMessages.Builders_Feature_mismatchUnpackBundleShape, (new String[] {"unpack=" + unpack, parent.getAttribute("id"), "Eclipse-BundleShape: " + value})); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		if ("true".equals(unpack) && !CoreUtility.guessUnpack(pModel.getBundleDescription())) {//$NON-NLS-1$
			String message = NLS.bind(MDECoreMessages.Builders_Feature_missingUnpackFalse, (new String[] {parent.getAttribute("id"), "unpack=\"false\""})); //$NON-NLS-1$ //$NON-NLS-2$			
			report(message, getLine(parent), severity, MDEMarkerFactory.CAT_OTHER);
		}
	}
	...
	private void validateVersion(Element plugin, Attr attr) {
		...
		report(NLS.bind(MDECoreMessages.Builders_Feature_mismatchPluginVersion, new String[] {version, id}), getLine(plugin, attr.getName()), CompilerFlags.WARNING, MDEMarkerFactory.CAT_OTHER);
	}
}
