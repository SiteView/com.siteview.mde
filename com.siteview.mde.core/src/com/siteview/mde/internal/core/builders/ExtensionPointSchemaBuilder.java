...
public class ExtensionPointSchemaBuilder extends IncrementalProjectBuilder {
	...
	protected void clean(IProgressMonitor monitor) throws CoreException {
		SubMonitor localmonitor = SubMonitor.convert(monitor, NLS.bind(MDECoreMessages.ExtensionPointSchemaBuilder_0, getProject().getName()), 1);
		try {
	...
	private void cleanSchemasIn(IContainer container, IProgressMonitor monitor) throws CoreException {
		...
		for (int i = 0; i < members.length; i++) {
			...
			if (member instanceof IContainer)
				...
				member.deleteMarkers(MDEMarkerFactory.MARKER_ID, true, IResource.DEPTH_ZERO);
			}
		}
	}

	private boolean isInterestingProject(IProject project) {
		return MDE.hasPluginNature(project) && !WorkspaceModelManager.isBinaryProject(project);
	}

	private void compileFile(IFile file, IProgressMonitor monitor) {

		String message = NLS.bind(MDECoreMessages.Builders_Schema_compiling, file.getFullPath().toString());
	...
	private void compileSchemasIn(IContainer container, IProgressMonitor monitor) throws CoreException {
		monitor.subTask(MDECoreMessages.Builders_Schema_compilingSchemas);
		IResource[] members = container.members();
		for (int i = 0; i < members.length; i++) {
			IResource member = members[i];
			if (member instanceof IContainer)
	...
	private String getMangledPluginId(IFile file) {
		...
		IModel model = MonitorRegistry.findModel(project);
		if (model instanceof IMonitorModelBase) {
			IMonitorBase plugin = ((IMonitorModelBase) model).getMonitorBase();
			if (plugin != null) {
	...
	private void removeOutputFile(IFile file, IProgressMonitor monitor) {
		String outputFileName = getOutputFileName(file);
		monitor.subTask(NLS.bind(MDECoreMessages.Builders_Schema_removing, outputFileName));
