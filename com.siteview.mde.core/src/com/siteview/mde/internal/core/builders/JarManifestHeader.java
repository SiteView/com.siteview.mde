			...
			if (getValue().trim().length() > 0) {
				try {
					...
					if (fErrorReporter != null) {
						String message = NLS.bind(MDECoreMessages.BundleErrorReporter_parseHeader, getName());
						fErrorReporter.report(message, getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					}
				}
			} else {
				fManifestElements = new ManifestElement[0];
