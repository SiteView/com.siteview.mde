	...
	public void initialize(ArrayList sourceEntries, ArrayList outputEntries, IClasspathEntry[] cpes, IProject project) {
		...
		for (Iterator iterator = sourceEntries.iterator(); iterator.hasNext();) {
			...
			if (!pluginLibraryNames.contains(libName)) {
				prepareError(sourceEntry.getName(), null, NLS.bind(MDECoreMessages.SourceEntryErrorReporter_MissingLibrary, libName), MDEMarkerFactory.B_REMOVAL, fSrcLibSeverity, MDEMarkerFactory.CAT_OTHER);
			}
		...
		for (Iterator iterator = outputEntries.iterator(); iterator.hasNext();) {
			...
			if (!pluginLibraryNames.contains(libName)) {
				prepareError(outputEntry.getName(), null, NLS.bind(MDECoreMessages.SourceEntryErrorReporter_MissingLibrary, libName), MDEMarkerFactory.B_REMOVAL, fOututLibSeverity, MDEMarkerFactory.CAT_OTHER);
			}
	...
	public void validate() {
				...
				if (!outputFolder.isLibrary()) {
					...
					for (Iterator libNameiterator = outputFolderLibs.iterator(); libNameiterator.hasNext();) {
						...
						if (folderEntry == null || !folderEntry.exists() || !(folderEntry instanceof IContainer))
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_missingFolder, outputPath.toString());
						else
							message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_InvalidOutputFolder, outputPath.toString());
						prepareError(PROPERTY_OUTPUT_PREFIX + libName, outputFolder.getToken(), message, MDEMarkerFactory.B_REMOVAL, fOututLibSeverity, MDEMarkerFactory.CAT_OTHER);
					}
				} else {
					if (outputFolderLibs.size() == 0) {
						...
						if (model != null) {
							...
							String message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_MissingOutputLibForClassFolder, outputPath.toString());
							if (libs.length > 0) {
								prepareError(PROPERTY_OUTPUT_PREFIX, null, message, MDEMarkerFactory.NO_RESOLUTION, fOututLibSeverity, MDEMarkerFactory.CAT_OTHER);
							} else {
								prepareError(DEF_OUTPUT_ENTRY, outputPath.toString(), message, MDEMarkerFactory.B_ADDITION, fOututLibSeverity, MDEMarkerFactory.CAT_OTHER);
							}
						}
						...
						default :
							...
							for (int j = 0; j < sourceFolders.size(); j++) {
								...
								for (int k = 0; k < srcFolder.getLibs().size(); k++) {
									...
									String message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_DifferentTargetLibrary, erringSrcFolders);
									prepareError(PROPERTY_SOURCE_PREFIX + libName, srcFolder.getToken(), message, MDEMarkerFactory.NO_RESOLUTION, fSrcLibSeverity, MDEMarkerFactory.CAT_OTHER);
								}
							}
					}
				}
				for (int i = 0; i < outputFolderLibs.size(); i++) {
					String message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_ExtraOutputFolder, outputFolder.getPath().toString(), PROPERTY_SOURCE_PREFIX + outputFolderLibs.get(i));
					prepareError(PROPERTY_OUTPUT_PREFIX + outputFolderLibs.get(i), outputFolder.getToken(), message, MDEMarkerFactory.B_REMOVAL, fOututLibSeverity, MDEMarkerFactory.CAT_OTHER);
				}

				if (outputFolder.getDupeLibName() != null) {
					String message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_DupeOutputFolder, outputPath.toString(), PROPERTY_OUTPUT_PREFIX + outputFolder.getDupeLibName());
					prepareError(PROPERTY_OUTPUT_PREFIX + outputFolder.getDupeLibName(), outputFolder.getToken(), message, MDEMarkerFactory.NO_RESOLUTION, fOututLibSeverity, MDEMarkerFactory.CAT_OTHER);
				}
			}
		}
		...
		for (Iterator iterator = fSourceFolderMap.keySet().iterator(); iterator.hasNext();) {
			...
			if (outputFolder == null) {
				...
				String message;
				if (folderEntry == null || !folderEntry.exists() || !(folderEntry instanceof IContainer))
					message = NLS.bind(MDECoreMessages.BuildErrorReporter_missingFolder, sourcePath.toString());
				else
					message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_InvalidSourceFolder, sourcePath.toString());

				ArrayList srcLibs = sourceFolder.getLibs();
				for (int i = 0; i < srcLibs.size(); i++) {
					String libName = (String) srcLibs.get(i);
					prepareError(PROPERTY_SOURCE_PREFIX + libName, sourceFolder.getToken(), message, MDEMarkerFactory.B_REMOVAL, fSrcLibSeverity, MDEMarkerFactory.CAT_OTHER);
				}
			} else {
				...
				if (sourceFolder.getDupeLibName() != null) {
					String message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_DupeSourceFolder, sourcePath.toString(), PROPERTY_SOURCE_PREFIX + sourceFolder.getDupeLibName());
					prepareError(PROPERTY_SOURCE_PREFIX + sourceFolder.getDupeLibName(), sourceFolder.getToken(), message, MDEMarkerFactory.NO_RESOLUTION, fSrcLibSeverity, MDEMarkerFactory.CAT_OTHER);
				}
		...
		for (Iterator iter = missingOutputEntryErrors.keySet().iterator(); iter.hasNext();) {
			...
			MissingOutputEntry errorEntry = (MissingOutputEntry) missingOutputEntryErrors.get(libName);
			String message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_MissingOutputEntry, errorEntry.getSourceList(), PROPERTY_OUTPUT_PREFIX + libName);
			prepareError(PROPERTY_OUTPUT_PREFIX + libName, errorEntry.getOutputList(), message, MDEMarkerFactory.B_ADDITION, fMissingOutputLibSeverity, MDEMarkerFactory.CAT_OTHER);
		}
		...
		if (fEncodingSeverity == CompilerFlags.ERROR || fEncodingSeverity == CompilerFlags.WARNING) {
						...
						if (tokens.length == 1) {
							...
							if (expected != null) {
								if (!specified.equals(expected)) {
									prepareError(name, specified, NLS.bind(MDECoreMessages.SourceEntryErrorReporter_0, new String[] {expected, specified, lib}), MDEMarkerFactory.NO_RESOLUTION, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
								}
							} else {
								// encoding is specified, but workspace does not specify one
								prepareError(name, null, NLS.bind(MDECoreMessages.SourceEntryErrorReporter_1, new String[] {specified, lib}), MDEMarkerFactory.B_REMOVAL, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
							}
						} else {
							// syntax error
							fDefaultLibraryEncodings.remove(lib);
							prepareError(name, null, NLS.bind(MDECoreMessages.SourceEntryErrorReporter_2, lib), MDEMarkerFactory.NO_RESOLUTION, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
						}
					}
						...
						for (int j = 0; j < tokens.length; j++) {
							...
							if (index >= 0 && special.endsWith("]")) { //$NON-NLS-1$
								...
								if (member == null) {
									...
									String message = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_3, new String[] {encoding, path});
									prepareError(name, special, message, MDEMarkerFactory.B_REMOVAL, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
								} else {
									encodings.add(new EncodingEntry(member, encoding));
								}
							} else {
								// syntax error - invalid
								String message = MDECoreMessages.SourceEntryErrorReporter_4;
								prepareError(name, special, message, MDEMarkerFactory.NO_RESOLUTION, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
							}
						}
						// compare with workspace encodings
						List workspace = (List) fCustomEncodings.remove(lib);
						if (workspace == null) {
							prepareError(name, null, NLS.bind(MDECoreMessages.SourceEntryErrorReporter_5, lib), MDEMarkerFactory.B_REMOVAL, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
						} else {
							Map map = new HashMap();
							...
							while (iter.hasNext()) {
								...
								if (expected == null) {
									prepareError(name, ee.getValue(), NLS.bind(MDECoreMessages.SourceEntryErrorReporter_6, new String[] {expected, ee.getResource().getProjectRelativePath().toString()}), MDEMarkerFactory.B_REMOVAL, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
								} else {
									if (!specified.equals(expected)) {
										prepareError(name, ee.getValue(), NLS.bind(MDECoreMessages.SourceEntryErrorReporter_7, new String[] {expected, ee.getResource().getProjectRelativePath().toString(), specified}), MDEMarkerFactory.NO_RESOLUTION, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
									}
								}
							}
							...
							if (!map.isEmpty()) {
								...
								while (iter.hasNext()) {
									...
									String m = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_8, new String[] {expected, res.getProjectRelativePath().toString()});
									prepareError(name, missing.getValue(), m, MDEMarkerFactory.B_ADDITION, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
								}
							}
						}
					}
			...
			while (iter.hasNext()) {
				...
				prepareError(PROPERTY_JAVAC_DEFAULT_ENCODING_PREFIX + lib, expected, NLS.bind(MDECoreMessages.SourceEntryErrorReporter_9, new String[] {expected, lib}), MDEMarkerFactory.B_ADDITION, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
			}
			...
			while (iter.hasNext()) {
				...
				while (iterator2.hasNext()) {
					...
					String m = NLS.bind(MDECoreMessages.SourceEntryErrorReporter_10, new String[] {encoding.getEncoding(), encoding.getResource().getProjectRelativePath().toString()});
					prepareError(PROPERTY_JAVAC_CUSTOM_ENCODINGS_PREFIX + lib, encoding.getValue(), m, MDEMarkerFactory.B_ADDITION, fEncodingSeverity, MDEMarkerFactory.CAT_OTHER);
				}
			}
		}
	}
