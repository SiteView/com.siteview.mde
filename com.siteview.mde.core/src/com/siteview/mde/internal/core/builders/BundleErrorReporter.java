...
public class BundleErrorReporter extends JarManifestErrorReporter {
	...
	public void validateContent(IProgressMonitor monitor) {
		...
		fModel = MonitorRegistry.findModel(fProject);
	...
	private boolean validateBundleManifestVersion() {
		...
		if (header != null) {
			...
			if (!(fOsgiR4 = "2".equals(version)) && !"1".equals(version)) { //$NON-NLS-1$ //$NON-NLS-2$
				report(MDECoreMessages.BundleErrorReporter_illegalManifestVersion, header.getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
				return false;
			}
		}
		return true;
	...
	private void validateExportPackages() {
		...
		if (fModel instanceof IBundlePluginModelBase) {
			...
			if (packages.toString().length() > 0) {
				IMarker marker = report(MDECoreMessages.BundleErrorReporter_missingPackagesInProject, header == null ? 1 : header.getLineNumber() + 1, CompilerFlags.P_MISSING_EXPORT_PKGS, MDEMarkerFactory.M_MISSING_EXPORT_PKGS, MDEMarkerFactory.CAT_OTHER);
	...
	private boolean validateBundleSymbolicName() {
		...
		if (id == null || id.length() == 0) {
			report(MDECoreMessages.BundleErrorReporter_NoSymbolicName, header.getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
			return false;
		}
		...
		if (!fOsgiR4)
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_R4SyntaxInR3Bundle, Constants.BUNDLE_SYMBOLICNAME), header.getLineNumber() + 1, CompilerFlags.WARNING, MDEMarkerFactory.M_R4_SYNTAX_IN_R3_BUNDLE, MDEMarkerFactory.CAT_OTHER);
	...
	private boolean validatePluginId(IHeader header, String value) {
		if (!IdUtil.isValidCompositeID3_0(value)) {
			String message = MDECoreMessages.BundleErrorReporter_InvalidSymbolicName;
			report(message, getLine(header, value), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
			return false;
		}
		return true;
	}
	...
	private void validateSingleton(IHeader header, ManifestElement element) {
		...
		IMonitorBase base = fModel.getMonitorBase();
				...
				if (!"true".equals(singletonDir)) { //$NON-NLS-1$
					if ("true".equals(singletonAttr)) { //$NON-NLS-1$
						if (isCheckDeprecated() && fOsgiR4) {
							String message = MDECoreMessages.BundleErrorReporter_deprecated_attribute_singleton;
							report(message, getLine(header, ICoreConstants.SINGLETON_ATTRIBUTE + "="), //$NON-NLS-1$
									CompilerFlags.P_DEPRECATED, MDEMarkerFactory.M_SINGLETON_DIR_NOT_SET, MDEMarkerFactory.CAT_DEPRECATION);
							return;
						}
					} else {
						String message = NLS.bind(MDECoreMessages.BundleErrorReporter_singletonRequired, Constants.SINGLETON_DIRECTIVE);
						report(message, header.getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.M_SINGLETON_DIR_NOT_SET, MDEMarkerFactory.CAT_FATAL);
						return;
					}
				}
			} else if (!"true".equals(singletonAttr)) { //$NON-NLS-1$
				String message = NLS.bind(MDECoreMessages.BundleErrorReporter_singletonAttrRequired, ICoreConstants.SINGLETON_ATTRIBUTE);
				report(message, header.getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.M_SINGLETON_ATT_NOT_SET, MDEMarkerFactory.CAT_OTHER);
				return;
			}
		}
		...
		if (fOsgiR4) {
			if (singletonAttr != null) {
				if (isCheckDeprecated()) {
					String message = MDECoreMessages.BundleErrorReporter_deprecated_attribute_singleton;
					report(message, getLine(header, ICoreConstants.SINGLETON_ATTRIBUTE + "="), //$NON-NLS-1$
							CompilerFlags.P_DEPRECATED, MDEMarkerFactory.M_SINGLETON_DIR_NOT_SET, MDEMarkerFactory.CAT_OTHER);
				}
			}
		} else if (singletonDir != null) {
			if (isCheckDeprecated()) {
				String message = MDECoreMessages.BundleErrorReporter_unsupportedSingletonDirective;
				report(message, getLine(header, Constants.SINGLETON_DIRECTIVE + ":="), //$NON-NLS-1$
						CompilerFlags.P_DEPRECATED, MDEMarkerFactory.M_SINGLETON_DIR_NOT_SUPPORTED, MDEMarkerFactory.CAT_OTHER);
	...
	private void validateFragmentHost() {
		...
		if (header == null) {
			if (isCheckNoRequiredAttr() && PDEProject.getFragmentXml(fProject).exists()) {
				report(MDECoreMessages.BundleErrorReporter_HostNeeded, 1, CompilerFlags.P_NO_REQUIRED_ATT, MDEMarkerFactory.CAT_FATAL);
			}
			return;
		}
		...
		// Header introduced in OSGi R4 - warn if R3 manifest
		if (!fOsgiR4)
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_R4SyntaxInR3Bundle,Constants.FRAGMENT_HOST), header.getLineNumber() + 1, CompilerFlags.WARNING, MDEMarkerFactory.M_R4_SYNTAX_IN_R3_BUNDLE, MDEMarkerFactory.CAT_OTHER);
		...
		if (desc == null) {
			...
			if (elems.length > 0) {
				if (!VersionUtil.validateVersionRange(elems[0].getAttribute(Constants.BUNDLE_VERSION_ATTRIBUTE)).isOK()) {
					...
					report(MDECoreMessages.BundleErrorReporter_InvalidFormatInBundleVersion, line, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
				}
			}
			return;
		}
		...
		if (host.getSupplier() == null) {
			...
			if (missingHost) {
				...
				for (int i = 0; i < suppliers.length; i++) {
					...
					if (suppliers[i].isResolved()) {
						...
						if (!range.isIncluded(version)) {
							...
							report(NLS.bind(MDECoreMessages.BundleErrorReporter_BundleRangeInvalidInBundleVersion, versionRange), getLine(header, versionRange), CompilerFlags.P_UNRESOLVED_IMPORTS, MDEMarkerFactory.CAT_FATAL);
							return;
						}
					} else {
						resolved = false;
				...
				if (!resolved) {
					report(NLS.bind(MDECoreMessages.BundleErrorReporter_unresolvedHost, name), getLine(header, name), CompilerFlags.P_UNRESOLVED_IMPORTS, MDEMarkerFactory.CAT_FATAL);
					return;
				}
			}
		}

		IMonitorModelBase model = MonitorRegistry.findModel(name);
		if (model == null || model instanceof IFragmentModel || !model.isEnabled()) {
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_HostNotExistPDE, name), getLine(header, name), CompilerFlags.P_UNRESOLVED_IMPORTS, MDEMarkerFactory.CAT_FATAL);
		}
	}
	...
	private void validateBundleVersion() {
		...
		if (!status.isOK()) {
			int line = getLine(header, header.getValue());
			report(status.getMessage(), line, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
		}
	}
	...
	private void validateRequiredExecutionEnvironment() {
					...
					for (int i = 0; i < entries.length; i++) {
						if (entries[i].getEntryKind() == IClasspathEntry.CPE_CONTAINER) {
							...
							if (JavaRuntime.newDefaultJREContainerPath().matchingFirstSegments(currentPath) > 0) {
								...
								if (eeId != null) {
									IMarker marker = report(MDECoreMessages.BundleErrorReporter_noExecutionEnvironmentSet, 1, sev, MDEMarkerFactory.M_EXECUTION_ENVIRONMENT_NOT_SET, MDEMarkerFactory.CAT_EE);
				...
				for (int i = 0; i < entries.length; i++) {
					...
					if (!validPath.equals(currentPath)) {
						...
						if (vm == null || !env.isStrictlyCompatible(vm)) {
							report(NLS.bind(MDECoreMessages.BundleErrorReporter_reqExecEnv_conflict, bundleEnvs[0]), getLine(header, bundleEnvs[0]), sev, MDEMarkerFactory.M_MISMATCHED_EXEC_ENV, MDEMarkerFactory.CAT_EE);
						}
					}
				}
			} catch (JavaModelException e) {
		...
		for (int i = 0; i < bundleEnvs.length; i++) {
			...
			if (!found) {
				report(NLS.bind(MDECoreMessages.BundleErrorReporter_reqExecEnv_unknown, bundleEnvs[i]), getLine(header, bundleEnvs[i]), sev, MDEMarkerFactory.M_UNKNOW_EXEC_ENV, MDEMarkerFactory.CAT_EE);
				break;
			}
		}
	}
	...
	private void validateEclipsePlatformFilter() {
		...
		try {
			MDECore.getDefault().getBundleContext().createFilter(header.getValue());
			...
			if (desc != null && !desc.isResolved()) {
				...
				for (int i = 0; i < errors.length; i++) {
					if (errors[i].getType() == ResolverError.PLATFORM_FILTER) {
						report(MDECoreMessages.BundleErrorReporter_badFilter, header.getLineNumber() + 1, severity, MDEMarkerFactory.CAT_OTHER);
					}
				}
			}
		} catch (InvalidSyntaxException ise) {
			report(MDECoreMessages.BundleErrorReporter_invalidFilterSyntax, header.getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
		}
	}
	...
	private void validateBundleActivator() {
		...
		if (desc != null && desc.getHost() != null) {
			report(MDECoreMessages.BundleErrorReporter_fragmentActivator, header.getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
			return;
		}
			...
			try {
				if (fProject.hasNature(JavaCore.NATURE_ID)) {
					...
					if (!PDEJavaHelper.isOnClasspath(activator, javaProject)) {
						report(NLS.bind(MDECoreMessages.BundleErrorReporter_NoExist, activator), getLine(header, activator), CompilerFlags.P_UNKNOWN_CLASS, MDEMarkerFactory.M_UNKNOWN_ACTIVATOR, MDEMarkerFactory.CAT_FATAL);
					}
				}
	...
	private void validateBundleClasspath() {
		...
		if (header != null) {
			if (header.getElements().length == 0) {
				report(MDECoreMessages.BundleErrorReporter_ClasspathNotEmpty, header.getLineNumber() + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
			}
		}
	}
	...
	private void validateRequireBundle(IProgressMonitor monitor) {
		...
		for (int i = 0; i < required.length; i++) {
			...
			if (specs != null && specs[i].getSupplier() == null) {
				if (desc.getContainingState().getBundle(specs[i].getName(), null) == null) {
					IMarker marker = report(NLS.bind(MDECoreMessages.BundleErrorReporter_NotExistPDE, bundleID), getPackageLine(header, required[i]), severity, MDEMarkerFactory.M_REQ_BUNDLE_NOT_AVAILABLE, MDEMarkerFactory.CAT_FATAL);
					try {
						if (marker != null) {
					...
					report(NLS.bind(MDECoreMessages.BundleErrorReporter_BundleRangeInvalidInBundleVersion, bundleID + ": " + specs[i].getVersionRange()), //$NON-NLS-1$
							getPackageLine(header, required[i]), severity, MDEMarkerFactory.CAT_FATAL);
				}
			}
		}
		...
		// Header introduced in OSGi R4 - warn if R3 manifest
		if (!fOsgiR4)
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_R4SyntaxInR3Bundle, Constants.REQUIRE_BUNDLE), header.getLineNumber() + 1, CompilerFlags.WARNING, MDEMarkerFactory.M_R4_SYNTAX_IN_R3_BUNDLE, MDEMarkerFactory.CAT_OTHER);
	...
	private void validateBundleVersionAttribute(IHeader header, ManifestElement element) {
		...
		if (severity != CompilerFlags.IGNORE && versionRange == null) {
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_MissingVersion, element.getValue()), getPackageLine(header, element), severity, MDEMarkerFactory.CAT_OTHER);
		}

		if (versionRange != null && !VersionUtil.validateVersionRange(versionRange).isOK()) {
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_InvalidFormatInBundleVersion, element.getValue()), getPackageLine(header, element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
		}
	}
	...
	private void validateReprovideAttribute(IHeader header, ManifestElement element) {
		...
		if (rexport != null) {
			...
			if (fOsgiR4 && isCheckDeprecated()) {
				message = NLS.bind(MDECoreMessages.BundleErrorReporter_deprecated_attribute_reprovide, ICoreConstants.REPROVIDE_ATTRIBUTE);
				report(message, getLine(header, ICoreConstants.REPROVIDE_ATTRIBUTE + "="), //$NON-NLS-1$
						CompilerFlags.P_DEPRECATED, MDEMarkerFactory.CAT_DEPRECATION);
			}
		}
	}
	...
	private void validateOptionalAttribute(IHeader header, ManifestElement element) {
		...
		if (rexport != null) {
			...
			if (fOsgiR4 && isCheckDeprecated()) {
				report(NLS.bind(MDECoreMessages.BundleErrorReporter_deprecated_attribute_optional, ICoreConstants.OPTIONAL_ATTRIBUTE), getLine(header, ICoreConstants.OPTIONAL_ATTRIBUTE + "="), //$NON-NLS-1$
						CompilerFlags.P_DEPRECATED, MDEMarkerFactory.CAT_DEPRECATION);
			}
		}
	}
			...
			for (int j = 0; j < length; j++) {
				...
				if (name.equals("java") || name.startsWith("java.")) { //$NON-NLS-1$ //$NON-NLS-2$
					...
					if (jreHeader == null || !"true".equals(jreHeader.getValue())) { //$NON-NLS-1$
						report(MDECoreMessages.BundleErrorReporter_importNoJRE, getPackageLine(header, elements[i]), CompilerFlags.ERROR, MDEMarkerFactory.M_JAVA_PACKAGE__PORTED, MDEMarkerFactory.CAT_FATAL);
						continue;
					}
				}
				...
				if (export != null) {
					if (export.getSupplier().isResolved()) {
						...
						if (range != null && !range.isIncluded(version)) {
							report(NLS.bind(MDECoreMessages.BundleErrorReporter_unsatisfiedConstraint, importSpec.toString()), getPackageLine(header, elements[i]), severity, MDEMarkerFactory.CAT_FATAL);
						}
					} else {
						report(NLS.bind(MDECoreMessages.BundleErrorReporter_unresolvedExporter, new String[] {export.getSupplier().getSymbolicName(), name}), getPackageLine(header, elements[i]), severity, MDEMarkerFactory.CAT_OTHER);
					}
				} else {
					IMarker marker = report(NLS.bind(MDECoreMessages.BundleErrorReporter_PackageNotExported, name), getPackageLine(header, elements[i]), severity, MDEMarkerFactory.M_IMPORT_PKG_NOT_AVAILABLE, MDEMarkerFactory.CAT_FATAL);
					try {
						if (marker != null) {
		...
		for (int i = 0; i < elements.length; i++) {
			...
			for (int j = 0; j < valueComps.length; j++) {
				...
				if (name.equals("java") || name.startsWith("java.")) { //$NON-NLS-1$ //$NON-NLS-2$
					IHeader jreHeader = getHeader(ICoreConstants.ECLIPSE_JREBUNDLE);
					if (jreHeader == null || !"true".equals(jreHeader.getValue())) { //$NON-NLS-1$
						message = MDECoreMessages.BundleErrorReporter_exportNoJRE;
						report(message, getPackageLine(header, elements[i]), CompilerFlags.ERROR, MDEMarkerFactory.M_JAVA_PACKAGE__PORTED, MDEMarkerFactory.CAT_FATAL);
					}
				...
				if (!getExportedPackages().contains(name) && !(fProject.getFolder(name.replace('.', '/')).exists())) {
					message = NLS.bind(MDECoreMessages.BundleErrorReporter_NotExistInProject, name);
					IMarker marker = report(message, getPackageLine(header, elements[i]), CompilerFlags.P_UNRESOLVED_IMPORTS, MDEMarkerFactory.M_EXPORT_PKG_NOT_EXIST, MDEMarkerFactory.CAT_OTHER);
	...
	private void addHostPackages(String hostID) {
		IMonitorModelBase model = MonitorRegistry.findModel(hostID);
		if (model != null) {
			...
			if (resource != null) {
				...
				try {
					if (fProject.hasNature(JavaCore.NATURE_ID)) {
						IPackageFragment[] packages = PluginJavaSearchUtil.collectPackageFragments(new IMonitorModelBase[] {model}, JavaCore.create(fProject), false);
		...
		for (int i = 0; i < ICoreConstants.TRANSLATABLE_HEADERS.length; i++) {
			...
			if (header != null) {
				...
				if (!value.startsWith("%")) { //$NON-NLS-1$
					report(NLS.bind(MDECoreMessages.Builders_Manifest_non_ext_attribute, header.getName()), getLine(header, value), severity, MDEMarkerFactory.P_UNTRANSLATED_NODE, header.getName(), MDEMarkerFactory.CAT_NLS);
					...
					if (helper == null || !helper.resourceExists(value))
						report(NLS.bind(MDECoreMessages.Builders_Manifest_key_not_found, value.substring(1)), getLine(header, value), severity, MDEMarkerFactory.CAT_NLS);
				}
			}
		}
	}
	...
	private void validateSpecificationVersionAttribute(IHeader header, ManifestElement element) {
		...
		if (!status.isOK()) {
			report(status.getMessage(), getPackageLine(header, element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
		}
		if (isCheckDeprecated()) {
			if (fOsgiR4 && version != null) {
				report(NLS.bind(MDECoreMessages.BundleErrorReporter_deprecated_attribute_specification_version, ICoreConstants.PACKAGE_SPECIFICATION_VERSION), getPackageLine(header, element), CompilerFlags.P_DEPRECATED, MDEMarkerFactory.CAT_DEPRECATION);
			}
		}
	}
	...
	private void validateX_InternalDirective(IHeader header, ManifestElement element) {
		...
		String message = NLS.bind(MDECoreMessages.BundleErrorReporter_dir_value, (new String[] {internal, ICoreConstants.INTERNAL_DIRECTIVE}));
		report(message, getPackageLine(header, element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
	}
	...
	private void validateX_FriendsDirective(IHeader header, ManifestElement element) {
		...
		if (friends != null && internal != null) {
			String message = NLS.bind(MDECoreMessages.BundleErrorReporter_directive_hasNoEffectWith_, new String[] {ICoreConstants.FRIENDS_DIRECTIVE, ICoreConstants.INTERNAL_DIRECTIVE});
			IMarker marker = report(message, getPackageLine(header, element), CompilerFlags.WARNING, MDEMarkerFactory.M_DIRECTIVE_HAS_NO_EFFECT, MDEMarkerFactory.CAT_OTHER);
	...
	private void validateBundleActivatorPolicy() {
		...
		// Header introduced in OSGi R4 - warn if R3 manifest
		if (!fOsgiR4)
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_R4SyntaxInR3Bundle, Constants.BUNDLE_ACTIVATIONPOLICY), header.getLineNumber() + 1, CompilerFlags.WARNING, MDEMarkerFactory.M_R4_SYNTAX_IN_R3_BUNDLE, MDEMarkerFactory.CAT_OTHER);
		...
		if (TargetPlatformHelper.getTargetVersion() >= 3.3) {
			...
			report(MDECoreMessages.BundleErrorReporter_bundleActivationPolicy_unsupported, header.getLineNumber() + 1, severity, MDEMarkerFactory.NO_RESOLUTION, MDEMarkerFactory.CAT_OTHER);
		}
	}
	...
	private void validateAutoStart() {
		...
		if (severity != CompilerFlags.IGNORE && TargetPlatformHelper.getTargetVersion() >= 3.2 && !containsValidActivationHeader()) {
			...
			String message = NLS.bind(MDECoreMessages.BundleErrorReporter_startHeader_autoStartDeprecated, new Object[] {ICoreConstants.ECLIPSE_AUTOSTART, getCurrentActivationHeader()});
			IMarker marker = report(message, line + 1, severity, MDEMarkerFactory.M_DEPRECATED_AUTOSTART, MDEMarkerFactory.CAT_DEPRECATION);
			if (marker != null) {
				try {
					marker.setAttribute(MDEMarkerFactory.ATTR_HEADER, ICoreConstants.ECLIPSE_AUTOSTART);
					...
					marker.setAttribute(MDEMarkerFactory.ATTR_CAN_ADD, !unnecessary);
				} catch (CoreException e) {
				}
			}
		}
	...
	private void validateLazyStart() {
		...
		if (header != null) {
			...
			if (targetVersion < 3.2) {
				report(MDECoreMessages.BundleErrorReporter_lazyStart_unsupported, header.getLineNumber() + 1, severity, MDEMarkerFactory.NO_RESOLUTION, MDEMarkerFactory.CAT_OTHER);
			} else if (targetVersion > 3.3) {
				int line = header.getLineNumber();
				String message = NLS.bind(MDECoreMessages.BundleErrorReporter_startHeader_autoStartDeprecated, new Object[] {ICoreConstants.ECLIPSE_LAZYSTART, getCurrentActivationHeader()});
				IMarker marker = report(message, line + 1, severity, MDEMarkerFactory.M_DEPRECATED_AUTOSTART, MDEMarkerFactory.CAT_DEPRECATION);
				if (marker != null) {
					try {
						marker.setAttribute(MDEMarkerFactory.ATTR_HEADER, ICoreConstants.ECLIPSE_LAZYSTART);
						ManifestElement elem = header.getElements()[0];
				...
				for (int i = 0; i < values.length; i++) {
					...
					while (st.hasMoreTokens()) {
						...
						if (!getExportedPackages().contains(name)) {
							String message = NLS.bind(MDECoreMessages.BundleErrorReporter_NotExistInProject, name);
							report(message, getLine(header, name), CompilerFlags.P_UNRESOLVED_IMPORTS, MDEMarkerFactory.CAT_OTHER);
							return false;
						}
					}
				}
	...
	public void report(String message, int line, int severity, int problemID, String headerName, String category) {
		try {
			...
			if (marker != null)
				marker.setAttribute(MDEMarkerFactory.MPK_LOCATION_PATH, headerName);
		} catch (CoreException e) {
		}
	}
	...
	private void validateImportExportServices() {
		if (fOsgiR4) {
			...
			if (importHeader != null) {
				...
				report(NLS.bind(MDECoreMessages.BundleErrorReporter_importexport_servicesDeprecated, ICoreConstants.IMPORT_SERVICE), line + 1, severity, MDEMarkerFactory.M_DEPRECATED_IMPORT_SERVICE, MDEMarkerFactory.CAT_DEPRECATION);
			}

			if (exportHeader != null) {
				int line = exportHeader.getLineNumber();
				report(NLS.bind(MDECoreMessages.BundleErrorReporter_importexport_servicesDeprecated, ICoreConstants.EXPORT_SERVICE), line + 1, severity, MDEMarkerFactory.M_DEPRECATED_EXPORT_SERVICE, MDEMarkerFactory.CAT_DEPRECATION);
			}
		}
	}
	...
	private void validateBundleLocalization() {
		...
		// Header introduced in OSGi R4 - warn if R3 manifest
		if (!fOsgiR4)
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_R4SyntaxInR3Bundle, Constants.BUNDLE_LOCALIZATION), header.getLineNumber() + 1, CompilerFlags.WARNING, MDEMarkerFactory.M_R4_SYNTAX_IN_R3_BUNDLE, MDEMarkerFactory.CAT_OTHER);

		IResource res = PDEProject.getBundleRoot(fProject).findMember(location);
		if (res == null || !(res instanceof IContainer)) {
			report(MDECoreMessages.BundleErrorReporter_localization_folder_not_exist, header.getLineNumber() + 1, CompilerFlags.getFlag(fProject, CompilerFlags.P_UNKNOWN_RESOURCE), MDEMarkerFactory.CAT_OTHER);
			return;
		}
		...
		report(MDECoreMessages.BundleErrorReporter_localization_properties_file_not_exist, header.getLineNumber() + 1, CompilerFlags.getFlag(fProject, CompilerFlags.P_UNKNOWN_RESOURCE), MDEMarkerFactory.CAT_OTHER);
	}
	...
	private void validateFragmentHost(IHeader requireBundleHeader, ManifestElement element) {
		...
		if (header != null && elements[0] != null && elements[0].getValue().equals(element.getValue())) {
			IMarker marker = report(NLS.bind(MDECoreMessages.BundleErrorReporter_unecessaryDependencyDueToFragmentHost, element.getValue()), getPackageLine(requireBundleHeader, element), CompilerFlags.WARNING, MDEMarkerFactory.M_UNECESSARY_DEP, MDEMarkerFactory.CAT_OTHER);
	...
	private void validateProvidePackage() {
		...
		if (fOsgiR4 && isCheckDeprecated()) {
			report(MDECoreMessages.BundleErrorReporter_providePackageHeaderDeprecated, header.getLineNumber() + 1, CompilerFlags.P_DEPRECATED, MDEMarkerFactory.M_DEPRECATED_PROVIDE_PACKAGE, MDEMarkerFactory.CAT_OTHER);
		}
	}
