...
public class XMLErrorReporter extends DefaultHandler {
	...
	private void addMarker(SAXParseException e, int severity) {
		addMarker(e.getMessage(), e.getLineNumber(), severity, MDEMarkerFactory.NO_RESOLUTION, MDEMarkerFactory.CAT_OTHER);
	}
	...
	public void report(String message, int line, int severity, int fixId, Element element, String attrName, String category) {
		...
		if (marker == null)
			return;
		try {
			marker.setAttribute(MDEMarkerFactory.MPK_LOCATION_PATH, generateLocationPath(element, attrName));
		} catch (CoreException e) {
		}
	}
	...
	public InputSource resolveEntity(String publicId, String systemId) throws SAXException {
		...
		if (x > 0) {
			try {
				...
				report(MDECoreMessages.XMLErrorReporter_ExternalEntityResolution, line, CompilerFlags.WARNING, MDEMarkerFactory.CAT_OTHER);
			} catch (BadLocationException e) {
			}
		}
