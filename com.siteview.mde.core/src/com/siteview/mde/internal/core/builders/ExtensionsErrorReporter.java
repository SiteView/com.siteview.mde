...
public class ExtensionsErrorReporter extends ManifestErrorReporter {
	...
	public void validateContent(IProgressMonitor monitor) {
		...
		if (!"plugin".equals(elementName) && !"fragment".equals(elementName)) { //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (extensions != null && extensions.getExtensions().length == 0 && extensions.getExtensionPoints().length == 0)
				report(MDECoreMessages.Builders_Manifest_useless_file, -1, IMarker.SEVERITY_WARNING, MDEMarkerFactory.P_USELESS_FILE, MDEMarkerFactory.CAT_OTHER);
		}
	}
	...
	protected void validateExtension(Element element) {
		...
		if (!MDECore.getDefault().getExtensionsRegistry().hasExtensionPoint(pointID)) {
			...
			if (severity != CompilerFlags.IGNORE) {
				report(NLS.bind(MDECoreMessages.Builders_Manifest_ex_point, pointID), getLine(element, "point"), severity, MDEMarkerFactory.CAT_OTHER); //$NON-NLS-1$
			}
		} else {
	...
	private void reportMaxOccurenceViolation(ElementOccurrenceResult result, int severity) {
		...
		String message = NLS.bind(MDECoreMessages.ExtensionsErrorReporter_maxOccurrence, new String[] {allowedOccurrences, childElement.getNodeName()});
		report(message, getLine(childElement), severity, MDEMarkerFactory.P_ILLEGAL_XML_NODE, childElement, null, MDEMarkerFactory.CAT_FATAL);
	}
	...
	private void reportMinOccurenceViolation(Element parentElement, ElementOccurrenceResult result, int severity) {
		...
		String allowedOccurrences = new Integer(result.getAllowedOccurrences()).toString();
		String message = NLS.bind(MDECoreMessages.ExtensionsErrorReporter_minOccurrence, new String[] {allowedOccurrences, childElement.getName()});
	...
	private void validateInternalExtensionAttribute(Element element, ISchemaElement schemaElement) {
		...
		if (schemaElement instanceof ISchemaRootElement) {
			...
			if (rootElement.isInternal() && !epid.equals(pid)) {
				...
				report(NLS.bind(MDECoreMessages.Builders_Manifest_internal_rootElement, point), getLine(element, "point"), severity, MDEMarkerFactory.CAT_DEPRECATION); //$NON-NLS-1$
			}
		}
	}
	...
	protected void validateExtensionPoint(Element element) {
		if (assertAttributeDefined(element, "id", CompilerFlags.ERROR)) { //$NON-NLS-1$
			...
			if (message != null)
				report(message, getLine(element, idAttr.getName()), CompilerFlags.WARNING, MDEMarkerFactory.CAT_OTHER);
		}
		...
		if (attr != null) {
			...
			if (errorMessage != null) {
				...
				if (severity != CompilerFlags.IGNORE)
					report(NLS.bind(errorMessage, schemaValue), getLine(element), severity, MDEMarkerFactory.CAT_OTHER);
			}
		}
	}
	...
	protected void validateTranslatableString(Element element, Attr attr, boolean shouldTranslate) {
		...
		if (!value.startsWith("%")) { //$NON-NLS-1$
			report(NLS.bind(MDECoreMessages.Builders_Manifest_non_ext_attribute, attr.getName()), getLine(element, attr.getName()), severity, MDEMarkerFactory.P_UNTRANSLATED_NODE, element, attr.getName(), MDEMarkerFactory.CAT_NLS);
			...
			if (helper == null || !helper.resourceExists(value)) {
				report(NLS.bind(MDECoreMessages.Builders_Manifest_key_not_found, value.substring(1)), getLine(element, attr.getName()), severity, MDEMarkerFactory.CAT_NLS);
			}
		}
	}
	...
	protected void validateTranslatableElementContent(Element element) {
		...
		if (value == null)
			return;
		if (!value.startsWith("%")) { //$NON-NLS-1$
			report(NLS.bind(MDECoreMessages.Builders_Manifest_non_ext_element, element.getNodeName()), getLine(element), severity, MDEMarkerFactory.P_UNTRANSLATED_NODE, element, null, MDEMarkerFactory.CAT_NLS);
		} else if (fModel instanceof AbstractNLModel) {
			NLResourceHelper helper = ((AbstractNLModel) fModel).getNLResourceHelper();
			if (helper == null || !helper.resourceExists(value)) {
				report(NLS.bind(MDECoreMessages.Builders_Manifest_key_not_found, value.substring(1)), getLine(element), severity, MDEMarkerFactory.CAT_NLS);
			}
		}
	}
	...
	protected void validateResourceAttribute(Element element, Attr attr) {
		...
		if (severity != CompilerFlags.IGNORE && !resourceExists(attr.getValue())) {
			report(NLS.bind(MDECoreMessages.Builders_Manifest_resource, (new String[] {attr.getValue(), attr.getName()})), getLine(element, attr.getName()), severity, MDEMarkerFactory.CAT_OTHER);
		}
	}
	...
	protected void validateJavaAttribute(Element element, Attr attr) {
		...
		if (severity != CompilerFlags.IGNORE && javaProject.isOpen()) {
			...
			if (!onClasspath) {
				report(NLS.bind(MDECoreMessages.Builders_Manifest_class, (new String[] {value, attr.getName()})), getLine(element, attr.getName()), severity, MDEMarkerFactory.P_UNKNOWN_CLASS, element, attr.getName() + F_ATT_VALUE_PREFIX + attr.getValue(), MDEMarkerFactory.CAT_FATAL);
			}
		}
		...
		if (severity != CompilerFlags.IGNORE && javaProject.isOpen()) {
			...
			if (onClasspath && PDEJavaHelper.isDiscouraged(value, javaProject, desc)) {
				report(NLS.bind(MDECoreMessages.Builders_Manifest_discouragedClass, (new String[] {value, attr.getName()})), getLine(element, attr.getName()), severity, MDEMarkerFactory.M_DISCOURAGED_CLASS, element, attr.getName() + F_ATT_VALUE_PREFIX + attr.getValue(), MDEMarkerFactory.CAT_OTHER);
			}
		}
	}
		...
		if (severity != CompilerFlags.IGNORE) {
			...
			if (value != null && basedOn != null && value.length() > 0 && basedOn.length() > 0) {
				...
				if (!attributes.containsKey(value)) { // report error if we are missing something
					report(NLS.bind(MDECoreMessages.ExtensionsErrorReporter_unknownIdentifier, (new String[] {attr.getValue(), attr.getName()})), getLine(element, attr.getName()), severity, MDEMarkerFactory.CAT_OTHER);
				}
			}
		}
	}
	...
	protected void reportUnusedAttribute(Element element, String attName, int severity) {
		String message = NLS.bind(MDECoreMessages.Builders_Manifest_unused_attribute, attName);
		report(message, getLine(element, attName), severity, MDEMarkerFactory.CAT_OTHER);
	}

	protected void reportUnusedElement(Element element, int severity) {
		Node parent = element.getParentNode();
		report(NLS.bind(MDECoreMessages.Builders_Manifest_unused_element, (new String[] {element.getNodeName(), parent.getNodeName()})), getLine(element), severity, MDEMarkerFactory.CAT_OTHER);
	}

	protected void reportDeprecatedElement(Element element) {
		int severity = CompilerFlags.getFlag(fProject, CompilerFlags.P_DEPRECATED);
		if (severity != CompilerFlags.IGNORE) {
			report(NLS.bind(MDECoreMessages.Builders_Manifest_deprecated_element, element.getNodeName()), getLine(element), severity, MDEMarkerFactory.CAT_DEPRECATION);
		}
	}
	...
	protected void reportDeprecatedRootElement(Element element, String suggestion) {
		...
		if (severity != CompilerFlags.IGNORE) {
			...
			String message;
			if (suggestion != null)
				message = NLS.bind(MDECoreMessages.Builders_Manifest_deprecated_rootElementSuggestion, point, suggestion);
			else
				message = NLS.bind(MDECoreMessages.Builders_Manifest_deprecated_rootElement, point);
			report(message, getLine(element, "point"), severity, MDEMarkerFactory.CAT_DEPRECATION); //$NON-NLS-1$
		}
	}
}
