...
public class ManifestConsistencyChecker extends IncrementalProjectBuilder {
	...
	static {
		DEBUG = MDECore.getDefault().isDebugging() && "true".equals(Platform.getDebugOption("org.eclipse.pde.core/validation")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	protected IProject[] build(int kind, Map args, IProgressMonitor monitor) throws CoreException {
		if (MDECore.getDefault().getBundle().getState() != Bundle.ACTIVE || monitor.isCanceled())
			return EMPTY_LIST;
	...
	private int getDeltaType(IProject project) throws CoreException {
		...
		if (Boolean.TRUE.equals(project.getSessionProperty(MDECore.TOUCH_PROJECT))) {
			project.setSessionProperty(MDECore.TOUCH_PROJECT, null);
			if (DEBUG) {
	...
	private void validateProject(int type, IProgressMonitor monitor) {
		monitor.beginTask(MDECoreMessages.ManifestConsistencyChecker_builderTaskName, getWorkAmount(type));
		if ((type & STRUCTURE) != 0)
	...
	private void validateProjectStructure(int type, IProgressMonitor monitor) {
		...
		IProject project = getProject();
		try {
			project.deleteMarkers(MDEMarkerFactory.MARKER_ID, false, IResource.DEPTH_ZERO);
	...
	private void validateFiles(IFile file, int type, IProgressMonitor monitor) {
		...
		if (bundleManifest.exists()) {
			...
			if (file.equals(PDEProject.getPluginXml(getProject()))) {
				reporter = new MonitorErrorReporter(file);
	...
	private void validateBuildProperties(IProgressMonitor monitor) {
		...
		if (file.exists()) {
			monitor.subTask(MDECoreMessages.ManifestConsistencyChecker_buildPropertiesSubtask);
	...
	private void validateBuildPropertiesExists(IProject project) {
		...
		if (!file.exists()) {
			...
			try {
				IMarker marker = project.createMarker(MDEMarkerFactory.MARKER_ID);
				marker.setAttribute(IMarker.SEVERITY, CompilerFlags.ERROR == severity ? IMarker.SEVERITY_ERROR : IMarker.SEVERITY_WARNING);
				marker.setAttribute(IMarker.MESSAGE, MDECoreMessages.ManifestConsistencyChecker_buildDoesNotExist);
			} catch (CoreException e) {
			}
		}
	}
	...
	private void validateManifestCasing(IProject project) {
		...
		if (manifestFolder.exists()) {
			try {
				manifestFolder.deleteMarkers(MDEMarkerFactory.MARKER_ID, false, IResource.DEPTH_ONE);
			...
			if (location != null) {
				...
				for (int i = 0; i < fileList.length; i++) {
					...
					try {
						IMarker marker = currentFile.createMarker(MDEMarkerFactory.MARKER_ID);
						marker.setAttribute(IMarker.SEVERITY, IMarker.SEVERITY_ERROR);
						marker.setAttribute(IMarker.MESSAGE, MDECoreMessages.ManifestConsistencyChecker_manifestMisspelled);
					} catch (CoreException e) {
					}
				}
				...
				if (fileList.length == 0) {
					try {
						IMarker marker = project.createMarker(MDEMarkerFactory.MARKER_ID);
						marker.setAttribute(IMarker.SEVERITY, IMarker.SEVERITY_ERROR);
						marker.setAttribute(IMarker.MESSAGE, MDECoreMessages.ManifestConsistencyChecker_manifestDoesNotExist);
					} catch (CoreException e) {
					}
				}
			}
	...
	protected void clean(IProgressMonitor monitor) throws CoreException {
		SubMonitor localmonitor = SubMonitor.convert(monitor, NLS.bind(MDECoreMessages.ManifestConsistencyChecker_0, getProject().getName()), 1);
		try {
	...
	private void cleanProblems(IResource resource, int depth) throws CoreException {
		if (resource.exists()) {
			resource.deleteMarkers(MDEMarkerFactory.MARKER_ID, true, depth);
		}
	}
}
