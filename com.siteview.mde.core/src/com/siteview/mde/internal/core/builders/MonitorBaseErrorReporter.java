...
public abstract class MonitorBaseErrorReporter extends ExtensionsErrorReporter {

	public MonitorBaseErrorReporter(IFile file) {
		super(file);
	}
	...
	protected void validatePluginIDRef(Element element, Attr attr) {
		...
		if (severity != CompilerFlags.IGNORE) {
			IMonitorModelBase model = MonitorRegistry.findModel(attr.getValue());
			if (model == null || !model.isEnabled()) {
				report(NLS.bind(MDECoreMessages.Builders_Manifest_dependency, attr.getValue()), getLine(element, attr.getName()), severity, MDEMarkerFactory.CAT_FATAL);
			}
		}
	}

	private void reportDeprecatedElement(Element element, int severity) {
		report(NLS.bind(MDECoreMessages.Builders_Manifest_deprecated_3_0, element.getNodeName()), getLine(element), severity, MDEMarkerFactory.CAT_DEPRECATION);
	}
