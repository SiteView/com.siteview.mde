...
public class FeatureConsistencyChecker extends IncrementalProjectBuilder {
	class DeltaVisitor implements IResourceDeltaVisitor {
		...
		public boolean visit(IResourceDelta delta) {
			...
			if (resource instanceof IProject) {
				...
				try {
					return (project.hasNature(MDE.FEATURE_NATURE));
	...
	protected void clean(IProgressMonitor monitor) throws CoreException {
		IFile file = PDEProject.getFeatureXml(getProject());
		if (file.exists()) {
			SubMonitor localmonitor = SubMonitor.convert(monitor, NLS.bind(MDECoreMessages.FeatureConsistencyChecker_0, file.getName()), 1);
			try {
