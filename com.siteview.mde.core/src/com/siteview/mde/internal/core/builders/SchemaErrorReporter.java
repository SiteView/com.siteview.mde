...
import com.siteview.mde.internal.core.schema.IncludedSchemaDescriptor;
...
public class SchemaErrorReporter extends XMLErrorReporter {
					...
					if (name != null && name.equals(ELEMENT)) {
						...
						if (value != null && value.length() > 0) {
							if (elements.contains(value)) { // report error
								report(NLS.bind(MDECoreMessages.Builders_Schema_duplicateElement, value), getLine((Element) child), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
							} else { // add to list
								elements.add(value);
							}
						}
							...
							if (closing.equals(">")) { //$NON-NLS-1$
								...
								if (tagName.endsWith("/")) { //$NON-NLS-1$
									...
									if (forbiddenEndTag(tagName)) {
										report(NLS.bind(MDECoreMessages.Builders_Schema_forbiddenEndTag, tagName), lineNumber, flag, MDEMarkerFactory.CAT_OTHER);
										errorReported = true;
									}
									...
									if (stack.isEmpty() && !found) {
										report(NLS.bind(MDECoreMessages.Builders_Schema_noMatchingStartTag, tagName), lineNumber, flag, MDEMarkerFactory.CAT_OTHER);
										errorReported = true;
									}
								} else {
				...
				if (!errorReported) {
					if (!stack.isEmpty()) {
						...
						if (!optionalEndTag(entry.tag))
							report(NLS.bind(MDECoreMessages.Builders_Schema_noMatchingEndTag, entry.tag), entry.line, flag, MDEMarkerFactory.CAT_OTHER);
					}
					stack.clear();
				}
			}
	...
	private void validateUse(Element element) {
		...
		if (use != null && "default".equals(use.getValue()) && value == null) { //$NON-NLS-1$
			report(NLS.bind(MDECoreMessages.Builders_Schema_valueRequired, element.getNodeName()), getLine(element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_OTHER);
		} else if (use == null && value != null) {
			report(NLS.bind(MDECoreMessages.Builders_Schema_valueNotRequired, element.getNodeName()), getLine(element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_OTHER);
		}
	}
				...
				try {
					...
					if (includedSchemaUrl != null && computedUrl != null && includedSchemaUrl.equals(computedUrl)) {
						if (!includedSchema.isValid())
							report(NLS.bind(MDECoreMessages.Builders_Schema_includeNotValid, schemaLocation), getLine(element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_OTHER);
					}
	...
	private void validateElementReference(Element element) {
		...
		if (value != null && value.length() > 0) {
			...
			if (referencedElement == null) {
				report(NLS.bind(MDECoreMessages.Builders_Schema_referencedElementNotFound, value), getLine(element), CompilerFlags.ERROR, MDEMarkerFactory.CAT_OTHER);
			}
		}
	}
}
