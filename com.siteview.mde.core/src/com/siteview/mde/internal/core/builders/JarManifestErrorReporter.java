...
public class JarManifestErrorReporter extends ErrorReporter {
		...
		try {
			...
			for (; l < document.getNumberOfLines(); l++) {
				...
				if (byteBuf.limit() + lineDelimiter.length() > 512) {
					report(MDECoreMessages.BundleErrorReporter_lineTooLong, l + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					return;
				}
				// parse
				if (line.length() == 0) {
					// Empty Line
					if (l == 0) {
						report(MDECoreMessages.BundleErrorReporter_noMainSection, 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
						return;
					}
					/* flush last line */
					if (header != null) {
				...
				if (colon == -1) { /* no colon */
					report(MDECoreMessages.BundleErrorReporter_noColon, l + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					return;
				}
				String headerName = getHeaderName(line);
				if (headerName == null) {
					report(MDECoreMessages.BundleErrorReporter_invalidHeaderName, l + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					return;
				}
				if (line.length() < colon + 2 || line.charAt(colon + 1) != ' ') {
					report(MDECoreMessages.BundleErrorReporter_noSpaceValue, l + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					return;
				}
				if ("Name".equals(headerName)) { //$NON-NLS-1$
					report(MDECoreMessages.BundleErrorReporter_nameHeaderInMain, l + 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
					return;
				}
				header = new JarManifestHeader(headerName, line.substring(colon + 2), l, this);
				if (fHeaders.containsKey(header.getName().toLowerCase())) {
					report(MDECoreMessages.BundleErrorReporter_duplicateHeader, l + 1, CompilerFlags.WARNING, MDEMarkerFactory.CAT_OTHER);
				}

			}
			if (header != null) {
				// lingering header, line not terminated
				IMarker marker = report(MDECoreMessages.BundleErrorReporter_noLineTermination, l, CompilerFlags.ERROR, MDEMarkerFactory.M_NO_LINE_TERMINATION, MDEMarkerFactory.CAT_FATAL);
				try {
					if (marker != null) {
						...
						String line = document.get(lineInfo.getOffset(), lineInfo.getLength());
						marker.setAttribute(MDEMarkerFactory.ATTR_HAS_CONTENT, !line.matches("\\s+")); //$NON-NLS-1$
					}
			...
			for (; l < document.getNumberOfLines(); l++) {
				...
				if (!line.startsWith("Name:")) { //$NON-NLS-1$
					report(MDECoreMessages.BundleErrorReporter_noNameHeader, l, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
				}
				break;
			}
	...
	protected void reportIllegalAttributeValue(IHeader header, String key, String value) {
		String msg = NLS.bind(MDECoreMessages.BundleErrorReporter_att_value, (new String[] {value, key}));
	...
	protected void reportIllegalValue(IHeader header, String value) {
		String msg = NLS.bind(MDECoreMessages.BundleErrorReporter_illegal_value, value);
		report(msg, getLine(header, value), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
	}

	protected void reportIllegalDirectiveValue(IHeader header, String key, String value) {
		String msg = NLS.bind(MDECoreMessages.BundleErrorReporter_dir_value, (new String[] {value, key}));
		report(msg, getLine(header, key + ":="), CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL); //$NON-NLS-1$
	}
	...
	protected IHeader validateRequiredHeader(String name) {
		...
		if (header == null) {
			report(NLS.bind(MDECoreMessages.BundleErrorReporter_headerMissing, name), 1, CompilerFlags.ERROR, MDEMarkerFactory.CAT_FATAL);
		}
		return header;
	}
