...
public class BuildErrorReporter extends ErrorReporter implements IBuildPropertiesConstants {
	...
	private class BuildProblem {
		...
		public void addExtraBuildEntryTokenAttribute(String entry, String value) {
			addAttribute(MDEMarkerFactory.BK_BUILD_ENTRY + '.' + fExtraBuntryEntryIndex, entry);
			addAttribute(MDEMarkerFactory.BK_BUILD_TOKEN + '.' + fExtraBuntryEntryIndex, value);
	...
	private void validateBuild(IBuild build) {
		...
		for (int i = 0; i < entries.length; i++) {
			...
			if (entries[i].getTokens().length == 0)
				prepareError(name, null, MDECoreMessages.BuildErrorReporter_emptyEntry, MDEMarkerFactory.B_REMOVAL, MDEMarkerFactory.CAT_FATAL);
			...
			if (projectComplianceLevel != null) {
				...
				if (projectJavaCompatibility != null) {
					if (jreCompilationProfileEntry == null) {
						message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceMissingEntry, PROPERTY_JRE_COMPILATION_PROFILE, MDECoreMessages.BuildErrorReporter_CompilercomplianceLevel);
						prepareError(PROPERTY_JRE_COMPILATION_PROFILE, projectJavaCompatibility, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
					} else {
						if (!projectJavaCompatibility.equalsIgnoreCase(jreCompilationProfileEntry.getTokens()[0])) {
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceDifferentToken, PROPERTY_JRE_COMPILATION_PROFILE, MDECoreMessages.BuildErrorReporter_CompilercomplianceLevel);
							prepareError(PROPERTY_JRE_COMPILATION_PROFILE, projectJavaCompatibility, message, MDEMarkerFactory.B_REPLACE, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
						}
					}
				} else {
					...
					if (projectSourceCompatibility.equals(defaultComplianceOptions.get(JavaCore.COMPILER_SOURCE))) {
						if (javacSourceEntry != null) {
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_BuildEntryNotRequiredMatchesDefault, PROPERTY_JAVAC_SOURCE, MDECoreMessages.BuildErrorReporter_SourceCompatibility);
							prepareError(PROPERTY_JAVAC_SOURCE, null, message, MDEMarkerFactory.B_REMOVAL, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
						}
					} else {
						if (javacSourceEntry == null) {
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceMissingEntry, PROPERTY_JAVAC_SOURCE, MDECoreMessages.BuildErrorReporter_SourceCompatibility);
							prepareError(PROPERTY_JAVAC_SOURCE, projectSourceCompatibility, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
						} else {
							if (!projectSourceCompatibility.equalsIgnoreCase(javacSourceEntry.getTokens()[0])) {
								message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceDifferentToken, PROPERTY_JAVAC_SOURCE, MDECoreMessages.BuildErrorReporter_SourceCompatibility);
								prepareError(PROPERTY_JAVAC_SOURCE, projectSourceCompatibility, message, MDEMarkerFactory.B_REPLACE, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
							}
						}
					}
					...
					if (projectClassCompatibility.equals(defaultComplianceOptions.get(JavaCore.COMPILER_CODEGEN_TARGET_PLATFORM))) {
						if (javacTargetEntry != null) {
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_BuildEntryNotRequiredMatchesDefault, PROPERTY_JAVAC_TARGET, MDECoreMessages.BuildErrorReporter_GeneratedClassFilesCompatibility);
							prepareError(PROPERTY_JAVAC_TARGET, null, message, MDEMarkerFactory.B_REMOVAL, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
						}
					} else {
						if (javacTargetEntry == null) {
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceMissingEntry, PROPERTY_JAVAC_TARGET, MDECoreMessages.BuildErrorReporter_GeneratedClassFilesCompatibility);
							prepareError(PROPERTY_JAVAC_TARGET, projectClassCompatibility, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
						} else {
							if (!projectClassCompatibility.equalsIgnoreCase(javacTargetEntry.getTokens()[0])) {
								message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceDifferentToken, PROPERTY_JAVAC_TARGET, MDECoreMessages.BuildErrorReporter_GeneratedClassFilesCompatibility);
								prepareError(PROPERTY_JAVAC_TARGET, projectClassCompatibility, message, MDEMarkerFactory.B_REPLACE, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
							}
						}
					}
				}
			...
			if (complianceWarnSettings.size() > 0) {
				for (Iterator iterator = libraryNames.iterator(); iterator.hasNext();) {
					...
					if (matchingEntry == null) {
						...
						String message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceMissingEntry, PROPERTY_JAVAC_WARNINGS_PREFIX + libName);
						prepareError(PROPERTY_JAVAC_WARNINGS_PREFIX + libName, missingTokens, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
					} else {
						String missingTokens = ""; //$NON-NLS-1$
						for (Iterator iterator2 = complianceWarnSettings.iterator(); iterator2.hasNext();) {
							String currentIdentifier = (String) iterator2.next();
						...
						if (missingTokens.length() > 0) {
							String message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceDifferentToken, PROPERTY_JAVAC_WARNINGS_PREFIX + libName);
							prepareError(PROPERTY_JAVAC_WARNINGS_PREFIX + libName, missingTokens, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
						}
					}
				}
			...
			if (complianceErrorSettings.size() > 0) {
				for (Iterator iterator = libraryNames.iterator(); iterator.hasNext();) {
					...
					if (matchingEntry == null) {
						...
						String message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceMissingEntry, PROPERTY_JAVAC_ERRORS_PREFIX + libName);
						prepareError(PROPERTY_JAVAC_ERRORS_PREFIX + libName, missingTokens, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
					} else {
						String missingTokens = ""; //$NON-NLS-1$
						for (Iterator iterator2 = complianceErrorSettings.iterator(); iterator2.hasNext();) {
							String currentIdentifier = (String) iterator2.next();
						...
						if (missingTokens.length() > 0) {
							String message = NLS.bind(MDECoreMessages.BuildErrorReporter_ProjectSpecificJavaComplianceDifferentToken, PROPERTY_JAVAC_ERRORS_PREFIX + libName);
							prepareError(PROPERTY_JAVAC_ERRORS_PREFIX + libName, missingTokens, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaComplianceSeverity, MDEMarkerFactory.CAT_EE);
						}
					}
				}
	...
	private void validateBinIncludes(IBuildEntry binIncludes, String key) {
		...
		if (!exists) {
			prepareError(PROPERTY_BIN_INCLUDES, key, NLS.bind(MDECoreMessages.BuildErrorReporter_binIncludesMissing, key), MDEMarkerFactory.B_ADDITION, fBinInclSeverity, MDEMarkerFactory.CAT_FATAL);
		}
	}
	...
	private void validateJarsExtraClasspath(IBuildEntry javaExtra) {
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			if (tokens[i].startsWith(platform)) {
				...
				if (sep > -1) {
					IMonitorModelBase model = MonitorRegistry.findModel(path.substring(0, sep));
					if (model == null)
						exists = false;
					else {
			...
			if (!exists && !startsWithAntVariable(tokens[i])) {
				prepareError(PROPERTY_JAR_EXTRA_CLASSPATH, tokens[i], NLS.bind(MDECoreMessages.BuildErrorReporter_cannotFindJar, tokens[i]), MDEMarkerFactory.NO_RESOLUTION, fBuildSeverity, MDEMarkerFactory.CAT_OTHER);
			}
		}
	}
	...
	private void validateMissingLibraries(ArrayList sourceEntryKeys, IClasspathEntry[] cpes) {
		...
		if (model instanceof IBundlePluginModelBase && !(model instanceof IBundleFragmentModel)) {
			...
			if ((mh == null || mh.getValue() == null)) {
				if (!sourceEntryKeys.contains(DEF_SOURCE_ENTRY)) {
					prepareError(DEF_SOURCE_ENTRY, null, MDECoreMessages.BuildErrorReporter_sourceMissing, MDEMarkerFactory.NO_RESOLUTION, fSrcLibSeverity, MDEMarkerFactory.CAT_OTHER);
				}
			}
		}
		...
		for (int i = 0; i < libraries.length; i++) {
			...
			if (!sourceEntryKeys.contains(sourceEntryKey) && !containedInFragment(model.getBundleDescription(), libname))
				prepareError(sourceEntryKey, null, NLS.bind(MDECoreMessages.BuildErrorReporter_missingEntry, sourceEntryKey), MDEMarkerFactory.B_ADDITION, MDEMarkerFactory.CAT_OTHER);
		}
	}
	...
	private boolean containedInFragment(BundleDescription description, String libname) {
		...
		for (int j = 0; j < fragments.length; j++) {
			IMonitorModelBase fragmentModel = MonitorRegistry.findModel(fragments[j]);
	...
	private void validateSourceEntries(ArrayList sourceEntries, IBuildEntry srcExcludes, IClasspathEntry[] cpes) {
		if (sourceEntries == null || sourceEntries.size() == 0)
			return;
		String[] unlisted = MDEBuilderHelper.getUnlistedClasspaths(sourceEntries, fProject, cpes);
		...
		String message = MDECoreMessages.BuildErrorReporter_classpathEntryMissing1;
		if (sourceEntries.size() > 1) {
			name = DEF_SOURCE_ENTRY;
			message = MDECoreMessages.BuildErrorReporter_classpathEntryMissing;
		}
		for (int i = 0; i < unlisted.length; i++) {
			if (unlisted[i] == null || excludeList.contains(unlisted[i]))
				continue;
			BuildProblem error = prepareError(name, unlisted[i], NLS.bind(message, unlisted[i], name), MDEMarkerFactory.B_ADDITION, fSrcLibSeverity, MDEMarkerFactory.CAT_OTHER);
	...
	private void validateSourceFoldersInSrcIncludes(IBuildEntry includes) {
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			if (errorMessage != null) {
				prepareError(includes.getName(), tokens[i], errorMessage, MDEMarkerFactory.B_REMOVAL, fSrcInclSeverity, MDEMarkerFactory.CAT_OTHER);
			}
		}
	...
	private void validateIncludes(IBuildEntry includes, ArrayList sourceIncludes, int severity) {
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			String message = null;
			int fixId = MDEMarkerFactory.NO_RESOLUTION;
			if (member == null) {
				if (sourceIncludes.contains(PROPERTY_SOURCE_PREFIX + token))
					continue;
				if (token.endsWith("/")) //$NON-NLS-1$
					message = NLS.bind(MDECoreMessages.BuildErrorReporter_missingFolder, token);
				else
					message = NLS.bind(MDECoreMessages.BuildErrorReporter_missingFile, token);
				fixId = MDEMarkerFactory.B_REMOVAL;
			} else if (token.endsWith("/") && !(member instanceof IFolder)) { //$NON-NLS-1$
				message = NLS.bind(MDECoreMessages.BuildErrorReporter_entiresMustRefDirs, token);
				fixId = MDEMarkerFactory.B_REMOVE_SLASH_FILE_ENTRY;
			} else if (!token.endsWith("/") && !(member instanceof IFile)) { //$NON-NLS-1$
				message = NLS.bind(MDECoreMessages.BuildErrorReporter_dirsMustEndSlash, token);
				fixId = MDEMarkerFactory.B_APPEND_SLASH_FOLDER_ENTRY;
			}

			if (message != null)
				prepareError(includes.getName(), token, message, fixId, severity, MDEMarkerFactory.CAT_OTHER);
		}
	}
	...
	private void validateDependencyManagement(IBuildEntry bundleList) {
		...
		for (int i = 0; i < bundles.length; i++) {
			if (MonitorRegistry.findModel(bundles[i]) == null)
				prepareError(IBuildEntry.SECONDARY_DEPENDENCIES, bundles[i], NLS.bind(MDECoreMessages.BuildErrorReporter_cannotFindBundle, bundles[i]), MDEMarkerFactory.NO_RESOLUTION, fClasspathSeverity, MDEMarkerFactory.CAT_OTHER);
		}
				...
				if (useJavaProjectSettings != null) {
					...
					if (!entryCorrect) {
						...
						if (fProject.equals(root)) {
							...
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_buildEntryMissingValidPath, PROPERTY_PROJECT_SETTINGS);
						} else {
							...
							message = NLS.bind(MDECoreMessages.BuildErrorReporter_buildEntryMissingValidRelativePath, PROPERTY_PROJECT_SETTINGS);
						}
						prepareError(PROPERTY_PROJECT_SETTINGS, token, message, MDEMarkerFactory.B_REPLACE, fJavaCompilerSeverity, MDEMarkerFactory.CAT_EE);
					}
				} else {
					String token = null;
					IContainer root = PDEProject.getBundleRoot(fProject);
					...
					String message = NLS.bind(MDECoreMessages.BuildErrorReporter_buildEntryMissingProjectSpecificSettings, PROPERTY_PROJECT_SETTINGS);
					prepareError(PROPERTY_PROJECT_SETTINGS, token, message, MDEMarkerFactory.B_JAVA_ADDDITION, fJavaCompilerSeverity, MDEMarkerFactory.CAT_EE);
				}
			} else if (useJavaProjectSettings != null) {
				String message = NLS.bind(MDECoreMessages.BuildErrorReporter_buildEntryInvalidWhenNoProjectSettings, PROPERTY_PROJECT_SETTINGS);
				prepareError(PROPERTY_PROJECT_SETTINGS, null, message, MDEMarkerFactory.B_REMOVAL, fJavaCompilerSeverity, MDEMarkerFactory.CAT_EE);
			}
		}
	}
	...
	private IMarker report(String message, int line, int problemID, String buildEntry, String buildToken, int severity, String category) {
		...
		if (marker != null) {
			try {
				marker.setAttribute(MDEMarkerFactory.BK_BUILD_ENTRY, buildEntry);
				marker.setAttribute(MDEMarkerFactory.BK_BUILD_TOKEN, buildToken);
			} catch (CoreException e) {
			}
		}
		return marker;
