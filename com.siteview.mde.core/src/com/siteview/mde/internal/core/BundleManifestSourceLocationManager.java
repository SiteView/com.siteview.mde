...
import com.siteview.mde.internal.core.monitor.MonitorBase;
...
public class BundleManifestSourceLocationManager {
	...
	public SourceLocation getSourceLocation(String pluginName, Version pluginVersion) {
		IMonitorModelBase plugin = (IMonitorModelBase) fPluginToSourceBundle.get(new SourceLocationKey(pluginName, pluginVersion));
		if (plugin != null) {
	...
	public Collection getSourceLocations() {
		...
		for (Iterator iterator = fPluginToSourceBundle.values().iterator(); iterator.hasNext();) {
			IMonitorModelBase currentBundle = (IMonitorModelBase) iterator.next();
				...
				if (binaryPluginName != null && binaryPluginName.equals(pluginName)) {
					if (versionEntry != null && versionEntry.length() > 0) {
						...
						try {
							...
							MDECore.log(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, MDECoreMessages.SourceLocationManager_problemProcessingBundleManifestSourceHeader, e));
						}
	...
	private ManifestElement[] getSourceEntries(String pluginName, Version pluginVersion) {
		IMonitorModelBase sourceBundle = (IMonitorModelBase) fPluginToSourceBundle.get(new SourceLocationKey(pluginName, pluginVersion));
		if (sourceBundle != null) {
			if (sourceBundle.getMonitorBase() instanceof MonitorBase) {
				String bundleSourceEntry = ((MonitorBase) sourceBundle.getMonitorBase()).getBundleSourceEntry();
	 ...
	 */
	public void setPlugins(IMonitorModelBase[] externalModels) {
		fPluginToSourceBundle = new HashMap();
		for (int i = 0; i < externalModels.length; i++) {
			IMonitorBase currentPlugin = externalModels[i].getMonitorBase();
			if (currentPlugin instanceof MonitorBase) {
				String bundleSourceEntry = ((MonitorBase) currentPlugin).getBundleSourceEntry();
				if (bundleSourceEntry != null) {
							...
							for (int j = 0; j < manifestElements.length; j++) {
								...
								if (binaryPluginName != null && binaryPluginName.length() > 0 && versionEntry != null && versionEntry.length() > 0) {
									...
									try {
										...
										MDECore.log(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(MDECoreMessages.SourceLocationManager_problemProcessingBundleManifestSourceHeader, new Object[] {currentPlugin.getName(), versionEntry, path.toString()}), e));

									}
									fPluginToSourceBundle.put(new SourceLocationKey(binaryPluginName, version), externalModels[i]);
								} else {
									MDECore.log(new Status(IStatus.WARNING, MDECore.PLUGIN_ID, NLS.bind(MDECoreMessages.BundleManifestSourceLocationManager_problemProcessBundleManifestHeaderAttributeMissing, currentPlugin.getName())));
								}
							}
						}
					}
