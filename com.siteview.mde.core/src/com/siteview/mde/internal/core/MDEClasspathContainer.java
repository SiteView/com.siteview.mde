...
public class MDEClasspathContainer {
	...
	public static IClasspathEntry[] getExternalEntries(IMonitorModelBase model) {
		ArrayList entries = new ArrayList();
		addExternalPlugin(model, new Rule[0], entries);
		return (IClasspathEntry[]) entries.toArray(new IClasspathEntry[entries.size()]);
	}

	protected static void addExternalPlugin(IMonitorModelBase model, Rule[] rules, ArrayList entries) {
		if (new File(model.getInstallLocation()).isFile()) {
			...
			File[] extractedLibraries = MDECore.getDefault().getModelManager().getExternalModelManager().getExtractedLibraries(model);
			...
			for (int i = 0; i < libraries.length; i++) {
				if (IMonitorLibrary.RESOURCE.equals(libraries[i].getType()))
					continue;
				model = (IMonitorModelBase) libraries[i].getModel();
	...
	private static IClasspathAttribute[] getClasspathAttributes(IMonitorModelBase model) {
	...
	protected static IMonitorModelBase resolveLibraryInFragments(IMonitorModelBase model, String libraryName) {
		BundleDescription desc = model.getBundleDescription();
		if (desc != null) {
			BundleDescription[] fragments = desc.getFragments();
			for (int i = 0; i < fragments.length; i++) {
				if (new File(fragments[i].getLocation(), libraryName).exists())
					return MonitorRegistry.findModel(fragments[i]);
			}
		}
		return null;
	}
