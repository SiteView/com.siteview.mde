...
public class PDEProject {
	...
	public static IContainer getBundleRoot(IProject project) {
		...
		IEclipsePreferences node = scope.getNode(MDECore.PLUGIN_ID);
		if (node != null) {
	...
	public static void setBundleRoot(IProject project, IContainer root) throws CoreException {
		...
		if (node != null) {
			...
			try {
				...
				MDECore.getDefault().getModelManager().bundleRootChanged(project);
			} catch (BackingStoreException e) {
				throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, e.getMessage(), e));
			}
		} else {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, "Failed to retrieve project scope preference settings")); //$NON-NLS-1$
		}
	}
