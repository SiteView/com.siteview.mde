...
public class UpdateManagerHelper {
	...
	private static class LocalSite {
		...
		public void add(IMonitorModelBase model) {
			fPlugins.add(model);
		}
		...
		public String[] getRelativePluginList() {
			...
			for (int i = 0; i < fPlugins.size(); i++) {
				IMonitorModelBase model = (IMonitorModelBase) fPlugins.get(i);
	...
	public static void createPlatformConfiguration(File configLocation, IMonitorModelBase[] models, IMonitorModelBase brandingPlugin) throws CoreException {
		try {
			...
			if (message == null || message.length() == 0)
				message = MDECoreMessages.TargetPlatform_exceptionThrown;
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IStatus.ERROR, message, e));
		}
	}

	private static void addToSite(IPath path, IMonitorModelBase model, ArrayList sites) {
		if (path.getDevice() != null)
	...
	private static void createFeatureEntries(IPlatformConfiguration config, IMonitorModelBase plugin) throws MalformedURLException {
		String id = plugin.getMonitorBase().getId();
		IFeatureModel featureModel = MDECore.getDefault().getFeatureModelManager().findFeatureModel(id);
		if (featureModel != null) {
			IFeature feature = featureModel.getFeature();
			IPlatformConfiguration.IFeatureEntry featureEntry = config.createFeatureEntry(id, feature.getVersion(), id, plugin.getMonitorBase().getVersion(), true, null, new URL[] {new URL("file:" + plugin.getInstallLocation())}); //$NON-NLS-1$
