...
import com.siteview.mde.internal.core.builders.SchemaTransformer;
...
public class WorkspaceMonitorModelManager extends WorkspaceModelManager {
	...
	protected void createModel(IProject project, boolean notify) {
		...
		if (PDEProject.getOptionsFile(project).exists())
			MDECore.getDefault().getTracingOptionsManager().reset();
	...
	protected void handleFileDelta(IResourceDelta delta) {
		...
		if (file.equals(PDEProject.getOptionsFile(project))) {
			...
			IMonitorModelBase model = getPluginModel(project);
			if (model != null) {
	...
	private void handleEclipseSchemaDelta(IFile schemaFile, IResourceDelta delta) {
		...
		try {
			property = schemaFile.getSessionProperty(MDECore.SCHEMA_PREVIEW_FILE);
	...
	private void handleExtensionFileDelta(IFile file, IResourceDelta delta) {
		int kind = delta.getKind();
		IMonitorModelBase model = (IMonitorModelBase) getModel(file.getProject());
	...
	protected Object removeModel(IProject project) {
		...
		if (model instanceof IMonitorModelBase) {
			...
			addExtensionChange((IMonitorModelBase) model, IModelProviderEvent.MODELS_REMOVED);
		}
		return model;
	}
	 ...
	 */
	protected IMonitorModelBase getPluginModel(IProject project) {
		return (IMonitorModelBase) getModel(project);
	}
	 ...
	 */
	protected IMonitorModelBase[] getPluginModels() {
		initialize();
		return (IMonitorModelBase[]) fModels.values().toArray(new IMonitorModelBase[fModels.size()]);
	}
	 ...
	 */
	protected void initializeModels(IMonitorModelBase[] models) {
	...
	protected void processModelChanges() {
		...
		processModelChanges("com.siteview.mde.internal.core.IExtensionDeltaEvent", fChangedExtensions); //$NON-NLS-1$
	...
	protected void createAndFireEvent(String eventId, int type, Collection added, Collection removed, Collection changed) {
		if (eventId.equals("com.siteview.mde.internal.core.IExtensionDeltaEvent")) { //$NON-NLS-1$
			IExtensionDeltaEvent event = new ExtensionDeltaEvent(type, (IMonitorModelBase[]) added.toArray(new IMonitorModelBase[added.size()]), (IMonitorModelBase[]) removed.toArray(new IMonitorModelBase[removed.size()]), (IMonitorModelBase[]) changed.toArray(new IMonitorModelBase[changed.size()]));
			fireExtensionDeltaEvent(event);
		} else
			super.createAndFireEvent(eventId, type, added, removed, changed);
	}

	protected void addExtensionChange(IMonitorModelBase plugin, int type) {
