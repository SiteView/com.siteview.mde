...
public class MDERegistryStrategy extends RegistryStrategy {
	...
	private MDEExtensionRegistry fPDERegistry = null;

	class RegistryListener {
		IExtensionRegistry fRegistry;

		protected final void removeModels(IMonitorModelBase[] bases, boolean onlyInactive) {
	...
	class ModelListener extends RegistryListener implements IMonitorModelListener {
		...
		public void modelsChanged(MonitorModelDelta delta) {
			...
			for (int i = 0; i < entries.length; i++) {
				...
				IMonitorModelBase[] workspaceModels = entries[i].getWorkspaceModels();
	...
	class ExtensionListener extends RegistryListener implements IExtensionDeltaListener {
		...
		public void extensionsChanged(IExtensionDeltaEvent event) {
			if (fRegistry == null)
				createRegistry();
			IMonitorModelBase[] bases = event.getRemovedModels();
			...
			for (int i = 0; i < bases.length; i++) {
				ModelEntry entry = MonitorRegistry.findEntry(bases[i].getMonitorBase().getId());
	...
	public MDERegistryStrategy(File[] storageDirs, boolean[] cacheReadOnly, Object key, MDEExtensionRegistry registry) {
	...
	public SAXParserFactory getXMLParser() {
		if (xmlTracker == null) {
			xmlTracker = new ServiceTracker(MDECore.getDefault().getBundleContext(), SAXParserFactory.class.getName(), null);
			xmlTracker.open();
		}
	...
	private void addBundles(IExtensionRegistry registry, IMonitorModelBase[] bases) {
		for (int i = 0; i < bases.length; i++)
			addBundle(registry, bases[i]);
	}

	private void addBundle(IExtensionRegistry registry, IMonitorModelBase base) {
	...
	private void removeBundle(IExtensionRegistry registry, IMonitorModelBase base) {
	...
	private File getFile(IMonitorModelBase base) {
	...
	public IContributor createContributor(IMonitorModelBase base) {
	...
	public long getContributionsTimestamp() {
		IMonitorModelBase[] bases = fPDERegistry.getModels();
		long timeStamp = 0;
		for (int i = 0; i < bases.length; i++) {
