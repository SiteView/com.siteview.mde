...
public class ExternalModelManager extends AbstractModelManager {
	...
	private IMonitorModelBase[] fModels = new IMonitorModelBase[0];
	private ExternalLibraryCache fLibCache = new ExternalLibraryCache();

	protected IMonitorModelBase[] getAllModels() {
		return fModels;
	}
		...
		if (saved.equals(ICoreConstants.VALUE_SAVED_ALL)) {
			...
			for (int i = 0; i < fModels.length; i++) {
				...
				if (versionIds.contains(id)) {
					fModels[i].setEnabled(!versions.contains(new NameVersionDescriptor(id, fModels[i].getMonitorBase().getVersion())));
				} else {
	...
	public void setModels(IMonitorModelBase[] models) {
		fModels = models;
		fLibCache.cleanExtractedLibraries(fModels);
	}
	...
	public URL[] getPluginPaths() {
		...
		if (P2TargetUtils.BUNDLE_POOL.isPrefixOf(new Path(baseLocation))) {
			...
			base = MonitorPathFinder.getPluginPaths(baseLocation);
		}
		...
		URL[] additional = MonitorPathFinder.scanLocations((File[]) extraLocations.toArray(new File[extraLocations.size()]));
	 ...
	 */
	public File[] getExtractedLibraries(IMonitorModelBase model) {
 ...
 * {@link MDEClasspathContainer}.  However, because the classpath does not support nested jars, we
 ...
 * 
 * @see MDEClasspathContainer#addExternalPlugin(IMonitorModelBase, com.siteview.mde.internal.core.MDEClasspathContainer.Rule[], ArrayList)
 * @since 3.7
 */
class ExternalLibraryCache {
			...
			if (!".".equals(libName)) { //$NON-NLS-1$
				...
				if (!fDestFile.isFile()) {
					try {
						...
						MDECore.logException(ie, "Could not extract library from jarred bundle " + desc.getSymbolicName()); //$NON-NLS-1$
					}
				} else
					files.add(fDestFile);
			}
	 ...
	 */
	public void cleanExtractedLibraries(IMonitorModelBase[] targetModels) {
	...
	private File getLibraryCacheDir() {
		IPath path = MDECore.getDefault().getStateLocation();
