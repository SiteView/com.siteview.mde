...
import com.siteview.mde.internal.core.util.Headers;
...
public class MinimalState {
	...
	static {
		DEBUG = MDECore.getDefault().isDebugging() && "true".equals(Platform.getDebugOption("org.eclipse.pde.core/cache")); //$NON-NLS-1$ //$NON-NLS-2$
		DIR = MDECore.getDefault().getStateLocation().toOSString();
	...
	public void addBundle(IMonitorModelBase model, boolean update) {
		if (model == null)
			return;
	...
	public BundleDescription addBundle(IMonitorModelBase model, long bundleId) {
		try {
	...
	public BundleDescription addBundle(File bundleLocation, long bundleId) throws PluginConversionException, CoreException, IOException {
		...
		if (!hasBundleStructure) {
			...
			if (manifest == null || manifest.get(Constants.BUNDLE_SYMBOLICNAME) == null)
				throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IStatus.ERROR, "Error parsing plug-in manifest file at " + bundleLocation.toString(), null)); //$NON-NLS-1$
		}
	...
	private PluginConverter acquirePluginConverter() {
		if (fConverter == null) {
			ServiceTracker tracker = new ServiceTracker(MDECore.getDefault().getBundleContext(), PluginConverter.class.getName(), null);
			tracker.open();
	...
	protected void logResolutionErrors() {
		MultiStatus errors = new MultiStatus(MDECore.PLUGIN_ID, 1, "Problems occurred during the resolution of the target platform", //$NON-NLS-1$
				null);
