...
public class P2Utils {
	...
	private static BundleInfo[] getBundlesFromFile(URL fileURL, File home) throws IOException {
		SimpleConfiguratorManipulator manipulator = (SimpleConfiguratorManipulator) MDECore.getDefault().acquireService(SimpleConfiguratorManipulator.class.getName());
		if (manipulator == null) {
			return null;
		}
	...
	public static URL writeBundlesTxt(Map bundles, int defaultStartLevel, boolean defaultAutoStart, File directory, String osgiBundleList) {
		...
		for (Iterator iterator = bundles.keySet().iterator(); iterator.hasNext();) {
			IMonitorModelBase currentModel = (IMonitorModelBase) iterator.next();
			IMonitorBase base = currentModel.getMonitorBase();

			BundleInfo info = new BundleInfo();
			String installLocation = currentModel.getInstallLocation();
			if (installLocation != null) {
				info.setLocation(new File(installLocation).toURI());
				if (base instanceof MonitorBase && ((MonitorBase) base).getBundleSourceEntry() != null) {
					...
					if ("default".equals(levelString)) {//$NON-NLS-1$
						...
						try {
							...
							MDECore.log(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, "Error writing bundles, could not parse start level for bundle " + currentModel)); //$NON-NLS-1$
						}
					}
				...
				MDECore.log(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, "Error writing bundles, could not find the bundle location for bundle " + currentModel)); //$NON-NLS-1$
			}
		}
	...
	public static boolean profileExists(String profileID, File p2DataArea) throws CoreException {
		...
		if (provider == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, MDECoreMessages.P2Utils_UnableToAcquireP2Service));
		}
