...
public class Feature extends VersionableObject implements IFeature {
	...
	public IMonitorModelBase getReferencedModel(IFeaturePlugin reference) {
		IMonitorModelBase model = MonitorRegistry.findModel(reference.getId());
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.ifeature.IFeature#getFeatureInfo(int)
	 */
	...
	public void computeImports() throws CoreException {
		...
		for (int i = 0; i < fPlugins.size(); i++) {
			...
			ModelEntry entry = MonitorRegistry.findEntry(fp.getId());
			if (entry == null)
				continue;
			IMonitorModelBase[] models = entry.getActiveModels();
			for (int j = 0; j < models.length; j++) {
				IMonitorModelBase m = models[j];
				if (fp.getVersion().equals(m.getMonitorBase().getVersion()) || fp.getVersion().equals("0.0.0")) //$NON-NLS-1$
					model = m;
			}
			if (model != null) {
				addPluginImports(preservedImports, newImports, model.getMonitorBase());
				if (model.isFragmentModel()) {
					...
					if (!(versionRange == null || VersionRange.emptyRange.equals(versionRange))) {
						...
						match = MonitorBase.getMatchRule(versionRange);
					}
	 ...
	 */
	private void addPluginImports(List preservedImports, List newImports, IMonitorBase plugin) throws CoreException {
		IMonitorImport[] pluginImports = plugin.getImports();
		for (int i = 0; i < pluginImports.length; i++) {
			IMonitorImport pluginImport = pluginImports[i];
