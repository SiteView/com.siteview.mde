...
public class FeatureImport extends VersionableObject implements IFeatureImport {
	...
	public IMonitor getPlugin() {
		if (id != null && fType == PLUGIN) {
			...
			return model instanceof IMonitorModel ? ((IMonitorModel) model).getMonitor() : null;
		}
		return null;
	}
	...
	public IFeature findFeature(String id, String version, int match) {
		IFeatureModel[] models = MDECore.getDefault().getFeatureModelManager().findFeatureModels(id);
