...
import com.siteview.mde.internal.core.target.provisional.LoadTargetDefinitionJob;
import com.siteview.mde.internal.core.util.CoreUtility;

public class MDEState extends MinimalState {

	private MDEAuxiliaryState fAuxiliaryState;
	 ...
	 * @param state
	 */
	public MDEState(MDEState state) {
		super(state);
		fCombined = false;
	...
	private void copyAuxiliaryState() {
		...
		fAuxiliaryState = new MDEAuxiliaryState();
		fAuxiliaryState.readPluginInfoCache(new File(DIR, Long.toString(fTargetTimestamp) + ".target")); //$NON-NLS-1$
	}

	private void copyModels(MDEState state) {
		IMonitorModelBase[] bases = state.getTargetModels();
		...
		for (int i = 0; i < bases.length; i++) {
			...
			IMonitorModelBase model = createExternalModel(newBD);
		...
		bases = MonitorRegistry.getWorkspaceModels();
		for (int i = 0; i < bases.length; i++) {
			removeBundleDescription(bases[i].getBundleDescription());
		}
	}

	public MDEState(URL[] urls, boolean resolve, IProgressMonitor monitor) {
		this(new URL[0], urls, resolve, false, monitor);
	}

	public MDEState(URL[] workspace, URL[] target, boolean resolve, boolean removeTargetDuplicates, IProgressMonitor monitor) {
	...
	private void createNewTargetState(boolean resolve, URL[] urls, IProgressMonitor monitor) {
		...
		for (int i = 0; i < urls.length; i++) {
			...
			try {
				...
				MDECore.log(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IStatus.ERROR, "Invalid manifest format at " + file.getAbsolutePath(), //$NON-NLS-1$
						null));
			} finally {
				monitor.worked(1);
			}
	...
	private IMonitorModelBase[] createTargetModels(BundleDescription[] bundleDescriptions) {
		HashMap models = new HashMap((4 / 3) * bundleDescriptions.length + 1);
		for (int i = 0; i < bundleDescriptions.length; i++) {
			BundleDescription desc = bundleDescriptions[i];
			IMonitorModelBase base = createExternalModel(desc);
			fTargetModels.add(base);
			models.put(desc.getSymbolicName(), base);
		}
		if (models.isEmpty())
			return new IMonitorModelBase[0];
		return (IMonitorModelBase[]) models.values().toArray(new IMonitorModelBase[models.size()]);
	}
	...
	private void readWorkspaceState(URL[] urls) {
		...
		if (fCombined) {
			...
			for (int i = 0; i < bundles.length; i++) {
				...
				IMonitorModelBase model = createWorkspaceModel(newbundle);
	...
	private IMonitorModelBase createWorkspaceModel(BundleDescription desc) {
		String projectName = fAuxiliaryState.getProject(desc.getBundleId());
		if (projectName == null)
			return null;
		IProject project = MDECore.getWorkspace().getRoot().getProject(projectName);
		if (!project.exists())
			return null;
		...
		if (desc.getHost() == null)
			model = new WorkspaceMonitorModel(pluginXml, true);
		else
	...
	private IMonitorModelBase createExternalModel(BundleDescription desc) {
		ExternalMonitorModelBase model = null;
		if (desc.getHost() == null)
			model = new ExternalMonitorModel();
		else
	...
	public IMonitorModelBase[] getTargetModels() {
		return (IMonitorModelBase[]) fTargetModels.toArray(new IMonitorModelBase[fTargetModels.size()]);
	}

	public IMonitorModelBase[] getWorkspaceModels() {
		return (IMonitorModelBase[]) fWorkspaceModels.toArray(new IMonitorModelBase[fWorkspaceModels.size()]);
	}
	...
	public void saveWorkspaceState() {
		...
		if (!"true".equals(System.getProperty("pde.nocache")) && shouldSaveState(models)) { //$NON-NLS-1$ //$NON-NLS-2$
			...
			saveState(state, dir);
			MDEAuxiliaryState.writePluginInfo(models, dir);
		}
	...
	private long computeTimestamp(IMonitorModelBase[] models) {
		URL[] urls = new URL[models.length];
		for (int i = 0; i < models.length; i++) {
			try {
	...
	private boolean shouldSaveState(IMonitorModelBase[] models) {
		int nonOSGiModels = 0;
		for (int i = 0; i < models.length; i++) {
	...
	private void clearStaleStates(String extension, long latest) {
		File dir = new File(MDECore.getDefault().getStateLocation().toOSString());
