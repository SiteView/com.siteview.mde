 ...
 *******************************************************************************/
package com.siteview.mde.internal.core.target.provisional;
...
public class LoadTargetDefinitionJob extends WorkspaceJob {
	...
	public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
		try {
			...
			MDECore.getDefault().getPreferencesManager().savePluginPreferences();
			if (monitor.isCanceled()) {
				return Status.CANCEL_STATUS;
			}
		} finally {
	 ...
	 */
	private void loadArgs(MDEPreferencesManager pref, IProgressMonitor monitor) {
	 ...
	 * @param monitor progress monitor
	 */
	private void loadEnvironment(MDEPreferencesManager pref, IProgressMonitor monitor) {
	 ...
	 */
	private void setEnvironmentPref(MDEPreferencesManager pref, String key, String value) {
		if (value == null) {
	 ...
	 * @param monitor
	 */
	private void loadJRE(MDEPreferencesManager pref, IProgressMonitor monitor) {
	 ...
	 * @param monitor progress monitor
	 */
	private void loadImplicitPlugins(MDEPreferencesManager pref, IProgressMonitor monitor) {
	 ...
	 */
	private void loadPlugins(MDEPreferencesManager pref, IProgressMonitor monitor) throws CoreException {
	 ...
	 * @param pref
	 */
	private void loadAdditionalPreferences(MDEPreferencesManager pref) throws CoreException {
	...
	private void handleReload(String targetLocation, List additionalLocations, MDEPreferencesManager pref, IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.LoadTargetDefinitionJob_1, e));
				}
			}
			...
			MDEState state = new MDEState(urls, true, new SubProgressMonitor(monitor, 45));
			IMonitorModelBase[] models = state.getTargetModels();
			for (int i = 0; i < models.length; i++) {
				NameVersionDescriptor nv = new NameVersionDescriptor(models[i].getMonitorBase().getId(), models[i].getMonitorBase().getVersion());
