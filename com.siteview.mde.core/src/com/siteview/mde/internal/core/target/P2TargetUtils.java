...
public class P2TargetUtils {
	...
	static {
		try {
			AGENT_LOCATION = URIUtil.fromString("file:" + MDECore.getDefault().getStateLocation().append(".p2")); //$NON-NLS-1$//$NON-NLS-2$
		} catch (Exception e) {
			// should never happen
		}
	}
	 ...
	 * based targets.
	 */
	public static final IPath BUNDLE_POOL = MDECore.getDefault().getStateLocation().append(".bundle_pool"); //$NON-NLS-1$

	/**
	 * Path to the local directory where install folders are created for p2 profile
	 * based targets.
	 */
	static final IPath INSTALL_FOLDERS = MDECore.getDefault().getStateLocation().append(".install_folders"); //$NON-NLS-1$	
	 ...
	 */
	static final String PROP_INSTALLED_IU = MDECore.PLUGIN_ID + ".installed_iu"; //$NON-NLS-1$

	/**
	 * Profile property that keeps track of provisioning mode for the target
	 * (slice versus plan).
	 */
	static final String PROP_PROVISION_MODE = MDECore.PLUGIN_ID + ".provision_mode"; //$NON-NLS-1$

	/**
	 * Profile property that keeps track of provisioning mode for the target
	 * (all environments/true versus false).
	 */
	static final String PROP_ALL_ENVIRONMENTS = MDECore.PLUGIN_ID + ".all_environments"; //$NON-NLS-1$	

	/**
	 * Profile property that keeps track of the target sequence number
	 */
	static final String PROP_SEQUENCE_NUMBER = MDECore.PLUGIN_ID + ".sequence"; //$NON-NLS-1$	

	/**
	 * Profile property that tracks whether or not source to be auto-included
	 */
	static final String PROP_AUTO_INCLUDE_SOURCE = MDECore.PLUGIN_ID + ".autoIncludeSource"; //$NON-NLS-1$	
	...
	public static List cleanOrphanedTargetDefinitionProfiles() throws CoreException {
		...
		ITargetPlatformService tps = (ITargetPlatformService) MDECore.getDefault().acquireService(ITargetPlatformService.class.getName());
	...
	public static IProvisioningAgent getAgent() throws CoreException {
		...
		IProvisioningAgentProvider provider = (IProvisioningAgentProvider) MDECore.getDefault().acquireService(IProvisioningAgentProvider.SERVICE_NAME);
		try {
			if (provider == null) {
				throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_7));
			}
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_7, e));
		}
	}
	...
	public static IProvisioningAgent getGlobalAgent() throws CoreException {
		IProvisioningAgent agent = (IProvisioningAgent) MDECore.getDefault().acquireService(IProvisioningAgent.SERVICE_NAME);
		if (agent == null)
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_11));
		return agent;
	}
	...
	public static IAgentLocation getAgentLocation() throws CoreException {
		...
		if (result == null)
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_10));
		return result;
	}
	...
	public static IArtifactRepositoryManager getArtifactRepositoryManager() throws CoreException {
		...
		if (manager == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_3));
		}
		return manager;
	}
	...
	public static IEngine getEngine() throws CoreException {
		...
		if (engine == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_4));
		}
		return engine;
	}
	...
	public static GarbageCollector getGarbageCollector() throws CoreException {
		...
		if (engine == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_9));
		}
		return engine;
	}
	...
	public static IPlanner getPlanner() throws CoreException {
		...
		if (planner == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_5));
		}
		return planner;
	}
	...
	public static IPreferencesService getPreferences() {
		return (IPreferencesService) MDECore.getDefault().acquireService(IPreferencesService.class.getName());
	}
	...
	private void createProfile(ITargetDefinition target) throws CoreException, ProvisionException {
		...
		IProfileRegistry registry = getProfileRegistry();
		if (registry == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.AbstractTargetHandle_0));
		}
	...
	public static IProfileRegistry getProfileRegistry() throws CoreException {
		...
		if (result == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_8));
		}
		return result;
	}
	...
	public static IMetadataRepositoryManager getRepoManager() throws CoreException {
		...
		if (manager == null) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.IUBundleContainer_2));
		}
		return manager;
	}
	...
	static IQueryable getQueryableMetadata(URI[] repos, IProgressMonitor monitor) throws CoreException {
		...
		MultiStatus repoStatus = new MultiStatus(MDECore.PLUGIN_ID, 0, Messages.IUBundleContainer_ProblemsLoadingRepositories, null);
	...
	private void resolveWithPlanner(ITargetDefinition target, IProgressMonitor monitor) throws CoreException {
		...
		if (installerPlan != null) {
			...
			MDECore.log(new Status(IStatus.INFO, MDECore.PLUGIN_ID, Messages.IUBundleContainer_6));
		}
			...
			if (container instanceof IUBundleContainer) {
				...
				for (int j = 0; j < ids.length; j++) {
					...
					if (queryResult.isEmpty())
						throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(Messages.IUBundleContainer_1, ids[j])));
