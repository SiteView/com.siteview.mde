...
public class LocalTargetHandle extends AbstractTargetHandle {
	 ...
	 * per project.
	 */
	static final IPath LOCAL_TARGET_CONTAINER_PATH = MDECore.getDefault().getStateLocation().append(".local_targets"); //$NON-NLS-1$
	...
	static ITargetHandle restoreHandle(URI uri) throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.LocalTargetHandle_0, null));
		} catch (NumberFormatException e) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.LocalTargetHandle_0, e));
		}
	}
	...
	protected InputStream getInputStream() throws CoreException {
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.LocalTargetHandle_1, e));
		}
	}
	...
	public String getMemento() throws CoreException {
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.LocalTargetHandle_2, e));
		}
	}
	...
	void delete() throws CoreException {
		...
		if (file.exists()) {
			file.delete();
			if (file.exists()) {
				throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(Messages.LocalTargetHandle_3, file.getName())));
			}
		}
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.target.impl.AbstractTargetHandle#getOutputStream()
	 */
	protected OutputStream getOutputStream() throws CoreException {
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.LocalTargetHandle_5, e));
		}
	}
	...
	void save(ITargetDefinition definition) throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(Messages.LocalTargetHandle_4, getFile().getName()), e));
		}
	}
