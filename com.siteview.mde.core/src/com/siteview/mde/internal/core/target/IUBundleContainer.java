...
public class IUBundleContainer extends AbstractBundleContainer {
	...
	static {
		DEBUG_PROFILE = MDECore.getDefault().isDebugging() && "true".equals(Platform.getDebugOption("org.eclipse.pde.core/target/profile")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	IInstallableUnit[] cacheIUs(ITargetDefinition target) throws CoreException {
		...
		for (int i = 0; i < fIds.length; i++) {
			...
			if (queryResult.isEmpty())
				throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(Messages.IUBundleContainer_1, fIds[i])));
