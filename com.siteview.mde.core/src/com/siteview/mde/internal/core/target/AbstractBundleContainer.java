...
public abstract class AbstractBundleContainer implements IBundleContainer {
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.target.provisional.IBundleContainer#isResolved()
	 */
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.target.provisional.IBundleContainer#resolve(com.siteview.mde.internal.core.target.provisional.ITargetDefinition, org.eclipse.core.runtime.IProgressMonitor)
	 */
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.target.provisional.IBundleContainer#getStatus()
	 */
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.target.provisional.IBundleContainer#getBundles()
	 */
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.target.provisional.IBundleContainer#getFeatures()
	 */
	...
	protected Map loadManifest(BundleInfo bundle) throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IResolvedBundle.STATUS_INVALID_MANIFEST, NLS.bind(Messages.DirectoryBundleContainer_3, bundle.getSymbolicName()), e));
		} catch (IOException e) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IResolvedBundle.STATUS_INVALID_MANIFEST, NLS.bind(Messages.DirectoryBundleContainer_3, bundle.getSymbolicName()), e));
		} finally {
	...
	protected Map loadManifest(File bundleLocation) throws CoreException {
		...
		try {
			...
			if (manifestStream == null) {
				throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IResolvedBundle.STATUS_INVALID_MANIFEST, NLS.bind(Messages.DirectoryBundleContainer_3, bundleLocation.getAbsolutePath()), null));
			}
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IResolvedBundle.STATUS_INVALID_MANIFEST, NLS.bind(Messages.DirectoryBundleContainer_3, bundleLocation.getAbsolutePath()), e));
		} catch (IOException e) {
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IResolvedBundle.STATUS_INVALID_MANIFEST, NLS.bind(Messages.DirectoryBundleContainer_3, bundleLocation.getAbsolutePath()), e));
		} finally {
			closeZipFileAndStream(manifestStream, jarFile);
	...
	private Map loadPluginXML(File pluginDir) throws CoreException {
		...
		if (pxml.exists() || fxml.exists()) {
			...
			PluginConverter converter = (PluginConverter) MDECore.getDefault().acquireService(PluginConverter.class.getName());
			if (converter != null) {
				try {
					...
					throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, NLS.bind(Messages.DirectoryBundleContainer_2, pluginDir.getAbsolutePath()), e));
				}
			}
		}
		return null;
	...
	protected IResolvedBundle generateBundle(File file) throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, IResolvedBundle.STATUS_INVALID_MANIFEST, NLS.bind(Messages.DirectoryBundleContainer_3, file.getAbsolutePath()), e));
		}
		return null;
	}
	...
	private IExtensionRegistry getRegistry() {
		if (fRegistry == null) {
			...
			RegistryContributor contributor = new RegistryContributor(MDECore.PLUGIN_ID, MDECore.PLUGIN_ID, null, null);
	...
	public String[] getVMArguments() {
		...
		if (fVMArgs == null) {
			try {
				FrameworkAdmin fwAdmin = (FrameworkAdmin) MDECore.getDefault().acquireService(FrameworkAdmin.class.getName());
				if (fwAdmin == null) {
					Bundle fwAdminBundle = Platform.getBundle(FWK_ADMIN_EQ);
					if (fwAdminBundle != null){
						fwAdminBundle.start();
						fwAdmin = (FrameworkAdmin) MDECore.getDefault().acquireService(FrameworkAdmin.class.getName());
					}
				}
				if (fwAdmin != null) {
