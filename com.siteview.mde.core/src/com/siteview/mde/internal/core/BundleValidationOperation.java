...
public class BundleValidationOperation implements IWorkspaceRunnable {
	...
	public BundleValidationOperation(IMonitorModelBase[] models) {
		this(models, new Dictionary[] {TargetPlatformHelper.getTargetEnvironment()});
	}

	public BundleValidationOperation(IMonitorModelBase[] models, Dictionary[] properties) {
		fModels = models;
		...
		for (int i = 0; i < bundles.length; i++) {
			...
			if (!desc.isResolved()) {
				...
				if (dups.length > 1) {
					...
					MultiStatus status = new MultiStatus(MDECore.PLUGIN_ID, 0, NLS.bind(MDECoreMessages.BundleValidationOperation_multiple_singletons, new String[] {Integer.toString(dups.length), desc.getSymbolicName()}), null);
					for (int j = 0; j < dups.length; j++) {
						status.add(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, dups[j].getLocation()));
					}
