...
public class SourceLocationManager implements ICoreConstants {
	 ...
	 */
	public IPath findSourcePath(IMonitorBase pluginBase, IPath sourceLibraryPath) {
	 ...
	 */
	public URL findSourceFile(IMonitorBase pluginBase, IPath filePath) {
		if (pluginBase.getId() == null || pluginBase.getVersion() == null) {
			return null;
		}
	 ...
	 */
	public File findSourcePlugin(IMonitorBase pluginBase) {
		if (pluginBase.getId() == null || pluginBase.getVersion() == null) {
			return null;
		}
	 ...
	 */
	public boolean hasBundleManifestLocation(IMonitorBase plugin) {
	 ...
	 */
	public Set findAllSourceRootsInSourceLocation(IMonitorBase plugin) {
		if (plugin.getId() == null || plugin.getVersion() == null) {
	 ...
	 */
	public Set findSourceRoots(IMonitorBase plugin) {
		if (plugin.getId() == null || plugin.getVersion() == null) {
			return Collections.EMPTY_SET;
		}
	...
	public List getUserLocations() {
		...
		String pref = MDECore.getDefault().getPreferencesManager().getString(P_SOURCE_LOCATIONS);
		if (pref.length() > 0) {
	 ...
	 */
	private IPath getRelativePath(IMonitorBase pluginBase, IPath sourceFilePath) {
		try {
	 ...
	 * @return path to the source file or <code>null</code> if one could not be found or if the file does not exist
	 */
	private IPath searchBundleManifestLocations(IMonitorBase pluginBase) {
	...
	private static List processExtensions() {
		...
		IExtension[] extensions = MDECore.getDefault().getExtensionsRegistry().findExtensions(MDECore.PLUGIN_ID + ".source", false); //$NON-NLS-1$
	...
	protected BundleManifestSourceLocationManager initializeBundleManifestLocations() {
		...
		manager.setPlugins(MDECore.getDefault().getModelManager().getExternalModels());
		return manager;
	}
