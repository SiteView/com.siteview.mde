 ...
 *******************************************************************************/
package com.siteview.mde.internal.core.converter;
...
import com.siteview.mde.internal.core.build.Build;
import com.siteview.mde.internal.core.build.WorkspaceBuildModel;
import com.siteview.mde.internal.core.converter.MonitorConverterParser.PluginInfo;
...
public class MonitorConverter {
	...
	private static MonitorConverter instance;
	...
	public static MonitorConverter getDefault() {
		if (instance == null)
			instance = new MonitorConverter(MDECore.getDefault().getBundleContext());
		return instance;
	}

	public MonitorConverter(BundleContext context) {
	...
	private void fillPluginInfo(File pluginBaseLocation) throws PluginConversionException {
		...
		if (pluginFile == null) {
			throw new PluginConversionException(NLS.bind(MDECoreMessages.PluginConverter_EclipseConverterFileNotFound, pluginBaseLocation.getAbsolutePath()));
		}
	...
	public void writeManifest(File generationLocation, Map manifestToWrite, boolean compatibilityManifest) throws PluginConversionException {
		...
		try {
			...
			if (!generationLocation.isFile()) {
				String message = NLS.bind(MDECoreMessages.PluginConverter_EclipseConverterErrorCreatingBundleManifest, this.pluginInfo.getUniqueId(), generationLocation);
	...
	private boolean requireRuntimeCompatibility() {
		...
		for (Iterator iter = requireList.iterator(); iter.hasNext();) {
			if (((MonitorConverterParser.Prerequisite) iter.next()).getName().equalsIgnoreCase(PI_RUNTIME_COMPATIBILITY))
				return true;
		}
		return false;
	}
	...
	private void generateRequireBundle() {
		...
		for (Iterator iter = requiredBundles.iterator(); iter.hasNext();) {
			MonitorConverterParser.Prerequisite element = (MonitorConverterParser.Prerequisite) iter.next();
	...
	private PluginInfo parsePluginInfo(URL pluginLocation) throws PluginConversionException {
		...
		try {
			...
			return new MonitorConverterParser(context, target).parsePlugin(input);
		} catch (Exception e) {
			String message = NLS.bind(MDECoreMessages.PluginConverter_EclipseConverterErrorParsingPluginManifest, pluginManifestLocation);
			throw new PluginConversionException(message, e);
		} finally {
			if (input != null)
				try {
	...
	public static boolean upToDate(File generationLocation, File pluginLocation, byte manifestType) {
		...
		try {
			generatedFrom = ManifestElement.parseHeader(MonitorConverter.GENERATED_FROM, secondLine)[0];
	...
	public synchronized File convertManifest(File pluginBaseLocation, File bundleManifestLocation, boolean compatibilityManifest, String target, boolean analyseJars, Dictionary devProperties) throws PluginConversionException {
		if (bundleManifestLocation == null)
			throw new PluginConversionException(MDECoreMessages.PluginConverter_BundleLocationIsNull);
