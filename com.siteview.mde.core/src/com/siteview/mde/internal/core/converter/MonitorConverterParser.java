 ...
 * Internal class.
 */
public class MonitorConverterParser extends DefaultHandler implements IModel {
	...
	public class PluginInfo {
		...
		public ArrayList getRequires() {
			if (!TARGET21.equals(target) && schemaVersion == null && !requiresExpanded) {
				...
				if (requires == null) {
					...
					requires.add(new Prerequisite(MonitorConverter.PI_RUNTIME_COMPATIBILITY, null, false, false, null));
				} else {
					...
					for (int i = 0; i < requires.size(); i++) {
						...
						if ("org.eclipse.ui".equals(analyzed.getName())) { //$NON-NLS-1$					
							...
							requires.add(i + 1, new Prerequisite(MonitorConverter.PI_RUNTIME_COMPATIBILITY, null, false, analyzed.isExported(), null));
						}
					}
					if (!requires.contains(new Prerequisite(MonitorConverter.PI_RUNTIME_COMPATIBILITY, null, false, false, null))) {
						requires.add(new Prerequisite(MonitorConverter.PI_RUNTIME_COMPATIBILITY, null, false, false, null));
					}
		...
		public String validateForm() {
			if (this.pluginId == null) {
				return NLS.bind(MDECoreMessages.PluginParser_EclipseConverterMissingAttribute, new String[] {getRoot(), PLUGIN_ID, getRoot()});
			}
			if (this.pluginName == null) {
				return NLS.bind(MDECoreMessages.PluginParser_EclipseConverterMissingAttribute, new String[] {getRoot(), PLUGIN_NAME, getRoot()});
			}
			if (this.version == null) {
				return NLS.bind(MDECoreMessages.PluginParser_EclipseConverterMissingAttribute, new String[] {getRoot(), PLUGIN_VERSION, getRoot()});
			}
			if (isFragment() && this.masterPluginId == null) {
				return NLS.bind(MDECoreMessages.PluginParser_EclipseConverterMissingAttribute, new String[] {getRoot(), FRAGMENT_PLUGIN_ID, getRoot()});
			}
			if (isFragment() && this.masterVersion == null) {
				return NLS.bind(MDECoreMessages.PluginParser_EclipseConverterMissingAttribute, new String[] {getRoot(), FRAGMENT_PLUGIN_VERSION, getRoot()});
			}
			return null;
		}
	}
	...
	public MonitorConverterParser(BundleContext context, Version target) {
		super();
	...
	public void parsePluginRequiresImport(Attributes attributes) {
		...
		if (plugin == null)
			return;
		if (plugin.equals(MonitorConverter.PI_BOOT))
			return;
		if (plugin.equals(MonitorConverter.PI_RUNTIME_COMPATIBILITY))
