...
public class DependencyManager {
	 ...
	 * {@link IMonitorModelBase}. The set includes the id of the given model base
	 * as well as all computed implicit / optional dependencies. 
	 * 
	 * @param model the {@link IMonitorModelBase} to compute dependencies for
	 ...
	 */
	public static Set getSelfAndDependencies(IMonitorModelBase model, String[] excludeFragments) {
	...
	/** 
	 * Returns a {@link Set} of bundle ids for the dependents of the given
	 * {@link IMonitorModelBase}s. The set includes the ids of the given model bases
	 * as well as all computed implicit / optional dependencies.
	 * 
	 * @param models the array of {@link IMonitorModelBase}s to compute dependencies for
	 * @param excludeFragments a collection of <b>fragment</b> bundle symbolic names to exclude from the dependency resolution
	 *  or <code>null</code> if none
	 * @return a set of bundle IDs
	 */
	public static Set getSelfandDependencies(IMonitorModelBase[] models, String[] excludeFragments) {
	 ...
	 * in this array that are not {@link IMonitorModelBase}s are ignored.
	...
	private static Set getDependencies(Object[] selected, String[] implicit, State state, boolean removeSelf, boolean includeOptional, Set excludeFragments) {
		...
		if (removeSelf) {
			for (int i = 0; i < selected.length; i++) {
				...
				IMonitorModelBase model = (IMonitorModelBase) selected[i];
				set.remove(model.getMonitorBase().getId());
			}
		}
		return set;
	}
	...
	/**
	 * Computes the set of implicit dependencies from the {@link MDEPreferencesManager}
