...
public class ModelEntry extends PlatformObject {
	...
	public IMonitorModelBase[] getWorkspaceModels() {
		return (IMonitorModelBase[]) fWorkspaceEntries.toArray(new IMonitorModelBase[fWorkspaceEntries.size()]);
	}
	...
	public IMonitorModelBase[] getExternalModels() {
		return (IMonitorModelBase[]) fExternalEntries.toArray(new IMonitorModelBase[fExternalEntries.size()]);
	}
	...
	public IMonitorModelBase getModel() {
		IMonitorModelBase model = getBestCandidate(getWorkspaceModels());
		if (model == null)
			model = getBestCandidate(getExternalModels());
		return model;
	}

	private IMonitorModelBase getBestCandidate(IMonitorModelBase[] models) {
	...
	public IMonitorModelBase[] getActiveModels() {
		...
		if (fExternalEntries.size() > 0) {
			...
			for (int i = 0; i < fExternalEntries.size(); i++) {
				IMonitorModelBase model = (IMonitorModelBase) fExternalEntries.get(i);
				if (model.isEnabled())
					list.add(model);
			}
	 ...
	 * model exists.
	 */
	public IMonitorModelBase getModel(BundleDescription desc) {
		if (desc == null)
			return null;

		for (int i = 0; i < fWorkspaceEntries.size(); i++) {
			IMonitorModelBase model = (IMonitorModelBase) fWorkspaceEntries.get(i);
