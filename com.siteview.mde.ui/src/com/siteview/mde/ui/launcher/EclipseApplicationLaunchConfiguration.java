 ...
 * </p>
 * @since 3.2
 * @deprecated use {@link com.siteview.mde.launching.EclipseApplicationLaunchConfiguration}
 * @see com.siteview.mde.launching.AbstractPDELaunchConfiguration
 */
public class EclipseApplicationLaunchConfiguration extends AbstractPDELaunchConfiguration {
	...
	/*
	 * (non-Javadoc)
	 * @see com.siteview.mde.ui.launcher.AbstractPDELaunchConfiguration#getProgramArguments(org.eclipse.debug.core.ILaunchConfiguration)
	 */
	public String[] getProgramArguments(ILaunchConfiguration configuration) throws CoreException {
		ArrayList programArgs = new ArrayList();

		// If a product is specified, then add it to the program args
		if (configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.USE_PRODUCT, false)) {
			String product = configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.PRODUCT, ""); //$NON-NLS-1$
			if (product.length() > 0) {
				...
				programArgs.add(configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.APPLICATION, "")); //$NON-NLS-1$
			}
		} else {
			// specify the application to launch
			programArgs.add("-application"); //$NON-NLS-1$
			programArgs.add(configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.APPLICATION, TargetPlatform.getDefaultApplication()));
		}
		...
		if (configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.USEFEATURES, false)) {
			validateFeatures();
			IPath installPath = MDEPlugin.getWorkspace().getRoot().getLocation();
			...
			if (!configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.CONFIG_USE_DEFAULT_AREA, true)) {
			...
			TargetPlatform.createPlatformConfiguration(getConfigDir(configuration), (IMonitorModelBase[]) fAllBundles.values().toArray(new IMonitorModelBase[fAllBundles.size()]), brandingId != null ? (IMonitorModelBase) fAllBundles.get(brandingId) : null);
	...
	private void validateFeatures() throws CoreException {
		...
		if (badStructure) {
			throw new CoreException(LauncherUtils.createErrorStatus(MDEMessages.WorkbenchLauncherConfigurationDelegate_badFeatureSetup));
		}
	...
	private IPath getProductPath() {
		return MDEPlugin.getWorkspace().getRoot().getLocation().removeLastSegments(1);
	}
	...
	/*
	 * (non-Javadoc)
	 * @see com.siteview.mde.ui.launcher.AbstractPDELaunchConfiguration#getConfigDir(org.eclipse.debug.core.ILaunchConfiguration)
	 */
	protected File getConfigDir(ILaunchConfiguration config) {
		if (fConfigDir == null) {
			try {
				if (config.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.USEFEATURES, false) && config.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.CONFIG_USE_DEFAULT_AREA, true)) {
	...
	protected void clear(ILaunchConfiguration configuration, IProgressMonitor monitor) throws CoreException {
		...
		if (configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.CONFIG_CLEAR_AREA, false))
			CoreUtility.deleteContent(getConfigDir(configuration));
	}

	/* (non-Javadoc)
	 * @see com.siteview.mde.ui.launcher.AbstractPDELaunchConfiguration#preLaunchCheck(org.eclipse.debug.core.ILaunchConfiguration, org.eclipse.debug.core.ILaunch, org.eclipse.core.runtime.IProgressMonitor)
	 */
	...
	private void validateConfigIni(ILaunchConfiguration configuration) throws CoreException {
		if (!configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.CONFIG_GENERATE_DEFAULT, true)) {
			String templateLoc = configuration.getAttribute(com.siteview.mde.launching.IPDELauncherConstants.CONFIG_TEMPLATE_LOCATION, ""); //$NON-NLS-1$
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.ui.launcher.AbstractPDELaunchConfiguration#getVMArguments(org.eclipse.debug.core.ILaunchConfiguration)
	 */
