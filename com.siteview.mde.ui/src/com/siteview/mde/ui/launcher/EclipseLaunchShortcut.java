...
import com.siteview.mde.internal.core.DependencyManager;
import com.siteview.mde.internal.core.TargetPlatformHelper;
import com.siteview.mde.internal.core.product.WorkspaceProductModel;
...
import com.siteview.mde.internal.ui.launcher.ApplicationSelectionDialog;
import com.siteview.mde.internal.ui.launcher.LaunchAction;
import java.util.*;
...
public class EclipseLaunchShortcut extends AbstractLaunchShortcut {
	...
	private IMonitorModelBase fModel = null;
	...
	private void launch(IMonitorModelBase model, String mode) {
		fModel = model;
		fApplicationName = null;
		if (fModel != null) {
			...
			if (applicationNames.length == 1) {
				...
				ApplicationSelectionDialog dialog = new ApplicationSelectionDialog(MDEPlugin.getActiveWorkbenchShell().getShell(), applicationNames, mode);
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.ui.launcher.AbstractLaunchShortcut#findLaunchConfiguration(java.lang.String)
	 */
	protected ILaunchConfiguration findLaunchConfiguration(String mode) {
		...
		if (config != null) {
			try {
				if (!(config.getAttribute(IPDELauncherConstants.USE_DEFAULT, false)) && (config.getAttribute(IMDEUIConstants.GENERATED_CONFIG, false))) {
			...
			for (int i = 0; i < extensions.length; i++) {
				...
				if ("org.eclipse.core.runtime.products".equals(point)) { //$NON-NLS-1$
					if (ext.getChildCount() == 1) {
						IMonitorElement prod = (IMonitorElement) ext.getChildren()[0];
						if (prod.getName().equals("product")) { //$NON-NLS-1$
							IMonitorAttribute attr = prod.getAttribute("application"); //$NON-NLS-1$
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.ui.launcher.AbstractLaunchShortcut#getName(org.eclipse.debug.core.ILaunchConfigurationType)
	 */
	...
	private void appendPlugin(StringBuffer buffer, IMonitorModelBase model) {
		if (buffer.length() > 0)
			buffer.append(',');
