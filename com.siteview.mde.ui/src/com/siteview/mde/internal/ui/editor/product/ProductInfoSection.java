...
import com.siteview.mde.internal.core.IStateDeltaListener;
...
import com.siteview.mde.internal.ui.wizards.product.ProductDefinitionWizard;
...
public class ProductInfoSection extends MDESection implements IRegistryChangeListener, IStateDeltaListener {
	...
	public ProductInfoSection(MDEFormPage page, Composite parent) {
	...
	protected void createClient(Section section, FormToolkit toolkit) {
		section.setText(MDEUIMessages.ProductInfoSection_title);
		section.setDescription(MDEUIMessages.ProductInfoSection_desc);
		...
		MDECore.getDefault().getExtensionsRegistry().addListener(this);
	...
	public void dispose() {
		...
		MDECore.getDefault().getExtensionsRegistry().removeListener(this);
		MDECore.getDefault().getModelManager().removeStateDeltaListener(this);
		super.dispose();
	}

	private void createProductEntry(Composite client, FormToolkit toolkit, IActionBars actionBars) {
		Label label = toolkit.createLabel(client, MDEUIMessages.ProductInfoSection_product);
		label.setToolTipText(MDEUIMessages.ProductInfoSection_productTooltip);
		...
		Button button = toolkit.createButton(client, MDEUIMessages.ProductInfoSection_new, SWT.PUSH);
	...
	private void createApplicationEntry(Composite client, FormToolkit toolkit, IActionBars actionBars) {
		Label label = toolkit.createLabel(client, MDEUIMessages.ProductInfoSection_app, SWT.WRAP);
		label.setToolTipText(MDEUIMessages.ProductInfoSection_appTooltip);
		label.setForeground(toolkit.getColors().getColor(IFormColors.TITLE));
	...
	private void createConfigurationOption(Composite client, FormToolkit toolkit) {
		...
		text.setText(MDEUIMessages.Product_overview_configuration, true, true);
		...
		fPluginButton = toolkit.createButton(comp, MDEUIMessages.ProductInfoSection_plugins, SWT.RADIO);
		gd = new GridData();
		...
		fFeatureButton = toolkit.createButton(comp, MDEUIMessages.ProductInfoSection_features, SWT.RADIO);
		gd = new GridData();
		gd.horizontalIndent = 25;
