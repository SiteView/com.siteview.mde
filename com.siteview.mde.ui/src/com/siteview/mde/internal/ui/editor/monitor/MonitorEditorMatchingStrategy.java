	...
	public boolean matches(IEditorReference editorRef, IEditorInput input) {
		...
		if (input instanceof IFileEditorInput && inputFile != null) {
			try {
				...
				// If a *.mon or *.java file is being opened, make sure we return a positive match
				// only if it is colocated with the plugin.xml/fragment.xml file already open
				if (inputFile.getName().endsWith(".mon") || inputFile.getName().endsWith(".MON") || inputFile.getName().endsWith(".java")) {//$NON-NLS-1$ //$NON-NLS-2$
					return true;
				}
