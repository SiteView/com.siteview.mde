...
import com.siteview.mde.internal.ui.editor.monitor.MissingResourcePage;
import com.siteview.mde.internal.ui.editor.monitor.OverviewPage;
...
import com.siteview.mde.internal.ui.editor.context.*;
 ...
 */
public abstract class MDEFormEditor extends FormEditor implements IInputContextListener, IGotoMarker, ISearchEditorAccess {
	/**
	...
	private MDEMultiPageContentOutline fContentOutline;
	...
	private static class PDEMultiPageEditorSite extends MultiPageEditorSite {
		...
		public IEditorActionBarContributor getActionBarContributor() {
			MDEFormEditor editor = (MDEFormEditor) getMultiPageEditor();
			MDEFormEditorContributor contributor = editor.getContributor();
	...
	public void setFocus() {
		...
		if ((page != null) && (page instanceof MDEFormPage)) {
			((MDEFormPage) page).updateFormSelection();
		}
	}
	...
	protected void contextMenuAboutToShow(IMenuManager manager) {
		MDEFormEditorContributor contributor = getContributor();
		IFormPage page = getActivePageInstance();
		if (page instanceof MDEFormPage)
			((MDEFormPage) page).contextMenuAboutToShow(manager);
		if (contributor != null)
			contributor.contextMenuAboutToShow(manager);
	}

	public MDEFormEditorContributor getContributor() {
		return (MDEFormEditorContributor) getEditorSite().getActionBarContributor();
	}
	...
	public void doRevert() {
		...
		// values to the model
		if ((formPage != null) && (formPage instanceof MDEFormPage)) {
	...
	private boolean doRevertSourcePages() {
		...
		for (int i = 0; i < pages.length; i++) {
			if (pages[i] instanceof MDESourcePage) {
				MDESourcePage sourcePage = (MDESourcePage) pages[i];
	...
	public void doRevert(IEditorInput input) {
		...
		if (currentPage != null && currentPage instanceof MDEFormPage)
			((MDEFormPage) currentPage).cancelEdit();
		InputContext context = fInputContextManager.getContext(input);
		IFormPage page = findPage(context.getId());
		if (page != null && page instanceof MDESourcePage) {
			MDESourcePage spage = (MDESourcePage) page;
	...
	public void doSaveAs() {
		try {
			...
			String title = MDEUIMessages.PDEFormEditor_errorTitleProblemSaveAs;
			String message = MDEUIMessages.PDEFormEditor_errorMessageSaveNotCompleted;
	...
	protected void setPropertyEditorPageKey(IFileEditorInput input, String pageId) {
		...
		try {
			...
			file.setPersistentProperty(IMDEUIConstants.PROPERTY_EDITOR_PAGE_KEY, pageId);
		} catch (CoreException e) {
			// Ignore
		}
	}
	...
	protected String getPropertyEditorPageKey(IFileEditorInput input) {
		...
		try {
			return file.getPersistentProperty(IMDEUIConstants.PROPERTY_EDITOR_PAGE_KEY);
		} catch (CoreException e) {
			return null;
		}
	}
	...
	private IDialogSettings getSettingsSection() {
		...
		IDialogSettings root = MDEPlugin.getDefault().getDialogSettings();
	...
	public void openToSourcePage(Object object, int offset, int length) {
		...
		if (context != null) {
			MDESourcePage page = (MDESourcePage) setActivePage(context.getId());
			if (page != null)
	...
	public MDEMultiPageContentOutline getContentOutline() {
		if (fContentOutline == null || fContentOutline.isDisposed()) {
			fContentOutline = new MDEMultiPageContentOutline(this);
	...
	private void updateContentOutline(IFormPage page) {
		...
		if (page instanceof MDESourcePage) {
			outline = ((MDESourcePage) page).getContentOutline();
		} else {
	...
	protected void performGlobalAction(String id) {
		...
		boolean handled = ((MDEFormPage) getActivePageInstance()).performGlobalAction(id);
		if (!handled) {
			IFormPage page = getActivePageInstance();
			if (page instanceof MDEFormPage) {
	...
	public boolean canPasteFromClipboard() {
		IFormPage page = getActivePageInstance();
		if (page instanceof MDEFormPage) {
			return ((MDEFormPage) page).canPaste(getClipboard());
		}
		return false;
	}
	...
	public void synchronizeOutlinePage() {
		...
		if (page instanceof MDESourcePage) {
			...
			((MDESourcePage) page).synchronizeOutlinePage();
		} else {
