...
public class LauncherSection extends MDESection {

	private TextValidator[] fMultipleWinIconValidator;

	private TextValidator fSingleWinIconValidator;

	private static final String[] F_WIN_ICON_LABELS = new String[] {MDEUIMessages.LauncherSection_Low16, MDEUIMessages.LauncherSection_High16, MDEUIMessages.LauncherSection_32Low, MDEUIMessages.LauncherSection_32High, MDEUIMessages.LauncherSection_48Low, MDEUIMessages.LauncherSection_48High};
	...
	class IconEntry extends FormEntry {
		...
		public IconEntry(Composite parent, FormToolkit toolkit, String labelText, String iconId) {
			super(parent, toolkit, labelText, MDEUIMessages.LauncherSection_browse, isEditable(), 20);
			fIconId = iconId;
	...
	public LauncherSection(MDEFormPage page, Composite parent) {
	...
	public void createClient(Section section, FormToolkit toolkit) {
		...
		section.setText(MDEUIMessages.LauncherSection_title);
		section.setDescription(MDEUIMessages.LauncherSection_desc);
		...
		IActionBars actionBars = getPage().getMDEEditor().getEditorSite().getActionBars();
		fNameEntry = new FormEntry(container, toolkit, MDEUIMessages.LauncherSection_launcherName, null, false);
		...
		createLabel(container, toolkit, MDEUIMessages.LauncherSection_label, 2);
	...
	private void addTab(String label) {
		...
		item.setText(label);
		item.setImage(MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_OPERATING_SYSTEM_OBJ));
	}

	private Composite addWin32Section(Composite parent, FormToolkit toolkit) {
		Composite comp = createComposite(parent, toolkit);

		fBmpButton = toolkit.createButton(comp, MDEUIMessages.LauncherSection_bmpImages, SWT.RADIO);
		...
		fIcoButton = toolkit.createButton(comp, MDEUIMessages.LauncherSection_ico, SWT.RADIO);
		...
		final IconEntry ientry = new IconEntry(comp, toolkit, MDEUIMessages.LauncherSection_file, ILauncherInfo.P_ICO_PATH);
		// Create validator
	...
	private Composite addLinuxSection(Composite parent, FormToolkit toolkit) {
		Composite comp = createComposite(parent, toolkit);
		createLabel(comp, toolkit, MDEUIMessages.LauncherSection_linuxLabel, 3);
		fIcons.add(new IconEntry(comp, toolkit, MDEUIMessages.LauncherSection_icon, ILauncherInfo.LINUX_ICON));
		toolkit.paintBordersFor(comp);
		return comp;
	}
	...
	private Composite addSolarisSection(Composite parent, FormToolkit toolkit) {
		Composite comp = createComposite(parent, toolkit);
		createLabel(comp, toolkit, MDEUIMessages.LauncherSection_solarisLabel, 3);

		fIcons.add(new IconEntry(comp, toolkit, MDEUIMessages.LauncherSection_large, ILauncherInfo.SOLARIS_LARGE));
		fIcons.add(new IconEntry(comp, toolkit, MDEUIMessages.LauncherSection_medium, ILauncherInfo.SOLARIS_MEDIUM));
		fIcons.add(new IconEntry(comp, toolkit, MDEUIMessages.LauncherSection_small, ILauncherInfo.SOLARIS_SMALL));
		fIcons.add(new IconEntry(comp, toolkit, MDEUIMessages.LauncherSection_tiny, ILauncherInfo.SOLARIS_TINY));
	...
	private Composite addMacSection(Composite parent, FormToolkit toolkit) {
		Composite comp = createComposite(parent, toolkit);
		createLabel(comp, toolkit, MDEUIMessages.LauncherSection_macLabel, 3);
		fIcons.add(new IconEntry(comp, toolkit, MDEUIMessages.LauncherSection_file, ILauncherInfo.MACOSX_ICON));
		toolkit.paintBordersFor(comp);
		return comp;
	}
	...
	private void handleBrowse(IconEntry entry) {
		...
		dialog.setTitle(MDEUIMessages.LauncherSection_dialogTitle);
		String extension = getExtension(entry.getIconId());
		dialog.setMessage(MDEUIMessages.LauncherSection_dialogMessage);
	...
	private void openImage(String value) {
		...
		if (path.isEmpty()) {
			MessageDialog.openWarning(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.WindowImagesSection_open, MDEUIMessages.WindowImagesSection_emptyPath); // 
			return;
		}
		...
		try {
			if (resource != null && resource instanceof IFile)
				...
				MessageDialog.openWarning(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.WindowImagesSection_open, MDEUIMessages.WindowImagesSection_warning); // 
