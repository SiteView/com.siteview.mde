...
import com.siteview.mde.internal.ui.editor.contentassist.TypeFieldAssistDisposer;
...
public class SchemaJavaAttributeDetails extends SchemaAttributeDetails {
	...
	protected void createTypeDetails(Composite parent, FormToolkit toolkit) {
		fClassEntry = new FormEntry(parent, toolkit, MDEUIMessages.SchemaAttributeDetails_extends, MDEUIMessages.SchemaAttributeDetails_browseButton, isEditable(), 13);
		fInterfaceEntry = new FormEntry(parent, toolkit, MDEUIMessages.SchemaAttributeDetails_implements, MDEUIMessages.SchemaAttributeDetails_browseButton, isEditable(), 13);
		fClassEntryFieldAssistDisposer = PDEJavaHelperUI.addTypeFieldAssistToText(fClassEntry.getText(), getPage().getMDEEditor().getCommonProject(), IJavaSearchConstants.CLASS);
		fInterfaceEntryFieldAssistDisposer = PDEJavaHelperUI.addTypeFieldAssistToText(fInterfaceEntry.getText(), getPage().getMDEEditor().getCommonProject(), IJavaSearchConstants.INTERFACE);
	}
	...
	private void doOpenSelectionDialog(int scopeType, FormEntry entry) {
		try {
			...
			SelectionDialog dialog = JavaUI.createTypeDialog(MDEPlugin.getActiveWorkbenchShell(), PlatformUI.getWorkbench().getProgressService(), SearchEngine.createWorkspaceScope(), scopeType, false, filter); 
