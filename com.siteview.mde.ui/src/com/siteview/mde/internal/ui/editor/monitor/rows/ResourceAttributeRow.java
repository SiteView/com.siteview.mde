...
public class ResourceAttributeRow extends ButtonAttributeRow {
	...
	private boolean openFile(IFile file) {
		if (file != null && file.exists()) {
			try {
				IDE.openEditor(MDEPlugin.getActivePage(), file, true);
	...
	private boolean openContainer(IContainer container) {
		if (container != null && container.exists())
			try {
				IViewPart part = MDEPlugin.getActivePage().showView(IPageLayout.ID_RES_NAV);
	...
	protected void browse() {
		final IProject project = part.getPage().getMDEEditor().getCommonProject();
		...
		dialog.setTitle(MDEUIMessages.ResourceAttributeCellEditor_title);
		dialog.setMessage(MDEUIMessages.ResourceAttributeCellEditor_message);
