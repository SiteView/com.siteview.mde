...
import com.siteview.mde.internal.ui.wizards.plugin.NewFragmentProjectWizard;
...
public class PluginSection extends TableSection implements IMonitorModelListener {
	...
	public static final QualifiedName OPTIONAL_PROPERTY = new QualifiedName(IMDEUIConstants.PLUGIN_ID, "product.includeOptional"); //$NON-NLS-1$

	class NewPluginAction extends Action {

		public NewPluginAction() {
			super(MDEUIMessages.Product_PluginSection_newPlugin, IAction.AS_PUSH_BUTTON);
			setImageDescriptor(MDEPluginImages.DESC_NEWPPRJ_TOOL);
		}
	...
	class NewFragmentAction extends Action {
		...
		public NewFragmentAction() {
			super(MDEUIMessages.Product_PluginSection_newFragment, IAction.AS_PUSH_BUTTON);
			setImageDescriptor(MDEPluginImages.DESC_NEWFRAGPRJ_TOOL);
		}
	...
	public PluginSection(MDEFormPage formPage, Composite parent) {
		super(formPage, parent, Section.DESCRIPTION, getButtonLabels());
	}

	private static String[] getButtonLabels() {
		String[] labels = new String[6];
		labels[0] = MDEUIMessages.Product_PluginSection_add;
		labels[1] = MDEUIMessages.Product_PluginSection_working;
		labels[2] = MDEUIMessages.Product_PluginSection_required;
		labels[3] = MDEUIMessages.PluginSection_remove;
		labels[4] = MDEUIMessages.Product_PluginSection_removeAll;
		labels[5] = MDEUIMessages.Product_FeatureSection_properties;
		return labels;
	}
	...
	private void createOptionalDependenciesButton(Composite container) {
		if (isEditable()) {
			...
			fIncludeOptionalButton.setText(MDEUIMessages.PluginSection_includeOptional);
			// initialize value
	...
	private void handleNewFragment() {
		...
		wizard.init(MDEPlugin.getActiveWorkbenchWindow().getWorkbench(), null);
	...
	private void handleProperties() {
		...
		if (ssel.size() == 1) {
			...
			VersionDialog dialog = new VersionDialog(MDEPlugin.getActiveWorkbenchShell(), isEditable(), plugin.getVersion());
			dialog.create();
	...
	private void handleAddWorkingSet() {
		...
		IWorkingSetSelectionDialog dialog = manager.createWorkingSetSelectionDialog(MDEPlugin.getActiveWorkbenchShell(), true);
		if (dialog.open() == Window.OK) {
			...
			for (int i = 0; i < workingSets.length; i++) {
				...
				for (int j = 0; j < elements.length; j++) {
					IMonitorModelBase model = findModel(elements[j]);
					if (model != null) {
						IProductPlugin plugin = factory.createPlugin();
	...
	/* (non-Javadoc)
	 * @see com.siteview.mde.internal.core.IPluginModelListener#modelsChanged(com.siteview.mde.internal.core.PluginModelDelta)
	 */
	...
	private IMonitorModelBase findModel(IAdaptable object) {
		...
		if (object instanceof PersistablePluginObject) {
			return MonitorRegistry.findModel(((PersistablePluginObject) object).getPluginID());
		}
		return null;
	}
