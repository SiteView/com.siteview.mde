...
import com.siteview.mde.internal.core.util.HeaderMap;
...
import com.siteview.mde.internal.ui.util.ImageOverlayIcon;
...
public class ManifestContentAssistProcessor extends TypePackageCompletionProcessor implements ICompletionListener {
	...
	protected MDESourcePage fSourcePage;
	...
	public ManifestContentAssistProcessor(MDESourcePage sourcePage) {
	...
	protected ICompletionProposal[] handleFragmentHostCompletion(String currentValue, int offset) {
		...
		if (index == -1) {
			...
			IMonitorModelBase base = MonitorRegistry.findModel(((ManifestEditor) fSourcePage.getEditor()).getCommonProject());
	...
	private ICompletionProposal[] getBundleVersionCompletions(String pluginID, String existingValue, int offset) {
		...
		if (entry != null) {
			IMonitorModelBase[] hosts = entry.getActiveModels();
	...
	private String getVersionProposal(IMonitorModelBase base) {
		...
		if (desc != null) {
			...
			char[] chars = base.getMonitorBase().getVersion().toCharArray();
			int periodCount = 0;
	...
	private void addPackageCompletions(String value, Set currentPackages, int offset, ArrayList proposals) {
		int length = value.length();
		IProject proj = ((MDEFormEditor) fSourcePage.getEditor()).getCommonProject();
		if (proj != null) {
	...
	public Image getImage(int type) {
		if (type >= 0 && type < F_TOTAL_TYPES)
			if (fImages[type] == null) {
				switch (type) {
					case F_TYPE_HEADER :
						return fImages[type] = MDEPluginImages.DESC_BUILD_VAR_OBJ.createImage();
					case F_TYPE_PKG :
						return MDEPluginImages.get(MDEPluginImages.OBJ_DESC_PACKAGE);
					case F_TYPE_BUNDLE :
						return fImages[type] = MDEPluginImages.DESC_PLUGIN_OBJ.createImage();
					case F_TYPE_CLASS :
						return MDEPluginImages.get(MDEPluginImages.OBJ_DESC_GENERATE_CLASS);
					case F_TYPE_ATTRIBUTE :
						return fImages[type] = MDEPluginImages.DESC_ATT_URI_OBJ.createImage();
					case F_TYPE_DIRECTIVE :
						fImages[F_TYPE_ATTRIBUTE] = MDEPluginImages.DESC_ATT_URI_OBJ.createImage();
						ImageOverlayIcon icon = new ImageOverlayIcon(fImages[F_TYPE_ATTRIBUTE], new ImageDescriptor[][] {new ImageDescriptor[] {MDEPluginImages.DESC_DOC_CO}, null, null, null});
						return fImages[type] = icon.createImage();
					case F_TYPE_EXEC_ENV :
						return fImages[type] = MDEPluginImages.DESC_JAVA_LIB_OBJ.createImage();
					case F_TYPE_VALUE :
						return null;
				}
			} else
