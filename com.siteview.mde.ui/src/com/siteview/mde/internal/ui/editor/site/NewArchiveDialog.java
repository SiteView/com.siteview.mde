...
public class NewArchiveDialog extends StatusDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		setTitle(MDEUIMessages.SiteEditor_NewArchiveDialog_title);
	...
	private void createEntries(Composite container) {
		...
		label.setText(MDEUIMessages.SiteEditor_NewArchiveDialog_path);
		fPathText = new Text(container, SWT.SINGLE | SWT.BORDER);
		fPathText.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		label = new Label(container, SWT.NULL);
		label.setText(MDEUIMessages.SiteEditor_NewArchiveDialog_url);
	...
	private IStatus createErrorStatus(String message) {
		return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.OK, message, null);
	}
	...
	private void dialogChanged() {
		...
		if (fUrlText.getText().length() == 0 || fPathText.getText().length() == 0)
			...
			if (hasPath(fPathText.getText()))
				status = createErrorStatus(MDEUIMessages.NewArchiveDialog_alreadyExists);
		}
		if (status == null)
			status = getOKStatus();
		updateStatus(status);
	...
	private IStatus getEmptyErrorStatus() {
		if (fErrorStatus == null)
			fErrorStatus = createErrorStatus(MDEUIMessages.SiteEditor_NewArchiveDialog_error);
		return fErrorStatus;
	}

	private IStatus getOKStatus() {
		if (fOkStatus == null)
			fOkStatus = new Status(IStatus.OK, MDEPlugin.getPluginId(), IStatus.OK, "", //$NON-NLS-1$
					null);
		return fOkStatus;
	}
