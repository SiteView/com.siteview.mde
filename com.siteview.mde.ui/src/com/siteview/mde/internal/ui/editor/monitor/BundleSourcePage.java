...
public class BundleSourcePage extends KeyValueSourcePage {
	...
	private class BundleOutlineContentProvider extends DefaultContentProvider implements ITreeContentProvider {
		...
		private Object[] getPluginLibraries() {
			IMonitorLibrary[] libraries = getBundleClasspathLibraries();
	...
	private IMonitorLibrary[] getBundleClasspathLibraries() {
		// The bundle classpath header has no model data members
		// Retrieve the plug-in library equivalents from the editor model
		FormEditor editor = getEditor();
		if (editor instanceof MDEFormEditor) {
			MDEFormEditor formEditor = (MDEFormEditor) editor;
			IBaseModel baseModel = formEditor.getAggregateModel();
			if (baseModel instanceof IMonitorModelBase) {
				IMonitorLibrary[] libraries = ((IMonitorModelBase) baseModel).getMonitorBase().getLibraries();
				return libraries;
			}
		}
		return null;
	...
	private class BundleLabelProvider extends LabelProvider {
		...
		public Image getImage(Object obj) {
			MDELabelProvider labelProvider = MDEPlugin.getDefault().getLabelProvider();
			if (obj instanceof PackageObject) {
				return labelProvider.get(MDEPluginImages.DESC_PACKAGE_OBJ);
			} else if (obj instanceof ExecutionEnvironment) {
				return labelProvider.get(MDEPluginImages.DESC_JAVA_LIB_OBJ);
			} else if (obj instanceof RequireBundleObject) {
				int flags = SharedLabelProvider.F_EXTERNAL;
				if (((RequireBundleObject) obj).isReexported()) {
					flags = flags | SharedLabelProvider.F_EXPORT;
				}
				return labelProvider.get(MDEPluginImages.DESC_REQ_PLUGIN_OBJ, flags);
			} else if (obj instanceof ManifestHeader) {
				return labelProvider.get(MDEPluginImages.DESC_BUILD_VAR_OBJ);
			} else if (obj instanceof IMonitorLibrary) {
				return labelProvider.get(MDEPluginImages.DESC_JAVA_LIB_OBJ);
			}
			return null;
		}
	 ...
	 * @param title
	 */
	public BundleSourcePage(MDEFormEditor editor, String id, String title) {
	...
	public IDocumentRange findRange() {
		...
		if (selection instanceof ImportObject) {
			IMonitorModelBase base = ((ImportObject) selection).getImport().getMonitorModel();
		...
		} else if (selection instanceof IMonitorLibrary) {
			IMonitorModelBase base = ((IMonitorLibrary) selection).getMonitorModel();
			if (base instanceof IBundlePluginModelBase)
				return getSpecificRange(((IBundlePluginModelBase) base).getBundleModel(), Constants.BUNDLE_CLASSPATH, ((IMonitorLibrary) selection).getName());
	...
	protected void editorContextMenuAboutToShow(IMenuManager menu) {
		...
		if (fRenameAction == null) {
			IBaseModel base = ((MDEFormEditor) getEditor()).getAggregateModel();
			if (base instanceof IMonitorModelBase) {
				fRenameAction = RefactoringActionFactory.createRefactorPluginIdAction(NLS.bind(MDEUIMessages.BundleSourcePage_renameActionText, Constants.BUNDLE_SYMBOLICNAME));
