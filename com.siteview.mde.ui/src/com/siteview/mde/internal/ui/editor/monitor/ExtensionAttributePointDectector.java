...
public class ExtensionAttributePointDectector implements MouseListener, KeyListener {
	...
	private void checkIfOnTarget() {
		...
		IMonitorObject node = XMLUtil.getTopLevelParent(documentAttribute);
		...
		if ((node instanceof IMonitorExtension) == false) {
			return;
		}
		...
		if (((schemaAttribute.getParent() instanceof SchemaRootElement) == false) || (documentAttribute.getAttributeName().equals(IMonitorExtension.P_POINT) == false)) {
			return;
		}
