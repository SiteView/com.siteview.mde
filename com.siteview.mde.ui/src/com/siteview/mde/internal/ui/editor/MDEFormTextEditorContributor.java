...
public class MDEFormTextEditorContributor extends MDEFormEditorContributor {
	...
	public MDEFormTextEditorContributor(String menuName) {
		super(menuName);
		fSourceContributor = createSourceContributor();
		if (supportsCorrectionAssist()) {
			fCorrectionAssist = new RetargetTextEditorAction(MDESourcePage.getBundleForConstructedKeys(), "CorrectionAssistProposal."); //$NON-NLS-1$
		...
		if (supportsFormatAction()) {
			...
			fFormatAction.setActionDefinitionId(MDEActionConstants.DEFN_FORMAT);
		}
		if (supportsContentAssist()) {
			fContentAssist = new RetargetTextEditorAction(MDESourcePage.getBundleForConstructedKeys(), "ContentAssistProposal."); //$NON-NLS-1$
	...
	protected void setSourceActionBarsActive(boolean active) {
		...
		rootBars.setGlobalActionHandler(MDEActionConstants.OPEN, active ? fHyperlinkAction : null);
		rootBars.setGlobalActionHandler(MDEActionConstants.FORMAT, active ? fFormatAction : null);
	...
	public void setActivePage(IEditorPart newEditor) {
		...
		boolean isSourcePage = fPage instanceof MDESourcePage;
	...
	private void updateQuickOutlineMenuEntry() {
		...
		// page
		navigateMenu.remove(MDEActionConstants.COMMAND_ID_QUICK_OUTLINE);
		// Ensure the active page is a source page
		// Only add the quick outline menu to the source pages
		if ((fPage instanceof MDEProjectionSourcePage) == false) {
			return;
		}
		MDEProjectionSourcePage page = (MDEProjectionSourcePage) fPage;
		...
		// source page
		IAction quickOutlineAction = page.getAction(MDEActionConstants.COMMAND_ID_QUICK_OUTLINE);
