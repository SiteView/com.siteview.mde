...
import com.siteview.mde.internal.core.text.IReconcilingParticipant;
...
import com.siteview.mde.internal.ui.editor.outline.QuickOutlinePopupDialog;
...
public abstract class ChangeAwareSourceViewerConfiguration extends TextSourceViewerConfiguration {
	 ...
	 * @param store
	 */
	public ChangeAwareSourceViewerConfiguration(MDESourcePage page, IColorManager manager, IPreferenceStore store) {
		super(store);
		fColorManager = manager;
		fSourcePage = page;
	}

	public ChangeAwareSourceViewerConfiguration(MDESourcePage page, IColorManager manager) {
		this(page, manager, new ChainedPreferenceStore(new IPreferenceStore[] {MDEPlugin.getDefault().getPreferenceStore(), EditorsUI.getPreferenceStore() // general text editor store
				}));
	}
	...
	public IInformationPresenter getOutlinePresenter(ISourceViewer sourceViewer) {
		...
		fOutlinePresenter = new InformationPresenter(getOutlinePresenterControlCreator(sourceViewer, MDEActionConstants.COMMAND_ID_QUICK_OUTLINE));
	...
	public IQuickAssistAssistant getQuickAssistAssistant(ISourceViewer sourceViewer) {
		if (sourceViewer.isEditable()) {
			if (fQuickAssistant == null) {
				...
				fQuickAssistant.setRestoreCompletionProposalSize(MDEPlugin.getDefault().getDialogSettingsSection("quick_assist_proposal_size")); //$NON-NLS-1$
			}
