...
package com.siteview.mde.internal.ui.editor.validation;
...
public class ControlValidationUtility {
	...
	public static boolean validateRequiredField(String value, IValidatorMessageHandler validator, int messageType) {
		...
		if (value.length() == 0) {
			validator.addMessage(MDEUIMessages.ControlValidationUtility_errorMsgValueMustBeSpecified, messageType);
			return false;
		}
		return true;
	}

	public static boolean validateTranslatableField(String value, IValidatorMessageHandler validator, IMonitorModelBase model, IProject project) {
		...
		if (value.startsWith("%") == false) { //$NON-NLS-1$
			validator.addMessage(MDEUIMessages.ControlValidationUtility_errorMsgValueNotExternalized, messageType);
			return false;
		}
		...
		if (model instanceof AbstractNLModel) {
			...
			if ((helper == null) || (helper.resourceExists(value) == false)) {
				validator.addMessage(MDEUIMessages.ControlValidationUtility_errorMsgKeyNotFound, messageType);
				return false;
			}
		}
		return true;
	...
	public static boolean validatePlatformFilterField(String value, IValidatorMessageHandler validator) {
		...
		try {
			...
			validator.addMessage(MDEUIMessages.ControlValidationUtility_errorMsgFilterInvalidSyntax, IMessageProvider.ERROR);
			return false;
		}
		...
		try {
			if (project.hasNature(JavaCore.NATURE_ID)) {
				...
				if (!PDEJavaHelper.isOnClasspath(value, javaProject)) {
					validator.addMessage(MDEUIMessages.ControlValidationUtility_errorMsgNotOnClasspath, messageType);
					return false;
				}
			}
	...
	public static boolean validateFragmentHostPluginField(String value, IValidatorMessageHandler validator, IProject project) {
		...
		// fragment itself
		IMonitorModelBase hostModel = MonitorRegistry.findModel(value);
		if ((hostModel == null) || (hostModel instanceof IFragmentModel) || (hostModel.isEnabled() == false)) {
			validator.addMessage(MDEUIMessages.ControlValidationUtility_errorMsgPluginUnresolved, unresImpMessageType);
			return false;
		}
