...
public class AddLibraryDialog extends SelectionStatusDialog {
	...
	class DuplicateStatusValidator {
		public IStatus validate(String text) {
			if (text.length() == 0)
				return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.ERROR, MDEUIMessages.AddLibraryDialog_emptyLibraries, null);

			if (text.indexOf(' ') != -1)
				return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.ERROR, MDEUIMessages.AddLibraryDialog_nospaces, null);
			...
			for (int i = 0; i < libraries.length; i++) {
				if (libraries[i].equals(text))
					return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.ERROR, MDEUIMessages.BuildEditor_RuntimeInfoSection_duplicateLibrary, null);
			}
			return new Status(IStatus.OK, MDEPlugin.getPluginId(), IStatus.OK, "", null); //$NON-NLS-1$
		}
	}
	...
	class TableContentProvider extends DefaultContentProvider implements IStructuredContentProvider {
		public Object[] getElements(Object input) {
			if (input instanceof IMonitorModelBase) {
				return ((IMonitorModelBase) input).getMonitorBase().getLibraries();
			}
			return new Object[0];
		}
	}
	...
	class TableLabelProvider extends LabelProvider implements ITableLabelProvider {
		public String getColumnText(Object obj, int index) {
			return ((IMonitorLibrary) obj).getName();
		}
	...
	public AddLibraryDialog(Shell shell, String[] libraries, IMonitorModelBase model) {
		super(shell);
	...
	public void setPluginModel(IMonitorModelBase model) {
		this.model = model;
	}

	private void initializeImages() {
		MDELabelProvider provider = MDEPlugin.getDefault().getLabelProvider();
		libImage = provider.get(MDEPluginImages.DESC_JAVA_LIB_OBJ);
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		label.setText(MDEUIMessages.BuildEditor_AddLibraryDialog_label);
		...
		libraryViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent e) {
				...
				IMonitorLibrary obj = (IMonitorLibrary) ((IStructuredSelection) sel).getFirstElement();
