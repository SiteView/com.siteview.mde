...
public abstract class ModelUndoManager implements IModelUndoManager, IModelChangedListener {
	...
	public ModelUndoManager(MDEFormEditor editor) {
		this.editor = editor;
	...
	private String getUndoText() {
		IModelChangedEvent op = getCurrentOperation();
		if (op == null) {
			return MDEUIMessages.UpdateManager_noUndo;
		}
		return NLS.bind(MDEUIMessages.UpdateManager_undo, getOperationText(op));
	}

	private String getRedoText() {
		IModelChangedEvent op = getNextOperation();
		if (op == null) {
			return MDEUIMessages.UpdateManager_noRedo;
		}
		return NLS.bind(MDEUIMessages.UpdateManager_redo, getOperationText(op));
	}
	...
	private String getOperationText(IModelChangedEvent op) {
		...
		switch (op.getChangeType()) {
			case IModelChangedEvent.INSERT :
				opText = MDEUIMessages.UpdateManager_op_add;
				break;
			case IModelChangedEvent.REMOVE :
				opText = MDEUIMessages.UpdateManager_op_remove;
				break;
			case IModelChangedEvent.CHANGE :
				opText = MDEUIMessages.UpdateManager_op_change;
				break;
		}
		return opText;
	}
