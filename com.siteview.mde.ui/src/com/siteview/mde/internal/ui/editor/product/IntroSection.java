...
import com.siteview.mde.internal.core.text.bundle.RequireBundleObject;
...
import com.siteview.mde.internal.ui.wizards.product.ProductIntroWizard;
...
public class IntroSection extends MDESection {
	...
	public IntroSection(MDEFormPage page, Composite parent) {
	...
	public void createClient(Section section, FormToolkit toolkit) {
		...
		section.setText(MDEUIMessages.IntroSection_sectionText);
		section.setDescription(MDEUIMessages.IntroSection_sectionDescription);
		...
		Label label = toolkit.createLabel(client, MDEUIMessages.IntroSection_introLabel, SWT.WRAP);
		GridData td = new GridData();
		td.horizontalSpan = canCreateNew ? 3 : 2;
		label.setLayoutData(td);

		Label introLabel = toolkit.createLabel(client, MDEUIMessages.IntroSection_introInput);
		...
		if (canCreateNew) {
			Button button = toolkit.createButton(client, MDEUIMessages.IntroSection_new, SWT.PUSH);
	...
	private void handleNewIntro() {
		...
		if (!productDefined()) {
			needNewProduct = true;
			MessageDialog mdiag = new MessageDialog(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.IntroSection_undefinedProductId, null, MDEUIMessages.IntroSection_undefinedProductIdMessage, MessageDialog.QUESTION, new String[] {IDialogConstants.YES_LABEL, IDialogConstants.NO_LABEL}, 0);
