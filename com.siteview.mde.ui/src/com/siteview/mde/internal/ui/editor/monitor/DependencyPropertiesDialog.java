...
public class DependencyPropertiesDialog extends StatusDialog {
	...
	public DependencyPropertiesDialog(boolean editable, IMonitorImport plugin) {
	...
	public DependencyPropertiesDialog(boolean editable, boolean showReexport, boolean export, boolean optional, String version, boolean showOptional, boolean isImport, String pluginId) {
		super(MDEPlugin.getActiveWorkbenchShell());
		fEditable = editable;
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (fShowOptional || fShowReexport) {
			...
			container.setText(MDEUIMessages.DependencyPropertiesDialog_properties);
			container.setLayout(new GridLayout());
			container.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

			if (fShowOptional) {
				fOptionalButton = new Button(container, SWT.CHECK);
				fOptionalButton.setText(MDEUIMessages.DependencyPropertiesDialog_optional);
			...
			if (fShowReexport) {
				...
				fReexportButton.setText(MDEUIMessages.DependencyPropertiesDialog_reexport);
				GridData gd = new GridData();
				gd.horizontalSpan = 2;
