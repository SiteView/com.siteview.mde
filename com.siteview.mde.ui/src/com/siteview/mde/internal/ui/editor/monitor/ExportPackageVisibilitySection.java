...
import com.siteview.mde.internal.core.text.bundle.ExportPackageObject;
import com.siteview.mde.internal.core.text.bundle.PackageFriend;
...
public class ExportPackageVisibilitySection extends TableSection implements IPartSelectionListener {
	...
	public ExportPackageVisibilitySection(MDEFormPage formPage, Composite parent) {
		super(formPage, parent, Section.DESCRIPTION, new String[] {MDEUIMessages.ManifestEditor_ExportSection_add, MDEUIMessages.ManifestEditor_ExportSection_remove});
		fHandleDefaultButton = false;
	}

	public void createClient(Section section, FormToolkit toolkit) {
		section.setText(MDEUIMessages.ExportPackageVisibilitySection_title);
		section.setDescription(MDEUIMessages.ExportPackageVisibilitySection_default);
		Composite comp = toolkit.createComposite(section);
		comp.setLayout(FormLayoutFactory.createSectionClientGridLayout(false, 1));

		fVisibleButton = toolkit.createButton(comp, MDEUIMessages.ExportPackageVisibilitySection_unconditional, SWT.RADIO);

		fInternalButton = toolkit.createButton(comp, MDEUIMessages.ExportPackageVisibilitySection_hideAll, SWT.RADIO);
		...
		makeActions();
		fImage = MDEPluginImages.DESC_PLUGIN_OBJ.createImage();
		update(null);
	...
	private void handleAdd() {
		PluginSelectionDialog dialog = new PluginSelectionDialog(MDEPlugin.getActiveWorkbenchShell(), getModels(), true);
		dialog.create();
		if (dialog.open() == Window.OK) {
			...
			for (int i = 0; i < selected.length; i++) {
				...
				for (int j = 0; j < fSelectedObjects.length; j++) {
					fSelectedObjects[j].addFriend(new PackageFriend(fSelectedObjects[j], model.getMonitorBase().getId()));
				}
			}
		}
	}

	private IMonitorModelBase[] getModels() {
		ArrayList list = new ArrayList();
		IMonitorModelBase[] models = MonitorRegistry.getActiveModels(true);
		for (int i = 0; i < models.length; i++) {
			String id = models[i].getMonitorBase().getId();
			if (!fSelectedObjects[0].hasFriend(id))
				list.add(models[i]);
		}
		return (IMonitorModelBase[]) list.toArray(new IMonitorModelBase[list.size()]);
	}
