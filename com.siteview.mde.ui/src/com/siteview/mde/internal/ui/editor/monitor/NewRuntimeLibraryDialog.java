...
public class NewRuntimeLibraryDialog extends SelectionStatusDialog {
	...
	private IMonitorLibrary[] libraries;
	...
	class DuplicateStatusValidator {
		public IStatus validate(String text) {
			...
			if (text.length() == 0)
				return new Status(IStatus.ERROR, id, IStatus.ERROR, MDEUIMessages.AddLibraryDialog_emptyLibraries, null);

			if (text.indexOf(' ') != -1)
				return new Status(IStatus.ERROR, id, IStatus.ERROR, MDEUIMessages.AddLibraryDialog_nospaces, null);
			...
			if (librarySet.contains(new Path(ClasspathUtilCore.expandLibraryName(text))))
				return new Status(IStatus.ERROR, id, IStatus.ERROR, MDEUIMessages.ManifestEditor_RuntimeLibraryDialog_validationError, null);
			return new Status(IStatus.OK, id, IStatus.OK, "", null); //$NON-NLS-1$
		}
	}

	public NewRuntimeLibraryDialog(Shell parent, IMonitorLibrary[] libraries) {
		super(parent);
	...
	protected Control createDialogArea(Composite parent) {
		...
		libraryLabel.setText(MDEUIMessages.ManifestEditor_RuntimeLibraryDialog_label);

		libraryText = new Text(container, SWT.SINGLE | SWT.BORDER);
		gd = new GridData(GridData.FILL_HORIZONTAL);
		libraryText.setLayoutData(gd);
		libraryText.setText(MDEUIMessages.ManifestEditor_RuntimeLibraryDialog_default);
