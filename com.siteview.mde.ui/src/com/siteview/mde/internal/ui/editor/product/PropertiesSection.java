...
public class PropertiesSection extends TableSection {
	...
	private class LabelProvider extends MDELabelProvider {
		public Image getColumnImage(Object obj, int index) {
			if (index == 0)
				return get(MDEPluginImages.DESC_PROPERTIES);
			return null;
		}
	...
	private class PropertyDialog extends StatusDialog {
		...
		public PropertyDialog(Shell shell, IConfigurationProperty property, Set existingNames) {
			...
			setTitle(MDEUIMessages.PropertiesSection_PropertyDialogTitle);
		}

		protected Control createDialogArea(Composite parent) {
			Composite comp = (Composite) super.createDialogArea(parent);
			((GridLayout) comp.getLayout()).numColumns = 2;
			SWTFactory.createLabel(comp, MDEUIMessages.PropertiesSection_Name, 1);
			...
			SWTFactory.createLabel(comp, MDEUIMessages.PropertiesSection_Value, 1);
			...
			updateStatus(new Status(IStatus.ERROR, IMDEUIConstants.PLUGIN_ID, "")); //$NON-NLS-1$
		...
		protected void validate() {
			...
			if (name.length() == 0) {
				updateStatus(new Status(IStatus.ERROR, IMDEUIConstants.PLUGIN_ID, MDEUIMessages.PropertiesSection_ErrorPropertyNoName));
			} else if (fExistingNames.contains(name)) {
				updateStatus(new Status(IStatus.ERROR, IMDEUIConstants.PLUGIN_ID, NLS.bind(MDEUIMessages.PropertiesSection_ErrorPropertyExists, name)));
			} else {
	...
	public PropertiesSection(MDEFormPage page, Composite parent) {
		super(page, parent, Section.DESCRIPTION, getButtonLabels());
	}

	private static String[] getButtonLabels() {
		String[] labels = new String[3];
		labels[0] = MDEUIMessages.PropertiesSection_Add;
		labels[1] = MDEUIMessages.PropertiesSection_Edit;
		labels[2] = MDEUIMessages.PropertiesSection_Remove;
		return labels;
	}
	...
	private void handleAdd() {
		PropertyDialog dialog = new PropertyDialog(MDEPlugin.getActiveWorkbenchShell(), null, getExistingNames());
	...
	private void handleEdit() {
		...
		if (ssel.size() > 0 && ssel.getFirstElement() instanceof IConfigurationProperty) {
			...
			PropertyDialog dialog = new PropertyDialog(MDEPlugin.getActiveWorkbenchShell(), propertyToEdit, existing);
			if (dialog.open() == Window.OK) {
				IConfigurationProperty result = dialog.getResult();
				if (result != null) {
					fPropertiesTable.refresh();
