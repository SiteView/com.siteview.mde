...
import com.siteview.mde.internal.core.schema.SchemaSimpleType;
...
public abstract class SchemaAttributeDetails extends AbstractSchemaDetails {
	...
	public void createDetails(Composite parent) {
		...
		fName = new FormEntry(parent, toolkit, MDEUIMessages.SchemaDetails_name, SWT.NONE);
		// Ensures label columns on every detail page are same width
		((GridData) fName.getLabel().getLayoutData()).widthHint = minLabelWeight;

		Label label = toolkit.createLabel(parent, MDEUIMessages.SchemaDetails_deprecated);
		label.setForeground(foreground);
		Button[] buttons = createTrueFalseButtons(parent, toolkit, 2);
		fDepTrue = buttons[0];
		fDepFalse = buttons[1];

		label = toolkit.createLabel(parent, MDEUIMessages.SchemaAttributeDetails_use);
		label.setForeground(foreground);
		...
		label = toolkit.createLabel(parent, MDEUIMessages.SchemaAttributeDetails_type);
		label.setForeground(foreground);
		...
		setText(MDEUIMessages.SchemaAttributeDetails_title);
	}
	...
	public void updateFields(ISchemaObject object) {
		...
		setDecription(NLS.bind(MDEUIMessages.SchemaAttributeDetails_description, fAttribute.getName()));
		...
		fValue.setValue(value != null ? value.toString() : MDEUIMessages.SchemaAttributeDetails_defaultDefaultValue, true);
	...
	private void doUseChange(int index) {
		...
		if (index == 2) {
			fUseLayout.topControl = fUseCompDefault;
			fUseComp.layout();
			if (fValue.getValue().equals(MDEUIMessages.SchemaAttributeDetails_defaultDefaultValue))
				fValue.getText().setSelection(0, fValue.getValue().length());
			fValue.getText().setFocus();
		} else if (index != 2) {
			fUseLayout.topControl = fUseCompOther;
			fUseComp.layout();
			fValue.setValue(MDEUIMessages.SchemaAttributeDetails_defaultDefaultValue);
