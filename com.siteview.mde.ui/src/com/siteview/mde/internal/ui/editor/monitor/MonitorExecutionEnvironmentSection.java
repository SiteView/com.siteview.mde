...
public class MonitorExecutionEnvironmentSection extends TableSection {
	...
	class EELabelProvider extends LabelProvider {
		...
		public EELabelProvider() {
			fImage = MDEPluginImages.DESC_JAVA_LIB_OBJ.createImage();
		}
	...
	public MonitorExecutionEnvironmentSection(MDEFormPage page, Composite parent) {
		super(page, parent, Section.DESCRIPTION, new String[] {MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_add, MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_remove, MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_up, MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_down});
		createClient(getSection(), page.getEditor().getToolkit());
	}

	protected void createClient(Section section, FormToolkit toolkit) {
		section.setText(MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_title);
		if (isFragment())
			section.setDescription(MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_fragmentDesc);
		else
			section.setDescription(MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_pluginDesc);
		...
		fEETable.setLabelProvider(MDEPlugin.getDefault().getLabelProvider());

		Hyperlink link = toolkit.createHyperlink(container, MDEUIMessages.BuildExecutionEnvironmentSection_configure, SWT.NONE);
		...
		link.addHyperlinkListener(new IHyperlinkListener() {
			...
			public void linkActivated(HyperlinkEvent e) {
				SWTFactory.showPreferencePage(MDEPlugin.getActiveWorkbenchShell(), "org.eclipse.jdt.debug.ui.jreProfiles", null); //$NON-NLS-1$
			}
		});
		...
		try {
			if (project != null && project.hasNature(JavaCore.NATURE_ID)) {
				link = toolkit.createHyperlink(container, MDEUIMessages.ExecutionEnvironmentSection_updateClasspath, SWT.NONE);
				link.setForeground(toolkit.getColors().getColor(IFormColors.TITLE));
				link.addHyperlinkListener(new IHyperlinkListener() {
					public void linkEntered(HyperlinkEvent e) {
					}
	...
	private void makeActions() {
		fAddAction = new Action(MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_add) {
			public void run() {
				handleAdd();
			}
		};
	...
	private void handleAdd() {
		ElementListSelectionDialog dialog = new ElementListSelectionDialog(MDEPlugin.getActiveWorkbenchShell(), new EELabelProvider());
		dialog.setElements(getEnvironments());
		dialog.setAllowDuplicates(false);
		dialog.setMultipleSelection(true);
		dialog.setTitle(MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_dialog_title);
		dialog.setMessage(MDEUIMessages.RequiredMonitorExecutionEnvironmentSection_dialogMessage);
		dialog.create();
	...
	protected boolean isFragment() {
		InputContextManager manager = getPage().getMDEEditor().getContextManager();
		IMonitorModelBase model = (IMonitorModelBase) manager.getAggregateModel();
