...
public class MDEMultiPageContentOutline extends Page implements IContentOutlinePage, ISelectionProvider, ISelectionChangedListener, ILaunchingPreferenceConstants {
	...
	private MDEFormEditor editor;

	public MDEMultiPageContentOutline(MDEFormEditor editor) {
		this.editor = editor;
		listeners = new ArrayList();
		sortingOn = MDEPlugin.getDefault().getPreferenceStore().getBoolean("PDEMultiPageContentOutline.SortingAction.isChecked"); //$NON-NLS-1$
	...
	class SortingAction extends Action {
		...
		public SortingAction() {
			...
			setText(MDEUIMessages.PDEMultiPageContentOutline_SortingAction_label);
			setImageDescriptor(MDEPluginImages.DESC_ALPHAB_SORT_CO);
			setDisabledImageDescriptor(MDEPluginImages.DESC_ALPHAB_SORT_CO_DISABLED);
			setToolTipText(MDEUIMessages.PDEMultiPageContentOutline_SortingAction_tooltip);
			setDescription(MDEUIMessages.PDEMultiPageContentOutline_SortingAction_description);
		...
		private void valueChanged(final boolean on) {
			...
			if (currentPage != null)
				currentPage.sort(on);
			MDEPlugin.getDefault().getPreferenceStore().setValue("PDEMultiPageContentOutline.SortingAction.isChecked", on); //$NON-NLS-1$
		}
