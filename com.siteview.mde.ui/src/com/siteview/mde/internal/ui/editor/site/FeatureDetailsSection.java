...
public class FeatureDetailsSection extends MDESection implements IFormPart, IPartSelectionListener {
	...
	public FeatureDetailsSection(MDEFormPage page, Composite parent) {
		this(page, parent, MDEUIMessages.FeatureDetailsSection_title, MDEUIMessages.FeatureDetailsSection_desc, SWT.NULL);
	}

	public FeatureDetailsSection(MDEFormPage page, Composite parent, String title, String desc, int toggleStyle) {
	...
	public void createClient(Section section, FormToolkit toolkit) {
		...
		fUrlText = new FormEntry(container, toolkit, MDEUIMessages.FeatureDetailsSection_url, null, false);
		fUrlText.setFormEntryListener(new FormEntryAdapter(this) {
			public void textValueChanged(FormEntry text) {
				try {
					if (text.getValue().length() <= 0) {
						setValue(PROPERTY_URL);
						MessageDialog.openError(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.FeatureDetailsSection_requiredURL_title, MDEUIMessages.FeatureDetailsSection_requiredURL);
					} else {
	...
	private void createPatchButton(FormToolkit toolkit, Composite container) {
		fPatchCheckBox = toolkit.createButton(container, MDEUIMessages.FeatureDetailsSection_patch, SWT.CHECK);
