...
public class BuildClasspathSection extends TableSection implements IModelChangedListener {
	...
	public BuildClasspathSection(MDEFormPage page, Composite parent) {
		super(page, parent, Section.DESCRIPTION | ExpandableComposite.TWISTIE, new String[] {MDEUIMessages.BuildEditor_ClasspathSection_add, MDEUIMessages.BuildEditor_ClasspathSection_remove, null, null});
		getSection().setText(MDEUIMessages.BuildEditor_ClasspathSection_title);
		getSection().setDescription(MDEUIMessages.BuildEditor_ClasspathSection_desc);
		initialize();
	...
	protected void fillContextMenu(IMenuManager manager) {
		...
		// add NEW action
		Action action = new Action(MDEUIMessages.BuildEditor_ClasspathSection_add) {
			public void run() {
				handleNew();
			}
		};
		...
		// add DELETE action
		action = new Action(MDEUIMessages.BuildEditor_ClasspathSection_remove) {
			public void run() {
				handleDelete();
			}
		};
	...
	private void initializeDialogSettings(ElementTreeSelectionDialog dialog) {
		...
		dialog.setMessage(MDEUIMessages.BuildEditor_ClasspathSection_jarsDesc);
	...
	private String getRelativePathTokenName(IResource elem) {
		...
		IMonitorModelBase model = MonitorRegistry.findModel(elemProject);
		if (model != null)
			return "platform:/plugin/" + model.getMonitorBase().getId() + '/' + projectRelative; //$NON-NLS-1$
		return null;
	}
