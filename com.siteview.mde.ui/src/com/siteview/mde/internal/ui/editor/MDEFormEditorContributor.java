...
public class MDEFormEditorContributor extends MultiPageEditorActionBarContributor {
	...
	class CutAction extends ClipboardAction {
		public CutAction() {
			...
			setText(MDEUIMessages.EditorActions_cut);
	...
	class CopyAction extends ClipboardAction {
		public CopyAction() {
			...
			setText(MDEUIMessages.EditorActions_copy);
	...
	class PasteAction extends ClipboardAction {
		public PasteAction() {
			...
			setText(MDEUIMessages.EditorActions_paste);
	...
	class SaveAction extends Action implements IUpdate {
		...
		public void run() {
			if (fEditor != null)
				MDEPlugin.getActivePage().saveEditor(fEditor, false);
		}
	...
	public MDEFormEditorContributor(String menuName) {
	}
	...
	public MDEFormEditor getEditor() {
		return fEditor;
	}
	...
	protected void makeActions() {
		...
		fSaveAction.setText(MDEUIMessages.EditorActions_save);
		fRevertAction = new RevertAction();
		fRevertAction.setText(MDEUIMessages.EditorActions_revert);
		addGlobalAction(ActionFactory.REVERT.getId(), fRevertAction);
	}

	public void setActiveEditor(IEditorPart targetEditor) {
		if (targetEditor instanceof MDESourcePage) {
			// Fixing the 'goto line' problem -
			// the action is thinking that source page
			// is a standalone editor and tries to activate it
			// #19361
			MDESourcePage page = (MDESourcePage) targetEditor;
			MDEPlugin.getActivePage().activate(page.getEditor());
			return;
		}
		if (!(targetEditor instanceof MDEFormEditor))
			return;

		fEditor = (MDEFormEditor) targetEditor;
