...
public class TranslationHyperlink extends AbstractHyperlink {
	...
	private String getLocalization() {
		String localiz = null;
		if (fBase instanceof MonitorModelBase) {
			if (((MonitorModelBase) fBase).getNLResourceHelper() != null)
				localiz = ((MonitorModelBase) fBase).getNLResourceHelper().getNLFileBasePath();
		} else if (fBase instanceof IMonitorModelBase)
			localiz = MDEManager.getBundleLocalization((IMonitorModelBase) fBase);
	...
	public boolean openHyperLink() {
		...
		try {
			IEditorPart editor = IDE.openEditor(MDEPlugin.getActivePage(), file);
