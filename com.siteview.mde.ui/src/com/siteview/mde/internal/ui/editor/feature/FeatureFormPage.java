 ...
 * Feature page.
 */
public class FeatureFormPage extends MDEFormPage implements IHyperlinkListener {
	...
	public FeatureFormPage(MDEFormEditor editor, String title) {
	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		form.setImage(MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_FTR_XML_OBJ));
	...
	private Section createContentSection(IManagedForm managedForm, Composite parent, FormToolkit toolkit) {
		...
		FormText text;
		section.setText(MDEUIMessages.FeatureEditor_InfoPage_ContentSection_title);
		text = createClient(section, MDEUIMessages.FeatureEditor_InfoPage_ContentSection_text, toolkit);
		MDELabelProvider lp = MDEPlugin.getDefault().getLabelProvider();
		text.setImage("page", lp.get(MDEPluginImages.DESC_PAGE_OBJ, //$NON-NLS-1$
	...
	private Section createPackagingSection(IManagedForm managedForm, Composite parent, FormToolkit toolkit) {
		Section section = createStaticSection(parent, toolkit);
		section.setText(MDEUIMessages.FeatureEditor_InfoPage_PackagingSection_title);
		...
		createClient(section, MDEUIMessages.FeatureEditor_InfoPage_PackagingSection_text, toolkit);
		return section;
	}

	private Section createPublishingSection(IManagedForm managedForm, Composite parent, FormToolkit toolkit) {
		Section section = createStaticSection(parent, toolkit);
		section.setText(MDEUIMessages.FeatureEditor_InfoPage_PublishingSection_title);
		// ImageHyperlink info = new ImageHyperlink(section, SWT.NULL);
		// toolkit.adapt(info, true, true);
		// Image image =
		// PDEPlugin.getDefault().getLabelProvider().get(PDEPluginImages.DESC_HELP);
		...
		// info.setBackground(section.getTitleBarGradientBackground());
		// section.setTextClient(info);
		createClient(section, MDEUIMessages.FeatureEditor_InfoPage_PublishingSection_text, toolkit);
		return section;
	}
	...
	public void linkActivated(HyperlinkEvent e) {
		...
		if (href.equals("plugins")) //$NON-NLS-1$
			...
			getEditor().setActivePage(FeatureReferencePage.PAGE_ID);
			final FeatureEditorContributor contributor = (FeatureEditorContributor) getMDEEditor().getContributor();
			...
			((FeatureEditor) getMDEEditor()).getFeatureExportAction().run();
