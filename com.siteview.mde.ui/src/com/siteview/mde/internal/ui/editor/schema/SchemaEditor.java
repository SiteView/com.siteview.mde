...
public class SchemaEditor extends MultiSourceEditor {
	...
	protected String getEditorID() {
		return IMDEUIConstants.SCHEMA_EDITOR_ID;
	}
	...
	public static boolean openSchema(IFile file) {
		if (file != null && file.exists()) {
			...
			try {
				return MDEPlugin.getActivePage().openEditor(input, IMDEUIConstants.SCHEMA_EDITOR_ID) != null;
			} catch (PartInitException e) {
			}
		}
	...
	private static boolean openEditor(IEditorInput input) {
		...
		try {
			...
			part = MDEPlugin.getActivePage().openEditor(input, IMDEUIConstants.SCHEMA_EDITOR_ID);
		} catch (PartInitException e) {
			Display.getDefault().beep();
			return false;
		}
	...
	public static void openToElement(IPath path, ISchemaElement element) {
		if (openSchema(path)) {
			IEditorPart editorPart = MDEPlugin.getActivePage().getActiveEditor();
