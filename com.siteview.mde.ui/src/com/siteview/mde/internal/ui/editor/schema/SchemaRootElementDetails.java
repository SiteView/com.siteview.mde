...
public class SchemaRootElementDetails extends AbstractSchemaDetails {
	...
	public void createDetails(Composite parent) {
		...
		fName = new FormEntry(parent, toolkit, MDEUIMessages.SchemaDetails_name, SWT.READ_ONLY);
		// Ensures label columns on every detail page are same width
		((GridData) fName.getLabel().getLayoutData()).widthHint = minLabelWeight;

		Label label = toolkit.createLabel(parent, MDEUIMessages.SchemaDetails_internal);
		...
		label = toolkit.createLabel(parent, MDEUIMessages.SchemaDetails_deprecated);
		label.setForeground(foreground);
		...
		fSuggestion = new FormEntry(parent, toolkit, MDEUIMessages.SchemaRootElementDetails_replacement, null, false, 6);

		setText(MDEUIMessages.SchemaElementDetails_rootTitle);
	}
	...
	public void updateFields(ISchemaObject element) {
		...
		setDecription(NLS.bind(MDEUIMessages.SchemaElementDetails_description, fElement.getName()));
