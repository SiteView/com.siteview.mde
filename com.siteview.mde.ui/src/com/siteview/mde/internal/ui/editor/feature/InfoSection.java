...
public class InfoSection extends MDESection {
	...
	public InfoSection(MDEFormPage page, Composite parent, IColorManager colorManager) {
		super(page, parent, Section.DESCRIPTION | ExpandableComposite.NO_TITLE, false);
		String description = MDEUIMessages.FeatureEditor_InfoSection_desc;
	...
	private void createTabs() {
		IFeatureModel model = (IFeatureModel) getPage().getModel();
		IFeature feature = model.getFeature();
		addTab(MDEUIMessages.FeatureEditor_info_description, feature.getFeatureInfo(0));
		addTab(MDEUIMessages.FeatureEditor_info_copyright, feature.getFeatureInfo(1));
		addTab(MDEUIMessages.FeatureEditor_info_license, feature.getFeatureInfo(2));
		addTab(MDEUIMessages.FeatureEditor_info_discoveryUrls, null);
	}
	...
	private void updateTabImage(CTabItem item) {
		...
		if (info != null) {
			item.setImage(MDEPlugin.getDefault().getLabelProvider().getImage(info));
		} else {
			item.setImage(MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_DOC_SECTION_OBJ));
		}
	}
