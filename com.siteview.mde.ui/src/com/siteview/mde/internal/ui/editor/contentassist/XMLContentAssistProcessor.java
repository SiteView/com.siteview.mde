...
public class XMLContentAssistProcessor extends TypePackageCompletionProcessor implements IContentAssistProcessor, ICompletionListener {
	...
	public XMLContentAssistProcessor(MDESourcePage sourcePage) {
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
		...
		if (caText != null) {
			...
			return null;
		} else if (model instanceof MonitorModelBase) {
			// broken model - infer from text content
			return computeBrokenModelProposal(((MonitorModelBase) model).getLastErrorNode(), offset, doc);
		}
		return null;
	}
...
//		String attribute = guess[1];
		...
		IMonitorObject obj = XMLUtil.getTopLevelParent(attr);
		if (obj instanceof IMonitorExtension) {
			if (attr.getAttributeName().equals(IMonitorExtension.P_POINT) && offset >= attr.getValueOffset()) {
				return computeExtPointAttrProposals(attr, offset, attrValue);
			}
			ISchemaAttribute sAttr = XMLUtil.getSchemaAttribute(attr, ((IMonitorExtension) obj).getPoint());
			if (sAttr == null)
				return null;
	...
	private ICompletionProposal[] computeRootNodeProposals(IDocumentElementNode node, int offset, String filter) {
		...
		addToList(filteredProposalList, filter, new VirtualSchemaObject(F_STR_EXT, MDEUIMessages.XMLContentAssistProcessor_extensions, F_EXTENSION));
		// Add extension point to the list
		addToList(filteredProposalList, filter, new VirtualSchemaObject(F_STR_EXT_PT, MDEUIMessages.XMLContentAssistProcessor_extensionPoints, F_EXTENSION_POINT));
	...
	private ICompletionProposal[] computeAddChildProposal(IDocumentElementNode node, int offset, IDocument doc, String filter) {
		...
		if (node instanceof IMonitorBase) {
			...
			if (obj instanceof IMonitorExtension) {
				ISchemaElement sElement = XMLUtil.getSchemaElement(node, ((IMonitorExtension) obj).getPoint());
	...
	private ICompletionProposal[] computeOpenTagProposal(IDocumentElementNode node, int offset, IDocument doc) {
		IMonitorObject obj = XMLUtil.getTopLevelParent(node);
		if (obj instanceof IMonitorExtension) {
			ISchemaElement sElem = XMLUtil.getSchemaElement(node, ((IMonitorExtension) obj).getPoint());
			if (sElem == null)
				return null;
	...
	private ICompletionProposal[] computeAddAttributeProposal(int type, IDocumentElementNode node, int offset, IDocument doc, String filter, String tag) {
		...
		if (type == F_EXTENSION || node instanceof IMonitorExtension) {
			...
			IMonitorObject obj = XMLUtil.getTopLevelParent(node);
			if (obj instanceof IMonitorExtension) {
				ISchemaElement sElem = XMLUtil.getSchemaElement(node, node != null ? ((IMonitorExtension) obj).getPoint() : null);
	...
	private ICompletionProposal[] computeBrokenModelProposal(IDocumentElementNode parent, int offset, IDocument doc) {
		...
		IMonitorObject obj = XMLUtil.getTopLevelParent(parent);
		if (obj instanceof IMonitorExtension) {
			String point = ((IMonitorExtension) obj).getPoint();
			if (attr == null)
		...
		} else if (parent instanceof IMonitorBase) {
			if (attr == null)
				return computeAddChildProposal(parent, elRepOffset, doc, element);
	...
	private ArrayList getExternalExtensionPoints(IMonitorModelBase model, int vSchemaType) {
		...
		for (int i = 0; i < plugins.length; i++) {
			...
			if (plugins[i].getMonitorBase().getId().equals(model.getMonitorBase().getId())) {
				// Skip this plugin
				continue;
			}
	...
	private ArrayList getInternalExtensionPoints(IMonitorModelBase model, int vSchemaType) {
	...
	private IMonitorModelBase getPluginModelBase() {
		...
		if ((formEditor instanceof MDEFormEditor) == false) {
			return null;
		}
		IBaseModel bModel = ((MDEFormEditor) formEditor).getAggregateModel();
		if ((bModel instanceof IMonitorModelBase) == false) {
			return null;
		}
		return (IMonitorModelBase) bModel;
	}
		...
		if (fImages[type] == null) {
			switch (type) {
				...
				case F_EXTENSION_ATTRIBUTE_POINT_VALUE :
					return fImages[type] = MDEPluginImages.DESC_EXT_POINT_OBJ.createImage();
				case F_EXTENSION_POINT_AND_VALUE :
				case F_EXTENSION :
					return fImages[type] = MDEPluginImages.DESC_EXTENSION_OBJ.createImage();
				case F_ELEMENT :
				case F_CLOSE_TAG :
					return fImages[type] = MDEPluginImages.DESC_XML_ELEMENT_OBJ.createImage();
				case F_ATTRIBUTE :
				case F_ATTRIBUTE_VALUE :
					return fImages[type] = MDEPluginImages.DESC_ATT_URI_OBJ.createImage();
				case F_ATTRIBUTE_ID_VALUE :
					return fImages[type] = MDEPluginImages.DESC_ATT_ID_OBJ.createImage();
				case F_ATTRIBUTE_BOOLEAN_VALUE :
					return fImages[type] = MDEPluginImages.DESC_ATT_BOOLEAN_OBJ.createImage();
			}
		}
		return fImages[type];
	}
	...
	public MDESourcePage getSourcePage() {
		return fSourcePage;
	}
