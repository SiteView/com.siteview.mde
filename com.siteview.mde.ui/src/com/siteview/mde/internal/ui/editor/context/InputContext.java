 ...
 *******************************************************************************/
package com.siteview.mde.internal.ui.editor.context;
...
import com.siteview.mde.internal.core.util.PropertiesUtil;
import com.siteview.mde.internal.ui.MDEPlugin;
import com.siteview.mde.internal.ui.MDEUIMessages;
import com.siteview.mde.internal.ui.editor.MDEFormEditor;
import com.siteview.mde.internal.ui.editor.MDEStorageDocumentProvider;
...
public abstract class InputContext {
	...
	public InputContext(MDEFormEditor editor, IEditorInput input, boolean primary) {
		this.fEditor = editor;
	...
	private IDocumentProvider createDocumentProvider(IEditorInput input) {
		if (input instanceof IFileEditorInput) {
			return new ForwardingDocumentProvider(getPartitionName(), getDocumentSetupParticipant(), MDEPlugin.getDefault().getTextFileDocumentProvider());
		}
		return new MDEStorageDocumentProvider(getDocumentSetupParticipant());
	}
		...
		if (!fValidated) {
			if (fEditorInput instanceof IFileEditorInput) {
				...
				if (file.isReadOnly()) {
					...
					IStatus validateStatus = MDEPlugin.getWorkspace().validateEdit(new IFile[] {file}, shell);
	...
	private void handleSaveAs(IProgressMonitor monitor, IPath path) throws Exception, CoreException, InterruptedException, InvocationTargetException {
		...
		if (path == null) {
			...
			throw new Exception(MDEUIMessages.InputContext_errorMessageLocationNotSet);
		}
