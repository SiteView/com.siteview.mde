...
import com.siteview.mde.internal.core.schema.SchemaObject;
...
public class DocSection extends MDESection {
	...
	public DocSection(MDEFormPage page, Composite parent, IColorManager colorManager) {
		super(page, parent, Section.DESCRIPTION, true);
		getSection().setText(MDEUIMessages.DocSection_text);
		getSection().setDescription(MDEUIMessages.SchemaEditor_DocSection_desc);
	...
	private String getTopicName(Object object) {
		if (object instanceof ISchema) {
			return MDEUIMessages.SchemaEditor_topic_overview;
			...
			if (sectionId.equals(IDocumentSection.EXAMPLES))
				return MDEUIMessages.SchemaEditor_topic_examples;
			if (sectionId.equals(IDocumentSection.SINCE))
				return MDEUIMessages.SchemaEditor_topic_since;
			if (sectionId.equals(IDocumentSection.IMPLEMENTATION))
				return MDEUIMessages.SchemaEditor_topic_implementation;
			if (sectionId.equalsIgnoreCase(IDocumentSection.API_INFO))
				return MDEUIMessages.SchemaEditor_topic_api;
			if (sectionId.equals(IDocumentSection.COPYRIGHT))
				return MDEUIMessages.SchemaEditor_topic_copyright;
		}
		return "?"; //$NON-NLS-1$
	}
	...
	private void updateTabImage(CTabItem item) {
		if (item != null) {
			...
			if (section != null)
				item.setImage(MDEPlugin.getDefault().getLabelProvider().getImage(section));
		}
	}
