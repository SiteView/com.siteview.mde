...
import com.siteview.mde.internal.ui.correction.java.FindClassResolutionsOperation.AbstractClassResolutionCollector;
...
public class QuickFixProcessor implements IQuickFixProcessor {
		...
		if (referencedElement != null) {
			...
			if (referencedJavaProject != null && WorkspaceModelManager.isPluginProject(referencedJavaProject.getProject())) {
				...
				if (!referencedJavaProject.equals(currentProject)) {
					IMonitorModelBase referencedModel = MonitorRegistry.findModel(referencedJavaProject.getProject());
	...
	private void handleAccessRestrictionByImportPackage(IProject currentProject, ExportPackageDescription desc, Collection results) {
		...
		if (supplier != null) {
			IMonitorModelBase base = MonitorRegistry.findModel(currentProject);
