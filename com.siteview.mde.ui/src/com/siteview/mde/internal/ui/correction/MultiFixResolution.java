...
public class MultiFixResolution extends WorkbenchMarkerResolution {
	...
	public MultiFixResolution(IMarker marker, String label) {
		...
		if (label != null)
			...
			fLabel = MDEUIMessages.MultiFixResolution_FixAll;
	...
	public IMarker[] findOtherMarkers(IMarker[] markers) {
		...
		try {
			String markerCategory = (String) fMarker.getAttribute(MDEMarkerFactory.CAT_ID);
			for (int i = 0; i < markers.length; i++) {
				if (markerCategory.equals(markers[i].getAttribute(MDEMarkerFactory.CAT_ID)) && !markers[i].equals(fMarker) && markers[i].getResource().equals(fMarker.getResource())) {
	...
	public Image getImage() {
		return MDEPluginImages.DESC_ADD_ATT.createImage();
	}
	...
	public boolean equals(Object obj) {
		...
		try {
			String categoryId = (String) multiFix.fMarker.getAttribute(MDEMarkerFactory.CAT_ID);
			if (categoryId == null)
				return false;
			return categoryId.equals(fMarker.getAttribute(MDEMarkerFactory.CAT_ID));
		} catch (CoreException e) {
		}
		return false;
	}
