...
import com.siteview.mde.internal.ui.search.dependencies.*;
...
public class OrganizeManifestsProcessor extends RefactoringProcessor implements IOrganizeManifestsSettings {
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm, CheckConditionsContext context) throws CoreException, OperationCanceledException {
		...
		for (Iterator i = fProjectList.iterator(); i.hasNext();) {
			if (!(i.next() instanceof IProject))
				status.addFatalError(MDEUIMessages.OrganizeManifestsProcessor_invalidParam);
		}
		return status;
	}
	...
	public Change createChange(IProgressMonitor pm) throws CoreException, OperationCanceledException {
		...
		pm.beginTask(MDEUIMessages.OrganizeManifestJob_taskName, fProjectList.size());
	...
	private CompositeChange cleanProject(IProject project, IProgressMonitor monitor) {
		...
		CompositeChange change = new CompositeChange(NLS.bind(MDEUIMessages.OrganizeManifestsProcessor_rootMessage, new String[] {fCurrentProject.getName()}));
		monitor.beginTask(NLS.bind(MDEUIMessages.OrganizeManifestsProcessor_rootMessage, new String[] {fCurrentProject.getName()}), getTotalTicksPerProject());
	...
	private void runCleanup(IProgressMonitor monitor, IBundlePluginModelBase modelBase, Change[] result) throws InvocationTargetException, InterruptedException {
		...
		IMonitorModelBase currentExtensionsModel = null;
		if (sharedExtensionsModel instanceof IMonitorModelBase)
			currentExtensionsModel = (IMonitorModelBase) sharedExtensionsModel;

		String projectName = fCurrentProject.getName();

		if (fAddMissing || fRemoveUnresolved) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_export, projectName));
		...
		if (fMarkInternal) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_filterInternal, projectName));
			if (!monitor.isCanceled())
		...
		if (fModifyDep) {
			String message = fRemoveDependencies ? NLS.bind(MDEUIMessages.OrganizeManifestsOperation_removeUnresolved, projectName) : NLS.bind(MDEUIMessages.OrganizeManifestsOperation_markOptionalUnresolved, projectName);
		...
		if (fAddDependencies) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_additionalDeps, projectName));
			if (!monitor.isCanceled()) {
		...
		if (fUnusedDependencies) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_unusedDeps, projectName));
			if (!monitor.isCanceled()) {
		...
		if (fRemoveLazy) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_lazyStart, fCurrentProject.getName()));
			if (!monitor.isCanceled())
		...
		if (fRemoveUselessFiles) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_uselessPluginFile, fCurrentProject.getName()));
			if (!monitor.isCanceled()) {
		...
		if (fPrefixIconNL) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_nlIconPath, projectName));
			if (!monitor.isCanceled())
		...
		if (fUnusedKeys) {
			monitor.subTask(NLS.bind(MDEUIMessages.OrganizeManifestsOperation_unusedKeys, projectName));
			if (!monitor.isCanceled()) {
	...
	public String getProcessorName() {
		return MDEUIMessages.OrganizeManifestsWizardPage_title;
	}
