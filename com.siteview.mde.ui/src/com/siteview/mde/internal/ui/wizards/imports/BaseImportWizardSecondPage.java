...
public abstract class BaseImportWizardSecondPage extends WizardPage implements IModelProviderListener {
	...
	protected Composite createImportList(Composite parent) {
		...
		label.setText(MDEUIMessages.ImportWizard_DetailedPage_importList);
		...
		fImportListViewer.setInput(MDECore.getDefault().getModelManager().getExternalModelManager());
		fImportListViewer.setComparator(ListUtil.PLUGIN_COMPARATOR);
		return container;
	}

	protected void createComputationsOption(Composite parent) {
		fAddFragmentsButton = SWTFactory.createCheckButton(parent, MDEUIMessages.ImportWizard_SecondPage_addFragments, null, true, 1);
		...
		if (!MDEPlugin.getWorkspace().isAutoBuilding()) {
			fAutoBuildButton = SWTFactory.createCheckButton(parent, MDEUIMessages.BaseImportWizardSecondPage_autobuild, null, getDialogSettings().getBoolean(SETTINGS_AUTOBUILD), 1);
		}
	}
	...
	protected void addDependencies(IMonitorModelBase model, ArrayList selected, boolean addFragments) {
		...
		if (required.length > 0) {
			for (int i = 0; i < required.length; i++) {
				IMonitorModelBase found = findModel(required[i].getId(), required[i].getVersion());
				if (found != null) {
		...
		if (addFragments) {
			if (model instanceof IMonitorModel) {
				...
				IMonitorModelBase found = findModel(fragment.getPluginId(), fragment.getVersion());
				if (found != null) {
					addPluginAndDependencies(found, selected, addFragments);
				}
			}
	...
	public IMonitorModelBase[] getModelsToImport() {
	...
	protected void checkRepositoryAvailability() {
		...
		if (page.getImportType() == PluginImportOperation.IMPORT_FROM_REPOSITORY) {
			if (getMessageType() != ERROR && getMessageType() != WARNING) {
				IMonitorModelBase[] selected = getModelsToImport();
				Set available = page.repositoryModels;
				for (int i = 0; i < selected.length; i++) {
					if (!available.contains(selected[i])) {
						setMessage(MDEUIMessages.BaseImportWizardSecondPage_0, WARNING);
						return;
					}
				}
			}
