...
public abstract class ContentPage extends WizardPage {
	...
	protected String validateProperties() {
		...
		if (!fInitialized) {
			if (!fIdText.getText().trim().equals(fProjectProvider.getProjectName())) {
				setMessage(MDEUIMessages.ContentPage_illegalCharactersInID, INFORMATION);
			} else {
				setMessage(null);
			}
			return null;
	...
	protected String validateVersion(Text text) {
		if (text.getText().trim().length() == 0) {
			return PDELabelUtility.qualifyMessage(PDELabelUtility.getFieldLabel(text), MDEUIMessages.ControlValidationUtility_errorMsgValueMustBeSpecified);
		} else if (!isVersionValid(text.getText().trim())) {
			return PDELabelUtility.qualifyMessage(PDELabelUtility.getFieldLabel(text), MDECoreMessages.BundleErrorReporter_InvalidFormatInBundleVersion);
		}
		return null;
	}
	...
	private String validateId() {
		...
		if (id.length() == 0)
			return MDEUIMessages.ContentPage_noid;

		if (!IdUtil.isValidCompositeID3_0(id)) {
			return MDEUIMessages.ContentPage_invalidId;
		}
		return null;
	}
