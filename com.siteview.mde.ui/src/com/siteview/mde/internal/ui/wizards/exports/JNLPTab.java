...
public class JNLPTab {
	...
	public Control createControl(Composite parent) {
		...
		fButton.setText(MDEUIMessages.AdvancedFeatureExportPage_createJNLP);
		...
		fURLLabel = createLabel(jnlpGroup, MDEUIMessages.AdvancedFeatureExportPage_siteURL);
		fURLText = createText(jnlpGroup);

		fVersionLabel = createLabel(jnlpGroup, MDEUIMessages.AdvancedFeatureExportPage_jreVersion);
	...
	protected String validate() {
		...
		if (fButton.getSelection()) {
			if (fURLText.getText().trim().length() == 0) {
				error = MDEUIMessages.AdvancedFeatureExportPage_noSite;
			} else if (fVersionText.getText().trim().length() == 0) {
				error = MDEUIMessages.AdvancedFeatureExportPage_noVersion;
			}
		}
		return error;
	}
