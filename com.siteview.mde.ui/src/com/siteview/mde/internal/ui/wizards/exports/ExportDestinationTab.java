...
public class ExportDestinationTab extends AbstractExportTab {
	...
	public Control createControl(Composite parent) {
		...
		fDirectoryButton = SWTFactory.createRadioButton(composite, MDEUIMessages.ExportWizard_directory, 2);
		...
		fBrowseDirectory = SWTFactory.createPushButton(composite, MDEUIMessages.ExportWizard_browse, null);
		SWTUtil.setButtonDimensionHint(fBrowseDirectory);

		fArchiveFileButton = SWTFactory.createRadioButton(composite, MDEUIMessages.ExportWizard_archive, 2);

		fArchiveCombo = SWTFactory.createCombo(composite, SWT.BORDER, 1, null);
		((GridData) fArchiveCombo.getLayoutData()).horizontalIndent = 15;

		fBrowseFile = SWTFactory.createPushButton(composite, MDEUIMessages.ExportWizard_browse, null);
		SWTUtil.setButtonDimensionHint(fBrowseFile);

		fInstallButton = SWTFactory.createRadioButton(composite, MDEUIMessages.ExportDestinationTab_InstallIntoCurrentPlatform, 2);

		fInstallCombo = SWTFactory.createCombo(composite, SWT.BORDER, 1, null);
		((GridData) fInstallCombo.getLayoutData()).horizontalIndent = 15;

		fBrowseInstall = SWTFactory.createPushButton(composite, MDEUIMessages.ExportWizard_browse, null);
	...
	protected void initialize(IDialogSettings settings) {
		...
		String defaultRepo = MDEPlugin.getWorkspace().getRoot().getLocation() + "/.metadata/.plugins/org.eclipse.pde.core/install/"; //$NON-NLS-1$
