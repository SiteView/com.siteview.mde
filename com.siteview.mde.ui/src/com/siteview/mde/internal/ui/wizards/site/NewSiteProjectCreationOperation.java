...
public class NewSiteProjectCreationOperation extends WorkspaceModifyOperation {
	...
	protected void execute(IProgressMonitor monitor) throws CoreException, InvocationTargetException, InterruptedException {
		...
		monitor.beginTask(MDEUIMessages.NewSiteWizard_creatingProject, numUnits);

		CoreUtility.createProject(fProject, fPath, monitor);
		fProject.open(monitor);
		CoreUtility.addNatureToProject(fProject, MDE.SITE_NATURE, monitor);
		monitor.worked(1);
		...
		monitor.subTask(MDEUIMessages.NewSiteWizard_creatingManifest);
	...
	private IFile createSiteManifest() throws CoreException {
		...
		IDE.setDefaultEditor(file, IMDEUIConstants.SITE_EDITOR_ID);
		return file;
	}
	...
	private void openFile(final IFile file) {
		fDisplay.asyncExec(new Runnable() {
			public void run() {
				...
				try {
					page.openEditor(new FileEditorInput(file), IMDEUIConstants.SITE_EDITOR_ID);
	...
	private void createHTMLFile() {
		...
		writer.println("				alert(\"" + MDEUIMessages.SiteHTML_loadError + "\");"); //$NON-NLS-1$ //$NON-NLS-2$ 
