...
public class TargetDefinitionExportWizardPage extends WizardPage {
	...
	protected TargetDefinitionExportWizardPage(ITargetDefinition target) {
		...
		setTitle(MDEUIMessages.ExportActiveTargetDefinition);
		setMessage(MDEUIMessages.ExportActiveTargetDefinition_message);
	}
	...
	private void createExportDirectoryControl(Composite parent) {
		...
		new Label(parent, SWT.NONE).setText(MDEUIMessages.ExportTargetCurrentTarget);
		...
		gd.horizontalSpan = 2;
		l.setLayoutData(gd);
		new Label(parent, SWT.NONE).setText(MDEUIMessages.ExportTargetChooseFolder);
		...
		fBrowseButton.setText(MDEUIMessages.ExportTargetBrowse);
		fBrowseButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				DirectoryDialog dialog = new DirectoryDialog(getShell());
				dialog.setText(MDEUIMessages.ExportTargetSelectDestination);
				dialog.setMessage(MDEUIMessages.ExportTargetSpecifyDestination);
		...
		fClearDestinationButton.setText(MDEUIMessages.ExportTargetClearDestination);
		gd = new GridData();
		gd.horizontalSpan = 2;
	...
	protected boolean validate() {
		...
		if (fDestinationCombo.getText().equals("")) { //$NON-NLS-1$
			setErrorMessage(MDEUIMessages.ExportTargetError_ChooseDestination);
			return false;
		} else if (!isValidLocation(fDestinationCombo.getText().trim())) {
			setErrorMessage(MDEUIMessages.ExportTargetError_validPath);
			return false;
		}
