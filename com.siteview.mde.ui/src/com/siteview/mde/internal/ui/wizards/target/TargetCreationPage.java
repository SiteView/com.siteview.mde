...
public class TargetCreationPage extends WizardSelectionPage {
	...
	public TargetCreationPage(String pageName) {
		super(pageName);
		setTitle(MDEUIMessages.TargetProfileWizardPage_title);
		setDescription(MDEUIMessages.TargetProfileWizardPage_description);
	}
	...
	protected void initializeTargetCombo() {
		IConfigurationElement[] elements = MDECore.getDefault().getTargetProfileManager().getSortedTargets();
	...
	protected ITargetDefinition createTarget(int targetOption) {
		...
		if (service != null) {
			...
			switch (targetOption) {
				case USE_EMPTY :
					definition.setName(MDEUIMessages.TargetCreationPage_6);
					break;
				case USE_DEFAULT :
					try {
