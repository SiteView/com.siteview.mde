...
public class OrganizeManifestsWizardPage extends UserInputWizardPage implements ILaunchingPreferenceConstants, IOrganizeManifestsSettings {
	...
	private static String title = MDEUIMessages.OrganizeManifestsWizardPage_title;

	protected OrganizeManifestsWizardPage() {
		super(title);
		setTitle(title);
		setDescription(MDEUIMessages.OrganizeManifestsWizardPage_description);
	}
	...
	private void createExportedPackagesGroup(Composite container) {
		Group group = createGroup(container, MDEUIMessages.OrganizeManifestsWizardPage_exportedGroup, 1, true);

		fAddMissing = new Button(group, SWT.CHECK);
		fAddMissing.setText(MDEUIMessages.OrganizeManifestsWizardPage_addMissing);

		fMarkInternal = new Button(group, SWT.CHECK);
		fMarkInternal.setText(MDEUIMessages.OrganizeManifestsWizardPage_markInternal);
		...
		fPackageFilterLabel.setText(MDEUIMessages.OrganizeManifestsWizardPage_packageFilter);
		...
		fRemoveUnresolved.setText(MDEUIMessages.OrganizeManifestsWizardPage_removeUnresolved);
		gd = new GridData();
		gd.verticalIndent = 5;
		fRemoveUnresolved.setLayoutData(gd);

		fCalculateUses = new Button(group, SWT.CHECK);
		fCalculateUses.setText(MDEUIMessages.OrganizeManifestsWizardPage_calculateUses);
		gd = new GridData();
		gd.verticalIndent = 5;
	...
	private void createRequireImportGroup(Composite container) {
		Group group = createGroup(container, MDEUIMessages.OrganizeManifestsWizardPage_dependenciesGroup, 1, true);
		...
		fModifyDependencies.setText(MDEUIMessages.OrganizeManifestsWizardPage_unresolvedDependencies);

		fRemoveImport = new Button(comp, SWT.RADIO);
		fRemoveImport.setText(MDEUIMessages.OrganizeManifestsWizardPage_remove);

		fOptionalImport = new Button(comp, SWT.RADIO);
		fOptionalImport.setText(MDEUIMessages.OrganizeManifestsWizardPage_markOptional);

		fUnusedDependencies = new Button(group, SWT.CHECK);
		fUnusedDependencies.setText(MDEUIMessages.OrganizeManifestsWizardPage_removeUnused);

		fAdditonalDependencies = new Button(group, SWT.CHECK);
		fAdditonalDependencies.setText(MDEUIMessages.OrganizeManifestsWizardPage_addDependencies);
	}

	private void createGeneralGroup(Composite container) {
		Group group = createGroup(container, MDEUIMessages.OrganizeManifestsWizardPage_generalGroup, 1, true);

		fRemoveLazy = new Button(group, SWT.CHECK);
		fRemoveLazy.setText(MDEUIMessages.OrganizeManifestsWizardPage_lazyStart);

		fRemoveUselessFiles = new Button(group, SWT.CHECK);
		fRemoveUselessFiles.setText(MDEUIMessages.OrganizeManifestsWizardPage_uselessPluginFile);

	}

	private void createNLSGroup(Composite container) {
		Group group = createGroup(container, MDEUIMessages.OrganizeManifestsWizardPage_internationalizationGroup, 1, true);

		fFixIconNLSPaths = new Button(group, SWT.CHECK);
		fFixIconNLSPaths.setText(MDEUIMessages.OrganizeManifestsWizardPage_prefixNL);

		fRemovedUnusedKeys = new Button(group, SWT.CHECK);
		fRemovedUnusedKeys.setText(MDEUIMessages.OrganizeManifestsWizardPage_removeUnusedKeys);
	}
