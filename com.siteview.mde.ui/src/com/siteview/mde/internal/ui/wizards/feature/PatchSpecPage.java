...
import com.siteview.mde.internal.ui.wizards.BundleProviderHistoryUtil;
...
public class PatchSpecPage extends AbstractFeatureSpecPage {
	...
	public PatchSpecPage() {
		super();
		setTitle(MDEUIMessages.PatchSpec_title);
		setDescription(MDEUIMessages.NewFeatureWizard_SpecPage_desc);
	}
	...
	protected void initialize() {
		...
		setMessage(MDEUIMessages.FeaturePatch_MainPage_desc);
	}

	protected String validateContent() {
		fFeatureToPatch = MDECore.getDefault().getFeatureModelManager().findFeatureModel(fFeatureIdText.getText(), fFeatureVersionText.getText());
		if (fFeatureToPatch != null) {
			setMessage(null);
			return null;
		}

		setMessage(NLS.bind(MDEUIMessages.NewFeaturePatch_SpecPage_notFound, fFeatureIdText.getText(), fFeatureVersionText.getText()), IMessageProvider.WARNING);
	...
	protected String verifyIdRules() {
		...
		if (id == null || id.length() == 0)
			return MDEUIMessages.NewFeatureWizard_SpecPage_pmissing;
	...
	protected void createTopGroup(Composite container) {
		...
		patchGroup.setText(MDEUIMessages.NewFeatureWizard_SpecPage_patchProperties);
		Label label = new Label(patchGroup, SWT.NULL);
		label.setText(MDEUIMessages.NewFeaturePatch_SpecPage_id);
		...
		label.setText(MDEUIMessages.NewFeaturePatch_SpecPage_name);
		fPatchNameText = new Text(patchGroup, SWT.BORDER);
		fPatchNameText.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		label = new Label(patchGroup, SWT.NULL);
		label.setText(MDEUIMessages.NewFeaturePatch_SpecPage_provider);
	...
	protected void createContents(Composite container) {
		...
		group.setText(MDEUIMessages.BaseFeatureSpecPage_patchGroup_title);

		Label label = new Label(group, SWT.NULL);
		label.setText(MDEUIMessages.NewFeatureWizard_SpecPage_id);
		...
		fBrowseButton.setText(MDEUIMessages.BaseFeatureSpecPage_browse);
		fBrowseButton.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_END));
		fBrowseButton.addSelectionListener(new SelectionAdapter() {

			public void widgetSelected(SelectionEvent e) {
				FeatureSelectionDialog dialog = new FeatureSelectionDialog(getShell(), MDECore.getDefault().getFeatureModelManager().getModels(), false);
				dialog.create();
