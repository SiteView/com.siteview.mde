...
public class ConvertProjectsAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		if (unconverted.length == 0) {
			MessageDialog.openInformation(this.getDisplay().getActiveShell(), MDEUIMessages.ConvertProjectsAction_find, MDEUIMessages.ConvertProjectsAction_none); // 
			return;
		}
	...
	private IProject[] getUnconvertedProjects() {
		...
		for (int i = 0; i < projects.length; i++) {
			if (projects[i].isOpen() && !MDE.hasPluginNature(projects[i]) && !MDE.hasFeatureNature(projects[i]) && !MDE.hasUpdateSiteNature(projects[i]) && projects[i].getName().indexOf('%') == -1 && projects[i].getLocation().toString().indexOf('%') == -1)
