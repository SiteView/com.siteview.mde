...
public abstract class AbstractCreateFeatureOperation extends WorkspaceModifyOperation {
	...
	protected void createFeature(IProgressMonitor monitor) throws CoreException {
		monitor.beginTask(MDEUIMessages.NewFeatureWizard_creatingProject, 3);
		IFile file;
		if (shouldOverwriteFeature()) {
			...
			// create feature.xml
			monitor.subTask(MDEUIMessages.NewFeatureWizard_creatingManifest);
			file = createFeature();
			monitor.worked(1);
		} else {
	...
	protected void createBuildProperties() throws CoreException {
		...
		IDE.setDefaultEditor(file, IMDEUIConstants.BUILD_EDITOR_ID);
	}
	...
	protected IFile createFeature() throws CoreException {
		...
		info.setDescription(MDEUIMessages.NewFeatureWizard_sampleCopyrightDesc);
		...
		info.setDescription(MDEUIMessages.NewFeatureWizard_sampleLicenseDesc);
		...
		info.setDescription(MDEUIMessages.NewFeatureWizard_sampleDescriptionDesc);

		// Save the model
		model.save();
		model.dispose();
		IDE.setDefaultEditor(file, IMDEUIConstants.FEATURE_EDITOR_ID);
		return file;
	}
	...
	protected void openFeatureEditor(IFile manifestFile) {
		...
		// Open the editor
		try {
			page.openEditor(new FileEditorInput(manifestFile), IMDEUIConstants.FEATURE_EDITOR_ID);
	...
	protected boolean shouldOverwriteFeature() {
		return !fLocation.append(fProject.getName()).toFile().exists() || MessageDialog.openQuestion(MDEPlugin.getActiveWorkbenchShell(), this instanceof CreateFeaturePatchOperation ? MDEUIMessages.FeaturePatch_wtitle : MDEUIMessages.NewFeatureWizard_wtitle, MDEUIMessages.NewFeatureWizard_overwriteFeature);
	}
}
