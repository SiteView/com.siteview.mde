...
import com.siteview.mde.internal.ui.shared.target.Messages;
...
public class FeatureImportWizardPage extends WizardPage {
	...
	public FeatureImportWizardPage() {
		...
		setTitle(MDEUIMessages.FeatureImportWizard_FirstPage_title);
		setDescription(MDEUIMessages.FeatureImportWizard_FirstPage_desc);
	...
	public void createControl(Composite parent) {
		...
		fSelectAllButton = SWTFactory.createPushButton(buttonComp, MDEUIMessages.WizardCheckboxTablePart_selectAll, null);
		...
		fDeselectAllButton = SWTFactory.createPushButton(buttonComp, MDEUIMessages.WizardCheckboxTablePart_deselectAll, null);
		...
		fBinaryButton = SWTFactory.createCheckButton(composite, MDEUIMessages.FeatureImportWizard_FirstPage_binaryImport, null, true, 1);
	...
	private IPath chooseDropLocation() {
		...
		dialog.setText(MDEUIMessages.FeatureImportWizard_messages_folder_title);
		dialog.setMessage(MDEUIMessages.FeatureImportWizard_messages_folder_message);
	...
	private void validateDropLocation() {
		...
		if (isOtherLocation()) {
			...
			if (curr.segmentCount() == 0)
				errorMessage = MDEUIMessages.FeatureImportWizard_errors_locationMissing;
			else if (!Path.ROOT.isValidPath(fDropLocation.getText()))
				errorMessage = MDEUIMessages.FeatureImportWizard_errors_buildFolderInvalid;
			else {
				File file = curr.toFile();
				if (!file.exists() || !file.isDirectory())
					errorMessage = MDEUIMessages.FeatureImportWizard_errors_buildFolderMissing;
			}
		}
	...
	public void createFeatureTable(Composite parent) {
		...
		fFeatureViewer.setLabelProvider(MDEPlugin.getDefault().getLabelProvider());
	...
	private void updateCounter() {
		...
		fCounterLabel.setText(NLS.bind(MDEUIMessages.WizardCheckboxTablePart_counter, new String[] {new Integer(checked).toString(), new Integer(total).toString()}));
	}
	...
	public IFeatureModel[] getModels() {
		...
		IRunnableWithProgress runnable = new IRunnableWithProgress() {
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				monitor.beginTask(MDEUIMessages.FeatureImportWizard_messages_updating, IProgressMonitor.UNKNOWN);
	...
	private MultiStatus doLoadFeatures(ArrayList result, File path) {
		...
		return new MultiStatus(IMDEUIConstants.PLUGIN_ID, IStatus.OK, (IStatus[]) resultStatus.toArray(new IStatus[resultStatus.size()]), MDEUIMessages.FeatureImportWizard_DetailedPage_problemsLoading, null);
	}
	...
	private IStatus doLoadFeature(File dir, File manifest, ArrayList result) {
		...
		try {
			...
			if (!model.isValid()) {
				status = new Status(IStatus.WARNING, IMDEUIConstants.PLUGIN_ID, IStatus.OK, NLS.bind(MDEUIMessages.FeatureImportWizardPage_importHasInvalid, dir), null);
			}
		} catch (Exception e) {
			// Errors in the file
			status = new Status(IStatus.ERROR, IMDEUIConstants.PLUGIN_ID, IStatus.OK, e.getMessage(), e);
		}
		if (stream != null) {
			try {
				stream.close();
