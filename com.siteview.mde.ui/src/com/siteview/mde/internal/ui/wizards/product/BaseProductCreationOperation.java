...
public class BaseProductCreationOperation extends WorkspaceModifyOperation {
	...
	protected void execute(IProgressMonitor monitor) throws CoreException, InvocationTargetException, InterruptedException {
		monitor.beginTask(MDEUIMessages.BaseProductCreationOperation_taskName, 2);
		createContent();
		monitor.worked(1);
		openFile();
		monitor.done();
	...
	private Properties getProductProperties(IMonitorElement element) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (child.getName().equals("property")) { //$NON-NLS-1$
				String name = null;
				String value = null;
				IMonitorAttribute attr = child.getAttribute("name"); //$NON-NLS-1$
				if (attr != null)
					name = attr.getValue();
	...
	protected IMonitorElement getProductExtension(String productId) {
		...
		if (model != null) {
			...
			for (int i = 0; i < extensions.length; i++) {
				if ("org.eclipse.core.runtime.products".equals(extensions[i].getPoint()) //$NON-NLS-1$
					...
					IMonitorObject[] children = extensions[i].getChildren();
					if (children.length > 0) {
						IMonitorElement object = (IMonitorElement) children[0];
	...
	protected void initializeProductInfo(IProductModelFactory factory, IProduct product, String id) {
		...
		IMonitorElement element = getProductExtension(id);
		if (element != null) {
	...
	protected void addPlugins(IProductModelFactory factory, IProduct product, Map plugins) {
		...
		IMonitorModelBase[] models = (IMonitorModelBase[]) plugins.keySet().toArray(new IMonitorModelBase[plugins.size()]);
		for (int i = 0; i < models.length; i++) {
			IMonitorModelBase model = models[i];

			// create plug-in model
			IProductPlugin pplugin = factory.createPlugin();
			pplugin.setId(model.getMonitorBase().getId());
			pplugins[i] = pplugin;
			...
			if (!model.isFragmentModel() && !sl.equals("default:default")) { //$NON-NLS-1$
				...
				configuration.setId(model.getMonitorBase().getId());
	...
	private void openFile() {
		Display.getCurrent().asyncExec(new Runnable() {
			public void run() {
				...
				try {
					IDE.openEditor(page, fFile, IMDEUIConstants.PRODUCT_EDITOR_ID);
