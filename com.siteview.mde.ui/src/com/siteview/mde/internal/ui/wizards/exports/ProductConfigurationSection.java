...
public class ProductConfigurationSection {
	...
	public Control createControl(Composite parent) {
		...
		group.setText(MDEUIMessages.ProductExportWizardPage_productGroup);

		Label label = new Label(group, SWT.NONE);
		label.setText(MDEUIMessages.ProductExportWizardPage_config);
		...
		browse.setText(MDEUIMessages.ProductExportWizardPage_browse);
		...
		label.setText(MDEUIMessages.ProductExportWizardPage_root);
	...
	private void handleBrowse() {
		...
		dialog.setMessage(MDEUIMessages.ProductExportWizardPage_productSelection);
	...
	protected IFile getProductFile(String path) {
		...
		return thePath.segmentCount() < 2 ? null : MDEPlugin.getWorkspace().getRoot().getFile(new Path(path));
	}
	...
	protected void updateProductFields() {
		...
		try {
			if (file != null && file.exists())
				root = file.getPersistentProperty(IMDEUIConstants.DEFAULT_PRODUCT_EXPORT_ROOT);
		} catch (CoreException e) {
		}
		if (root != null)
	...
	protected void saveSettings(IDialogSettings settings) {
		...
		try {
			if (file != null && file.exists()) {
				file.setPersistentProperty(IMDEUIConstants.DEFAULT_PRODUCT_EXPORT_ROOT, getRootDirectory());
			}
		} catch (CoreException e) {
		}
	}
	...
	protected String validate() {
		...
		if (configLocation.length() == 0)
			return MDEUIMessages.ProductExportWizardPage_noProduct;

		IPath path = new Path(configLocation);
		IResource resource = MDEPlugin.getWorkspace().getRoot().findMember(path);
		if (resource == null || !(resource instanceof IFile))
			return MDEUIMessages.ProductExportWizardPage_productNotExists;

		if (!resource.getName().endsWith(".product")) //$NON-NLS-1$
			return MDEUIMessages.ProductExportWizardPage_wrongExtension;
