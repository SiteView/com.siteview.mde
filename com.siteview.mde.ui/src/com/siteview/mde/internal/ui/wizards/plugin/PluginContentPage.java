...
public class PluginContentPage extends ContentPage {
	...
	private final static String NO_EXECUTION_ENVIRONMENT = MDEUIMessages.PluginContentPage_noEE;
	...
	public PluginContentPage(String pageName, IProjectProvider provider, NewProjectCreationPage page, AbstractFieldData data) {
		...
		setTitle(MDEUIMessages.ContentPage_title);
		setDescription(MDEUIMessages.ContentPage_desc);
	}
	...
	private void createPluginPropertiesGroup(Composite container) {
		Group propertiesGroup = SWTFactory.createGroup(container, MDEUIMessages.ContentPage_pGroup, 3, 1, GridData.FILL_HORIZONTAL);

		Label label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_pid);
		fIdText = createText(propertiesGroup, propertiesListener, 2);

		label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_pversion);
		fVersionText = createText(propertiesGroup, propertiesListener, 2);

		label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_pname);
		fNameText = createText(propertiesGroup, propertiesListener, 2);

		label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_pprovider);
	...
	private void createExecutionEnvironmentControls(Composite container) {
		...
		fEELabel.setText(MDEUIMessages.NewProjectCreationPage_executionEnvironments_label);
		...
		fExeEnvButton.setText(MDEUIMessages.NewProjectCreationPage_environmentsButton);
	...
	protected void createPluginClassGroup(Composite container) {
		Group classGroup = SWTFactory.createGroup(container, MDEUIMessages.ContentPage_pClassGroup, 2, 1, GridData.FILL_HORIZONTAL);

		IDialogSettings settings = getDialogSettings();

		fGenerateActivator = SWTFactory.createCheckButton(classGroup, MDEUIMessages.ContentPage_generate, null, (settings != null) ? !settings.getBoolean(S_GENERATE_ACTIVATOR) : true, 2);
		...
		fClassLabel.setText(MDEUIMessages.ContentPage_classname);
		GridData gd = new GridData();
		gd.horizontalIndent = 20;
		fClassLabel.setLayoutData(gd);
		fClassText = createText(classGroup, classListener);

		fUIPlugin = SWTFactory.createCheckButton(classGroup, MDEUIMessages.ContentPage_uicontribution, null, (settings != null) ? !settings.getBoolean(S_UI_PLUGIN) : true, 2);
		...
		fApiAnalysisButton = SWTFactory.createCheckButton(classGroup, MDEUIMessages.PluginContentPage_enable_api_analysis, null, false, 2);
	...
	protected void createRCPQuestion(Composite parent, int horizontalSpan) {
		fRCPGroup = SWTFactory.createGroup(parent, MDEUIMessages.PluginContentPage_rcpGroup, 2, 1, GridData.FILL_HORIZONTAL);
		...
		label.setText(MDEUIMessages.PluginContentPage_appQuestion);
		...
		fYesButton.setText(MDEUIMessages.PluginContentPage_yes);
		...
		fNoButton.setText(MDEUIMessages.PluginContentPage_no);
		...
		if (errorMessage == null) {
			...
			if (fEEChoice.isEnabled()) {
				...
				if (ee != null && ee.getCompatibleVMs().length == 0) {
					errorMessage = MDEUIMessages.NewProjectCreationPage_invalidEE;
				}
			}
		}
