...
public class FragmentContentPage extends ContentPage {
	...
	private void createFragmentPropertiesGroup(Composite container) {
		...
		propertiesGroup.setText(MDEUIMessages.ContentPage_fGroup);

		Label label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_fid);
		fIdText = createText(propertiesGroup, propertiesListener, 2);

		label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_fversion);
		fVersionText = createText(propertiesGroup, propertiesListener, 2);

		label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_fname);
		fNameText = createText(propertiesGroup, propertiesListener, 2);

		label = new Label(propertiesGroup, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_fprovider);
	...
	private void createParentPluginGroup(Composite container) {
		...
		parentGroup.setText(MDEUIMessages.ContentPage_parentPluginGroup);
	...
	private Composite createNewVersionComp(Composite notebook) {
		...
		label.setText(MDEUIMessages.FragmentContentPage_pid);
	...
	private Composite createOldVersionComp(Composite notebook) {
		...
		label.setText(MDEUIMessages.FragmentContentPage_pversion);
		fPluginVersion = createText(comp, listener);

		label = new Label(comp, SWT.NONE);
		label.setText(MDEUIMessages.ContentPage_matchRule);
	...
	private Text createPluginIdContainer(Composite parent, final boolean validateRange, int span) {
		...
		browse.setText(MDEUIMessages.ContentPage_browse);
	...
	protected void validatePage() {
		...
		if (errorMessage == null) {
			...
			if (pluginID.length() == 0) {
				errorMessage = MDEUIMessages.ContentPage_nopid;
			} else if (!(MonitorRegistry.findModel(pluginID) instanceof IMonitorModel)) {
				errorMessage = MDEUIMessages.ContentPage_pluginNotFound;
			} else {
				if (fNewVersion) {
