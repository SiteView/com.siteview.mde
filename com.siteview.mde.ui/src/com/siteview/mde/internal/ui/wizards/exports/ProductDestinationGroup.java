...
public class ProductDestinationGroup extends AbstractExportTab {
	...
	public Control createControl(Composite parent) {
		...
		group.setText(MDEUIMessages.ExportWizard_destination);
		group.setLayout(new GridLayout(3, false));
		group.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		fDirectoryButton = new Button(group, SWT.RADIO);
		fDirectoryButton.setText(MDEUIMessages.ExportWizard_directory);
		...
		fBrowseDirectory.setText(MDEUIMessages.ExportWizard_browse);
		...
		fArchiveFileButton.setText(MDEUIMessages.ExportWizard_archive);
		...
		fBrowseFile.setText(MDEUIMessages.ExportWizard_browse);
	...
	protected void initialize(IDialogSettings settings, IFile file) {
		try {
			String toDirectory = (file != null) ? file.getPersistentProperty(IMDEUIConstants.DEFAULT_PRODUCT_EXPORT_DIR) : null;
			if (toDirectory == null)
	...
	protected void updateDestination(IFile file) {
		try {
			if (file == null)
				return;
			String toDirectory = file.getPersistentProperty(IMDEUIConstants.DEFAULT_PRODUCT_EXPORT_DIR);
			if (toDirectory == null)
				return;
			...
			String destination = file.getPersistentProperty(IMDEUIConstants.DEFAULT_PRODUCT_EXPORT_LOCATION);
			if (destination != null) {
	...
	protected void saveSettings(IDialogSettings settings) {
		...
		try {
			if (file != null && file.exists()) {
				file.setPersistentProperty(IMDEUIConstants.DEFAULT_PRODUCT_EXPORT_DIR, Boolean.toString(doExportToDirectory()));
				file.setPersistentProperty(IMDEUIConstants.DEFAULT_PRODUCT_EXPORT_LOCATION, doExportToDirectory() ? fDirectoryCombo.getText().trim() : fArchiveCombo.getText().trim());
			}
		} catch (CoreException e) {
		}
	}
	...
	private void chooseDestination() {
		...
		if (path.trim().length() == 0)
			path = MDEPlugin.getWorkspace().getRoot().getLocation().toString();
		dialog.setFilterPath(path);
		dialog.setText(MDEUIMessages.ExportWizard_dialog_title);
		dialog.setMessage(MDEUIMessages.ExportWizard_dialog_message);
	...
	protected String validate() {
		if (fArchiveFileButton.getSelection()) {
			if (fArchiveCombo.getText().trim().length() == 0)
				return MDEUIMessages.ExportWizard_status_nofile;
			else if (!isValidLocation(fArchiveCombo.getText().trim()))
				return MDEUIMessages.ExportWizard_status_invaliddirectory;
		}
		if (fDirectoryButton.getSelection()) {
			if (fDirectoryCombo.getText().trim().length() == 0)
				return MDEUIMessages.ExportWizard_status_nodirectory;
