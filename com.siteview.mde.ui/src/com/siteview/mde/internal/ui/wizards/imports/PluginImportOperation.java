...
import com.siteview.mde.internal.core.bundle.WorkspaceBundleModel;
...
public class PluginImportOperation extends WorkspaceJob {
	 ...
	 */
	public PluginImportOperation(IMonitorModelBase[] models, int importType, boolean forceAutobuild) {
		super(MDEUIMessages.ImportWizard_title);
		fModels = models;
	...
	private class NotImportedProjectsWarningDialog extends MessageDialog {
		...
		public NotImportedProjectsWarningDialog(String warningMessage, List namesOfNotImportedProjects) {
			super(PlatformUI.getWorkbench().getModalDialogShellProvider().getShell(), MDEUIMessages.ImportWizard_title, null, warningMessage, MessageDialog.WARNING, new String[] {IDialogConstants.OK_LABEL}, 0);
	...
	public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
		try {
			...
			MultiStatus multiStatus = new MultiStatus(MDEPlugin.getPluginId(), IStatus.OK, MDEUIMessages.ImportWizard_operation_multiProblem, null);
			...
			if (fImportType == IMPORT_FROM_REPOSITORY) {
				...
				if (namesOfNotImportedProjects.size() > 0) {
					UIJob job = new UIJob(MDEUIMessages.PluginImportOperation_WarningDialogJob) {

						public IStatus runInUIThread(IProgressMonitor monitor) {
							String dialogMessage = namesOfNotImportedProjects.size() == 1 ? MDEUIMessages.PluginImportOperation_WarningDialogMessageSingular : MDEUIMessages.PluginImportOperation_WarningDialogMessagePlural;
				...
				for (int i = 0; i < fModels.length; i++) {
					monitor.setTaskName(NLS.bind(MDEUIMessages.PluginImportOperation_Importing_plugin, fModels[i].getMonitorBase().getId()));
					try {
				...
				monitor.setTaskName(MDEUIMessages.PluginImportOperation_Set_up_classpaths);
				try {
	...
	private void deleteConflictingProjects(MultiStatus status, IProgressMonitor monitor) {
		...
		try {
			IMonitorModelBase[] workspacePlugins = MonitorRegistry.getWorkspaceModels();
			HashMap workspacePluginMap = new HashMap();
			for (int i = 0; i < workspacePlugins.length; i++) {
				IMonitorModelBase plugin = workspacePlugins[i];
			...
			if (conflictingPlugins.size() > 0) {
				...
				UIJob job = new UIJob(MDEUIMessages.PluginImportOperation_OverwritePluginProjects) {
				...
				for (int i = 0; i < overwriteProjectList.size(); i++) {
					IMonitorModelBase plugin = (IMonitorModelBase) overwriteProjectList.get(i);
					monitor.setTaskName(NLS.bind(MDEUIMessages.PluginImportOperation_Importing_plugin, plugin.getMonitorBase().getId()));
					IProject project = plugin.getUnderlyingResource().getProject();
					try {
						if (RepositoryProvider.isShared(project))
							RepositoryProvider.unmap(project);
						if (!safeDeleteCheck(project, monitor)) {
							status.add(new Status(IStatus.ERROR, MDEPlugin.getPluginId(), NLS.bind(MDEUIMessages.PluginImportOperation_could_not_delete_project, project.getName())));
						}
						project.delete(true, true, monitor);
					} catch (CoreException ex) {
						status.add(new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.OK, NLS.bind(MDEUIMessages.PluginImportOperation_could_not_delete_project, project.getName()), ex));
					}
				}
				monitor.worked(2);
			} else {
	 ...
	 */
	private void importPlugin(IMonitorModelBase model, int importType, IProgressMonitor monitor) throws CoreException {
		try {
			monitor.beginTask("", 5); //$NON-NLS-1$
	 ...
	 */
	private Map importAsBinary(IProject project, IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
		try {
			...
			RepositoryProvider.map(project, MDECore.BINARY_REPOSITORY_PROVIDER);
			project.setPersistentProperty(MDECore.EXTERNAL_PROJECT_PROPERTY, MDECore.BINARY_PROJECT_VALUE);
	 ...
	 * @throws CoreException if there is a problem completing the import
	 */
	private Map importAsBinaryWithLinks(IProject project, IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
		try {
	 ...
	 * @throws CoreException if there is a problem completing the import
	 */
	private void importAsSource(IProject project, IMonitorModelBase model, SubProgressMonitor monitor) throws CoreException {
		try {
			monitor.beginTask("", 4); //$NON-NLS-1$
			...
			IStatus status = new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.ERROR, e.getMessage(), e);
	 ...
	 */
	private IProject createProject(IMonitorModelBase model, IProgressMonitor monitor) throws TeamException, CoreException {
		try {
			monitor.beginTask("", 2); //$NON-NLS-1$
			IProject project = findProject(model.getMonitorBase().getId());
			if (project.exists() || new File(project.getParent().getLocation().toFile(), project.getName()).exists()) {

				project = MDEPlugin.getWorkspace().getRoot().getProject(model.getMonitorBase().getId());
				if (project.exists()) {
					...
					project = MDEPlugin.getWorkspace().getRoot().getProject(projectName);
					int index = 0;
					while (project.exists() == true) {
						index++;
						project = MDEPlugin.getWorkspace().getRoot().getProject(projectName + '_' + index);
					}
				}
			}
	...
	private IProject findProject(String id) {
		...
		return MDEPlugin.getWorkspace().getRoot().getProject(id);
	}
	...
	private boolean safeDeleteCheck(IProject project, IProgressMonitor monitor) {
		if (!fPluginsAreInUse)
			return true;
		IMonitorModelBase base = MonitorRegistry.findModel(project);
		if (base != null) {
			IMonitorLibrary[] libraries = base.getMonitorBase().getLibraries();
	 ...
	 */
	private boolean canFindSource(IMonitorModelBase model) {
	 ...
	 */
	private SourceLocationManager getSourceManager(IMonitorModelBase model) {
		// Check the alternate source manager first
		if (fAlternateSource != null) {
			// Check for a source bundle
			if (fAlternateSource.hasBundleManifestLocation(model.getMonitorBase())) {
			...
			for (int i = 0; i < libraries.length; i++) {
				...
				IPath srcPath = fAlternateSource.findSourcePath(model.getMonitorBase(), new Path(zipName));
				if (srcPath != null) {
					return fAlternateSource;
				}
			}
		...
		// Check for a source bundle
		if (manager.hasBundleManifestLocation(model.getMonitorBase())) {
			return manager;
		}
		...
		for (int i = 0; i < libraries.length; i++) {
			String zipName = ClasspathUtilCore.getSourceZipName(libraries[i]);
			IPath srcPath = manager.findSourcePath(model.getMonitorBase(), new Path(zipName));
			if (srcPath != null) {
				return manager;
			}
		}
	 ...
	 */
	private boolean isExempt(IMonitorModelBase model, int importType) {
	...
	private void runBuildJob() {
		Job buildJob = new Job(MDEUIMessages.CompilersConfigurationBlock_building) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					MDEPlugin.getWorkspace().build(IncrementalProjectBuilder.INCREMENTAL_BUILD, monitor);
				} catch (CoreException e) {
				}
				return Status.OK_STATUS;
			}
	 ...
	 */
	private Map importSourceArchives(IProject project, IMonitorModelBase model, int mode, IProgressMonitor monitor) throws CoreException {
		String[] libraries = getLibraryNames(model);
		try {
			monitor.beginTask(MDEUIMessages.ImportWizard_operation_importingSource, libraries.length);
	 ...
	 */
	private boolean extractSourceFolders(IProject project, IMonitorModelBase model, WorkspaceBuildModel buildModel, Map packageLocations, IProgressMonitor monitor) throws CoreException {
		try {
			String[] libraries = getLibraryNames(model);
			monitor.beginTask(MDEUIMessages.ImportWizard_operation_importingSource, libraries.length);
			...
			if (manager != null) {
				...
				if (manager.hasBundleManifestLocation(model.getMonitorBase())) {
					File srcFile = manager.findSourcePlugin(model.getMonitorBase());
					Set sourceRoots = manager.findSourceRoots(model.getMonitorBase());
					for (int i = 0; i < libraries.length; i++) {
	 ...
	 * @throws IOException
	 */
	private boolean handleInternalSource(IMonitorModelBase model, WorkspaceBuildModel buildModel, Map packageLocations) throws CoreException, ZipException, IOException {
		IImportStructureProvider provider;
		Object root;
		IPath prefixPath;
	 ...
	 */
	private void importAdditionalSourceFiles(IProject project, IMonitorModelBase model, SubProgressMonitor monitor) throws CoreException {
		SourceLocationManager manager = getSourceManager(model);
		if (manager != null) {
			File sourceLocation = manager.findSourcePlugin(model.getMonitorBase());
	 ...
	 */
	private void importRequiredPluginFiles(IProject project, IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
		if (isJARd(model)) {
			ZipFile zip = null;
			try {
				zip = new ZipFile(new File(model.getInstallLocation()));
	...
	private void configureBinIncludes(WorkspaceBuildModel buildModel, IMonitorModelBase model, IProject project) throws CoreException {
	 ...
	 * @param base
	 */
	private void modifyBundleClasspathHeader(IProject project, IMonitorModelBase base) {
	...
	private boolean needsJavaNature(IProject project, IMonitorModelBase model) {
		// If there are class libraries we need a java nature
		if (model.getMonitorBase().getLibraries().length > 0)
			return true;
	...
	private void setProjectNatures(IProject project, IMonitorModelBase model) throws CoreException {
		IProjectDescription desc = project.getDescription();
		if (!desc.hasNature(MDE.PLUGIN_NATURE)) {
	 ...
	 */
	private String[] getLibraryNames(IMonitorModelBase model) {
	 ...
	 */
	private boolean isJARd(IMonitorModelBase model) {
