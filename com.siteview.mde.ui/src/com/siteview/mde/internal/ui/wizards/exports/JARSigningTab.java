...
public class JARSigningTab {
	...
	public Control createControl(Composite parent) {
		...
		fButton.setText(MDEUIMessages.AdvancedPluginExportPage_signButton);
		...
		fKeystoreLabel = createLabel(comp, MDEUIMessages.AdvancedPluginExportPage_keystore);
		fKeystoreText = createText(comp, 1);

		fBrowseButton = new Button(comp, SWT.PUSH);
		fBrowseButton.setText(MDEUIMessages.ExportWizard_browse);
		...
		fKeypassLabel = createLabel(comp, MDEUIMessages.JARSigningTab_keypass);
		fKeypassText = createText(comp, 2);
		fKeypassText.setEchoChar('*');

		fAliasLabel = createLabel(comp, MDEUIMessages.AdvancedPluginExportPage_alias);
		fAliasText = createText(comp, 2);

		fPasswordLabel = createLabel(comp, MDEUIMessages.AdvancedPluginExportPage_password);
	...
	protected String validate() {
		...
		if (fButton.getSelection()) {
			if (fKeystoreText.getText().trim().length() == 0) {
				error = MDEUIMessages.AdvancedPluginExportPage_noKeystore;
			} else if (fAliasText.getText().trim().length() == 0) {
				error = MDEUIMessages.AdvancedPluginExportPage_noAlias;
			} else if (fPasswordText.getText().trim().length() == 0) {
				error = MDEUIMessages.AdvancedPluginExportPage_noPassword;
			}
		}
		return error;
	}
