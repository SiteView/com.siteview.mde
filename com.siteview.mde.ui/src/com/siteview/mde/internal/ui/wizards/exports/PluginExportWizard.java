...
import com.siteview.mde.internal.core.exports.MonitorExportOperation;
import com.siteview.mde.internal.ui.MDEPluginImages;
import com.siteview.mde.internal.ui.MDEUIMessages;
import com.siteview.mde.internal.ui.build.RuntimeInstallJob;
...
public class PluginExportWizard extends AntGeneratingExportWizard {
	...
	public PluginExportWizard() {
		setDefaultPageImageDescriptor(MDEPluginImages.DESC_PLUGIN_EXPORT_WIZ);
	}
	...
	protected void scheduleExportJob() {
		...
		final MonitorExportOperation job = new MonitorExportOperation(info, MDEUIMessages.PluginExportJob_name);
		job.setUser(true);
		job.setRule(ResourcesPlugin.getWorkspace().getRoot());
		job.setProperty(IProgressConstants.ICON_PROPERTY, MDEPluginImages.DESC_PLUGIN_OBJ);
		job.addJobChangeListener(new JobChangeAdapter() {
			public void done(IJobChangeEvent event) {
				if (job.hasAntErrors()) {
					...
					RuntimeInstallJob installJob = new RuntimeInstallJob(MDEUIMessages.PluginExportWizard_InstallJobName, info);
					installJob.setUser(true);
					installJob.setProperty(IProgressConstants.ICON_PROPERTY, MDEPluginImages.DESC_FEATURE_OBJ);
					installJob.schedule();
				}
			}
		});
	...
	private String getPluginIDs() {
		...
		for (int i = 0; i < objects.length; i++) {
			...
			if (object instanceof IMonitorModelBase) {
				buffer.append(((IMonitorModelBase) object).getMonitorBase().getId());
