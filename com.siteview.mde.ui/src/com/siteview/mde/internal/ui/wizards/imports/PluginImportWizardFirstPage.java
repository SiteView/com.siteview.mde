...
import com.siteview.mde.internal.ui.preferences.TargetPlatformPreferenceNode;
...
public class PluginImportWizardFirstPage extends WizardPage {
	...
	private IMonitorModelBase[] models = new IMonitorModelBase[0];
	/**
	...
	public PluginImportWizardFirstPage(String name) {
		super(name);
		setTitle(MDEUIMessages.ImportWizard_FirstPage_title);
		setMessage(MDEUIMessages.ImportWizard_FirstPage_desc);
	...
	private void createImportChoicesGroup(Composite container) {
		Group importChoices = SWTFactory.createGroup(container, MDEUIMessages.ImportWizard_FirstPage_importGroup, 1, 1, GridData.FILL_HORIZONTAL);
		scanButton = SWTFactory.createRadioButton(importChoices, MDEUIMessages.ImportWizard_FirstPage_scanAll);
		importButton = SWTFactory.createRadioButton(importChoices, MDEUIMessages.ImportWizard_FirstPage_importPrereqs);
	}
	...
	private void createImportOptionsGroup(Composite container) {
		Group options = SWTFactory.createGroup(container, MDEUIMessages.ImportWizard_FirstPage_importAs, 1, 1, GridData.FILL_HORIZONTAL);
		binaryButton = SWTFactory.createRadioButton(options, MDEUIMessages.ImportWizard_FirstPage_binary);
		binaryWithLinksButton = SWTFactory.createRadioButton(options, MDEUIMessages.ImportWizard_FirstPage_binaryLinks);
		sourceButton = SWTFactory.createRadioButton(options, MDEUIMessages.ImportWizard_FirstPage_source);
		repositoryButton = SWTFactory.createRadioButton(options, MDEUIMessages.PluginImportWizardFirstPage_3);
	}
	...
	private void createImportFromGroup(Composite parent) {
		Group composite = SWTFactory.createGroup(parent, MDEUIMessages.ImportWizard_FirstPage_importFrom, 3, 1, GridData.FILL_HORIZONTAL);

		importActiveTargetButton = SWTFactory.createRadioButton(composite, MDEUIMessages.ImportWizard_FirstPage_target, 1);
		...
		openTargetPrefsLink.setText(MDEUIMessages.ImportWizard_FirstPage_goToTarget);
		...
		importTargetDefinitionButton = SWTFactory.createRadioButton(composite, MDEUIMessages.PluginImportWizardFirstPage_0, 1);
		...
		importDirectoryButton = SWTFactory.createRadioButton(composite, MDEUIMessages.ImportWizard_FirstPage_otherFolder, 1);
		...
		browseButton = SWTFactory.createPushButton(composite, MDEUIMessages.ImportWizard_FirstPage_browse, null, GridData.HORIZONTAL_ALIGN_FILL);
	...
	private IPath chooseDropLocation() {
		...
		dialog.setText(MDEUIMessages.ImportWizard_messages_folder_title);
		dialog.setMessage(MDEUIMessages.ImportWizard_messages_folder_message);
	...
	private void validateDropLocation() {
		if (importTargetDefinitionButton.getSelection() && targetDefinitionCombo.getText().length() == 0) {
			...
			setErrorMessage(MDEUIMessages.PluginImportWizardFirstPage_2);
			return;
		}
		if (importDirectoryButton.getSelection()) {
			...
			if (curr.segmentCount() == 0 && curr.getDevice() == null) {
				setErrorMessage(MDEUIMessages.ImportWizard_errors_locationMissing);
				setPageComplete(false);
				return;
			}
			if (!Path.ROOT.isValidPath(importDirectory.getText())) {
				setErrorMessage(MDEUIMessages.ImportWizard_errors_buildFolderInvalid);
				setPageComplete(false);
				return;
			}

			if (!curr.toFile().isDirectory()) {
				setErrorMessage(MDEUIMessages.ImportWizard_errors_buildFolderMissing);
				setPageComplete(false);
				return;
			}
		}
	...
	private void resolveTargetPlatform(final int type) {
		IRunnableWithProgress op = new IRunnableWithProgress() {
			public void run(IProgressMonitor monitor) throws InvocationTargetException {
				models = MonitorRegistry.getExternalModels();
				state = MDECore.getDefault().getModelManager().getState();
	...
	private void resolveTargetDefinition(final ITargetDefinition target, final int type) {
		IRunnableWithProgress op = new IRunnableWithProgress() {
			public void run(IProgressMonitor monitor) throws InvocationTargetException {
				monitor.beginTask(MDEUIMessages.PluginImportWizardFirstPage_1, 100);
				...
				alternateSource = new AlternateSourceLocations((IMonitorModelBase[]) sourceModels.toArray(new IMonitorModelBase[sourceModels.size()]), (IResolvedBundle[]) sourceBundles.toArray(new IResolvedBundle[sourceBundles.size()]));
				try {
	...
	/**
	 * @return the complete set of {@link IMonitorModelBase}s for the given drop location
	 */
	public IMonitorModelBase[] getModels() {
	 ...
	 */
	public void configureBundleImportPages(IMonitorModelBase[] models) {
		...
		while (iterator.hasNext()) {
			...
			ScmUrlImportDescription[] descriptions = (ScmUrlImportDescription[]) entry.getValue();
			for (int i = 0; i < descriptions.length; i++) {
				IMonitorModelBase model = (IMonitorModelBase) descriptions[i].getProperty(BundleProjectService.PLUGIN);
