...
public class OverwriteProjectsSelectionDialog extends SelectionStatusDialog {
	...
	private class PluginContentProvider implements ITreeContentProvider {
		...
		public Object[] getElements(Object inputElement) {
			return (IMonitorModelBase[]) inputElement;
		}
	...
	private class ButtonSelectionListener extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			...
			if (MDEUIMessages.DuplicatePluginResolutionDialog_selectAll.equals(buttonID)) {
				fCheckboxTreeViewer.setAllChecked(true);
			} else if (MDEUIMessages.DuplicatePluginResolutionDialog_deselectAll.equals(buttonID)) {
	...
	public OverwriteProjectsSelectionDialog(Shell parent, ArrayList plugins) {
		super(parent);
		setTitle(MDEUIMessages.PluginImportOperation_OverwritePluginProjects);
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (fPluginProjectList != null && fPluginProjectList.size() == 1) {
			setMessage(MDEUIMessages.DuplicatePluginResolutionDialog_messageSingular);
		} else {
			setMessage(MDEUIMessages.DuplicatePluginResolutionDialog_message);
		}
		SWTFactory.createWrapLabel(tableComposite, getMessage(), 1, 400);
		SWTFactory.createVerticalSpacer(tableComposite, 1);
		SWTFactory.createLabel(tableComposite, MDEUIMessages.OverwriteProjectsSelectionDialog_0, 1);
		...
		Button buttonSelectAll = SWTFactory.createPushButton(buttonComposite, MDEUIMessages.DuplicatePluginResolutionDialog_selectAll, null);
		Button buttonDeselectAll = SWTFactory.createPushButton(buttonComposite, MDEUIMessages.DuplicatePluginResolutionDialog_deselectAll, null);
		buttonSelectAll.addSelectionListener(new ButtonSelectionListener());
		buttonSelectAll.setData(ID, MDEUIMessages.DuplicatePluginResolutionDialog_selectAll);
		buttonDeselectAll.addSelectionListener(new ButtonSelectionListener());
		buttonDeselectAll.setData(ID, MDEUIMessages.DuplicatePluginResolutionDialog_deselectAll);
	...
	private void createTableArea(Composite parent) {
		...
		fCheckboxTreeViewer.setInput(fPluginProjectList.toArray(new IMonitorModelBase[fPluginProjectList.size()]));
