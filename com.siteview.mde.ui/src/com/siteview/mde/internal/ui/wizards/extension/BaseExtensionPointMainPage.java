...
import com.siteview.mde.internal.core.ischema.IDocumentSection;
...
public abstract class BaseExtensionPointMainPage extends WizardPage {
	...
	private InputStream createSchemaStream(String pluginId, String pointId, String name, boolean shared) {
		...
		schema.setDescription(MDEUIMessages.BaseExtensionPoint_sections_overview);
		DocumentSection section;
		section = new DocumentSection(schema, IDocumentSection.SINCE, MDEUIMessages.BaseExtensionPointMainPage_since);
		section.setDescription(MDEUIMessages.BaseExtensionPoint_sections_since);
		...
		section.setDescription(MDEUIMessages.BaseExtensionPoint_sections_usage);
		schema.addDocumentSection(section);
		section = new DocumentSection(schema, IDocumentSection.API_INFO, "API Information"); //$NON-NLS-1$
		section.setDescription(MDEUIMessages.BaseExtensionPoint_sections_api);
		schema.addDocumentSection(section);
		section = new DocumentSection(schema, IDocumentSection.IMPLEMENTATION, "Supplied Implementation"); //$NON-NLS-1$
		section.setDescription(MDEUIMessages.BaseExtensionPoint_sections_supplied);
		schema.addDocumentSection(section);
		section = new DocumentSection(schema, IDocumentSection.COPYRIGHT, "Copyright"); //$NON-NLS-1$
		section.setDescription(MDEUIMessages.BaseExtensionPoint_sections_copyright);
		schema.addDocumentSection(section);
	...
	private IFile generateSchemaFile(String pluginId, String id, String name, boolean shared, String schema, IProgressMonitor monitor) throws CoreException {
		...
		monitor.subTask(MDEUIMessages.BaseExtensionPoint_generating);
		...
		IDE.setDefaultEditor(schemaFile, IMDEUIConstants.SCHEMA_EDITOR_ID);
		return schemaFile;
	}
	...
	protected String validateExtensionPointID() {
		...
		if (id.length() == 0)
			return MDEUIMessages.BaseExtensionPointMainPage_missingExtensionPointID;
		...
		if (model == null) {
			return NLS.bind(MDEUIMessages.BaseExtensionPointMainPage_errorMsgPluginNotFound, pluginID);
		}
	...
	protected String validateExtensionPointName() {
		...
		if (fNameText.getText().length() == 0)
			return MDEUIMessages.BaseExtensionPointMainPage_missingExtensionPointName;
	...
	protected String validateExtensionPointSchema() {
		...
		if (fSchemaText.getText().length() == 0)
			return MDEUIMessages.BaseExtensionPointMainPage_missingExtensionPointSchema;
	...
	private void handlePluginBrowse() {
		...
		if (dialog.open() == Window.OK) {
			IMonitorModelBase workspaceModelBase = (IMonitorModelBase) dialog.getFirstResult();
			fPluginIdText.setText(workspaceModelBase.getMonitorBase().getId());
		}
	}

	private void handleSchemaLocation() {
		ElementTreeSelectionDialog dialog = new ElementTreeSelectionDialog(getShell(), new WorkbenchLabelProvider(), new WorkbenchContentProvider());
		dialog.setTitle(MDEUIMessages.BaseExtensionPointMainPage_schemaLocation_title);
		dialog.setMessage(MDEUIMessages.BaseExtensionPointMainPage_schemaLocation_desc);
	...
	private String getWorkspaceRelativePath(String path) {
		String workspacePath = MDECore.getWorkspace().getRoot().getLocation().toString();
