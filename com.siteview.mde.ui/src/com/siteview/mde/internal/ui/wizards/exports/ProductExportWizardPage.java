...
public class ProductExportWizardPage extends AbstractExportWizardPage {
	...
	public ProductExportWizardPage(IStructuredSelection selection) {
		...
		setTitle(MDEUIMessages.ProductExportWizardPage_title);
		setDescription(MDEUIMessages.ProductExportWizardPage_desc);
	}
	...
	private void createSynchronizationSection(Composite parent) {
		...
		group.setText(MDEUIMessages.ProductExportWizardPage_sync);

		Label label = new Label(group, SWT.WRAP);
		label.setText(MDEUIMessages.ProductExportWizardPage_syncText);
		GridData gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.widthHint = 400;
		label.setLayoutData(gd);

		fSyncButton = new Button(group, SWT.CHECK);
		fSyncButton.setText(MDEUIMessages.ProductExportWizardPage_syncButton);
		gd = new GridData();
	...
	private void createOptionsSection(Composite container) {
		...
		group.setText(MDEUIMessages.ProductExportWizardPage_exportOptionsGroup);
		...
		fExportSourceButton.setText(MDEUIMessages.ExportWizard_includeSource);
		...
		fExportMetadata.setText(MDEUIMessages.ExportWizard_includesMetadata);

		if (getWizard().getPages().length > 1) {
			fMultiPlatform = new Button(group, SWT.CHECK);
			fMultiPlatform.setText(MDEUIMessages.ExportWizard_multi_platform);
		...
		fAllowBinaryCycles.setText(MDEUIMessages.ExportOptionsTab_allowBinaryCycles);
	}
	...
	protected void initialize() {
		...
		fExportSourceCombo.setItems(new String[] {MDEUIMessages.ExportWizard_generateAssociatedSourceBundles, MDEUIMessages.ExportWizard_includeSourceInBinaryBundles});
		String sourceComboValue = settings.get(S_EXPORT_SOURCE_FORMAT) != null ? settings.get(S_EXPORT_SOURCE_FORMAT) : MDEUIMessages.ExportWizard_generateAssociatedSourceBundles;
	...
	protected boolean doExportSourceBundles() {
		return MDEUIMessages.ExportWizard_generateAssociatedSourceBundles.equals(fExportSourceCombo.getText());
	}
