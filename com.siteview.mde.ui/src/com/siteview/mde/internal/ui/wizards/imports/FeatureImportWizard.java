...
import com.siteview.mde.internal.ui.wizards.imports.FeatureImportOperation.IReplaceQuery;
...
public class FeatureImportWizard extends Wizard implements IImportWizard {
	...
	public FeatureImportWizard() {
		...
		setDefaultPageImageDescriptor(MDEPluginImages.DESC_FEATURE_IMPORT_WIZ);
		setWindowTitle(MDEUIMessages.FeatureImportWizard_title);
	}
	...
	public boolean performFinish() {
		...
		Job job = new Job(MDEUIMessages.FeatureImportWizard_title) {
			protected IStatus run(IProgressMonitor monitor) {
				try {
					MDEPlugin.getWorkspace().run(op, monitor);
	...
	private IPath computeTargetPath() {
		IPath pluginsLocation = MDEPlugin.getWorkspace().getRoot().getLocation();
	...
	private static class ReplaceDialog extends MessageDialog {
		public ReplaceDialog(Shell parentShell, String dialogMessage) {
			super(parentShell, MDEUIMessages.FeatureImportWizard_messages_title, null, dialogMessage, MessageDialog.QUESTION, new String[] {IDialogConstants.YES_LABEL, IDialogConstants.YES_TO_ALL_LABEL, IDialogConstants.NO_LABEL, MDEUIMessages.FeatureImportWizard_noToAll, IDialogConstants.CANCEL_LABEL}, 0);
		}
	}
	...
	public static class ReplaceQuery implements IReplaceQuery {
		...
		public int doQuery(IProject project) {
			...
			final String message = NLS.bind(MDEUIMessages.FeatureImportWizard_messages_exists, project.getName());
