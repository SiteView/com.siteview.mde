...
import com.siteview.mde.internal.core.target.IUBundleContainer;
...
public class ExportTargetJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
		try {
			...
			monitor.beginTask(MDEUIMessages.ExportTargetDefinition_task, totalWork);

			monitor.subTask(MDEUIMessages.ExportTargetJob_ConfiguringDestination);
			setupDestination(monitor);

			monitor.subTask(MDEUIMessages.ExportTargetJob_ExportingTargetContents);
			...
			return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), "Failed to export the target", e); //$NON-NLS-1$ 
		} finally {
			monitor.done();
		}
	...
	private void setupDestination(IProgressMonitor monitor) throws CoreException {
		...
		if (!fileSystem.canWrite()) {
			throw new CoreException(new Status(IStatus.ERROR, MDEPlugin.getPluginId(), "Destination directory not writable.")); //$NON-NLS-1$ 
		}
		...
		if (fclearDestinationDirectory) {
			monitor.subTask(MDEUIMessages.ExportTargetDeleteOldData); //Deleting old data...
	...
	private void exportContainer(IBundleContainer container, ITargetDefinition target, IFileStore featureDir, IFileStore pluginDir, IFileSystem fileSystem, IProgressMonitor monitor) throws CoreException {
		...
		if (features != null) {
			monitor.subTask(MDEUIMessages.ExportTargetExportFeatures);
		...
		if (bundles != null) {
			monitor.subTask(MDEUIMessages.ExportTargetExportPlugins);
