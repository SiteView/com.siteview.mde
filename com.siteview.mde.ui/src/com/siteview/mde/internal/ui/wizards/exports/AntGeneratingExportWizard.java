...
public abstract class AntGeneratingExportWizard extends BaseExportWizard {
	...
	protected boolean performPreliminaryChecks() {
		...
		if (!MessageDialogWithToggle.ALWAYS.equals(MDEPlugin.getDefault().getPreferenceStore().getString(IPreferenceConstants.OVERWRITE_BUILD_FILES_ON_EXPORT))) {
	...
	private void setDefaultValues(File dir, String buildFilename) {
		try {
			IContainer container = MDEPlugin.getWorkspace().getRoot().getContainerForLocation(new Path(dir.toString()));
	...
	protected class AntErrorDialog extends MessageDialog {
		...
		public AntErrorDialog(File logLocation) {
			super(PlatformUI.getWorkbench().getDisplay().getActiveShell(), MDECoreMessages.FeatureBasedExportOperation_ProblemDuringExport, null, null, MessageDialog.ERROR, new String[] {IDialogConstants.OK_LABEL}, 0);
		...
		protected Control createMessageArea(Composite composite) {
			...
			try {
				link.setText(NLS.bind(MDEUIMessages.PluginExportWizard_Ant_errors_during_export_logs_generated, "<a>" + fLogLocation.getCanonicalPath() + "</a>")); //$NON-NLS-1$ //$NON-NLS-2$
			} catch (IOException e) {
				MDEPlugin.log(e);
			}
