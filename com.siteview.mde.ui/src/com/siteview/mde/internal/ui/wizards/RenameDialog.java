...
public class RenameDialog extends SelectionStatusDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		label.setText(MDEUIMessages.RenameDialog_label);
	...
	public int open() {
		...
		status = new Status(IStatus.OK, MDEPlugin.getPluginId(), IStatus.OK, "", //$NON-NLS-1$
				null);
		updateStatus(status);
	...
	private void textChanged(String text) {
		...
		if (fValidator != null) {
			...
			if (message != null) {
				status = new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.ERROR, message, null);
				updateStatus(status);
				okButton.setEnabled(false);
				return;
			}
		...
		for (int i = 0; i < oldNames.size(); i++) {
			if ((isCaseSensitive && text.equals(oldNames.get(i))) || (!isCaseSensitive && text.equalsIgnoreCase(oldNames.get(i).toString()))) {
				status = new Status(IStatus.ERROR, MDEPlugin.getPluginId(), IStatus.ERROR, MDEUIMessages.RenameDialog_validationError, null);
				updateStatus(status);
				okButton.setEnabled(false);
				break;
			}
