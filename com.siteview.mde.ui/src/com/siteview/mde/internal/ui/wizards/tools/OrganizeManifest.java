...
import com.siteview.mde.internal.core.text.monitor.FragmentModel;
...
public class OrganizeManifest implements IOrganizeManifestsSettings {
	...
	public static void organizeRequireBundles(IBundle bundle, boolean removeImports) {
		...
		if (header != null) {
			...
			for (int i = 0; i < bundles.length; i++) {
				...
				if (MonitorRegistry.findModel(pluginId) == null) {
					if (removeImports)
	...
	public static Change deleteUselessPluginFile(IProject project, IMonitorModelBase modelBase) {
		if (modelBase == null)
			return null;
	...
	public static TextFileChange[] removeUnusedKeys(final IProject project, final IBundle bundle, final IMonitorModelBase modelBase) {
	...
	private static void findTranslatedStrings(IProject project, IMonitorModelBase pluginModel, IBundle bundle, ArrayList list) {
		...
		if (hostSpec != null) {
			...
			for (int i = 0; i < hosts.length; i++) {
				IMonitorModelBase hostModel = MonitorRegistry.findModel(hosts[i]);
				if (hostModel != null) {
			...
			IFragmentModel[] fragmentModels = MDEManager.findFragmentsFor(model);
	...
	private static IMonitorModelBase getTextModel(IMonitorModelBase model, boolean fragment) {
		if (model instanceof MonitorModel || model instanceof FragmentModel)
			return model;
		...
		if (model != null) {
			...
			IMonitorModelBase returnModel;
			if (fragment)
				returnModel = new FragmentModel(doc, false);
			else
				returnModel = new MonitorModel(doc, false);
			try {
				returnModel.load();
	...
	private static IBundle getTextBundle(IMonitorModelBase model) {
		if (model != null) {
	...
	private static void findTranslatedXMLStrings(IMonitorModelBase model, ArrayList list) {
		if (model == null)
			return;
	...
	private static void inspectElementForTranslation(IDocumentElementNode parent, ArrayList list) {
		...
		IMonitorObject[] children = ((IMonitorParent) parent).getChildren();
	 ...
	 * @param model - 
	 */
	public static void prefixIconPaths(IMonitorModelBase model) {
		if (model == null)
			return;
	...
	private static void inspectElementsIconPaths(ISchema schema, IMonitorParent parent) {
