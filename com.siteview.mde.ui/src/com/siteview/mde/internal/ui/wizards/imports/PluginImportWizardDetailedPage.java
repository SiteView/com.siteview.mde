...
import com.siteview.mde.internal.core.monitor.AbstractMonitorModelBase;
...
public class PluginImportWizardDetailedPage extends BaseImportWizardSecondPage {
	...
	private class AvailableFilter extends ViewerFilter {
		...
		public boolean select(Viewer viewer, Object parentElement, Object element) {
			...
			if (fSelected.contains(element))
				return false;
			if (!(element instanceof AbstractMonitorModelBase))
				return false;
			String itemID = ((AbstractMonitorModelBase) element).getMonitorBase().getId();
	...
	private class VersionFilter extends ViewerFilter {
		...
		public void setModel(IMonitorModelBase[] plugins) {
	...
	public PluginImportWizardDetailedPage(String pageName, PluginImportWizardFirstPage firstPage) {
		...
		setTitle(MDEUIMessages.ImportWizard_DetailedPage_title);
		setMessage(MDEUIMessages.ImportWizard_DetailedPage_desc);
	...
	private void createFilterOptions(Composite container) {
		fFilterOldVersionButton = SWTFactory.createCheckButton(container, MDEUIMessages.ImportWizard_DetailedPage_filterDesc, null, true, 1);
	...
	private Composite createAvailableList(Composite parent) {
		...
		label.setText(MDEUIMessages.ImportWizard_DetailedPage_availableList);
		...
		fAvailableListViewer.setInput(MDECore.getDefault().getModelManager());
	...
	private Composite createButtonArea(Composite parent) {
		...
		button.setText(MDEUIMessages.ImportWizard_DetailedPage_existing);
		...
		button.setText(MDEUIMessages.ImportWizard_DetailedPage_existingUnshared);
		button.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		button.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
		...
		button = new Button(container, SWT.PUSH);
		button.setText(MDEUIMessages.ImportWizard_DetailedPage_swap);
		button.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		button.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				handleSwap();
		...
		fAddRequiredButton.setText(MDEUIMessages.ImportWizard_DetailedPage_addRequired);
	...
	private Composite createScrollArea(Composite parent) {
		...
		gd.horizontalSpan = 3;
		container.setLayoutData(gd);
		container.setText(MDEUIMessages.ImportWizard_DetailedPage_filter);

		Label filterLabel = new Label(container, SWT.NONE);
		filterLabel.setText(MDEUIMessages.ImportWizard_DetailedPage_search);
	...
	private void updateCount() {
		...
		fCountLabel.setText(NLS.bind(MDEUIMessages.ImportWizard_DetailedPage_count, (new String[] {Integer.toString(fImportListViewer.getTable().getItemCount()), Integer.toString(fCountTotal)})));
	...
	private void handleExistingProjects() {
		...
		for (int i = 0; i < fModels.length; i++) {
			String id = fModels[i].getMonitorBase().getId();
			IProject project = (IProject) MDEPlugin.getWorkspace().getRoot().findMember(id);
	...
	private void handleAddRequiredPlugins() {
		...
		if (items.length == 0)
			return;
		if (items.length == 1) {
			IMonitorModelBase model = (IMonitorModelBase) items[0].getData();
			if (model.getMonitorBase().getId().equals("org.eclipse.core.boot")) { //$NON-NLS-1$
				return;
			}
		}
		...
		ArrayList result = new ArrayList();
		for (int i = 0; i < items.length; i++) {
			addPluginAndDependencies((IMonitorModelBase) items[i].getData(), result, fAddFragmentsButton.getSelection());
		}
		handleSetImportSelection(result);
	}
