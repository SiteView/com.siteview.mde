...
import com.siteview.mde.internal.core.product.SplashInfo;
...
public class ProductDefinitionOperation extends BaseManifestOperation {
	...
	private void updateSplashProgress(IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
		// Sanity checks
		if (fProject == null) {
			return;
		} else if (model == null) {
	...
	private void updateSplashHandler(IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
	...
	private void updateSplashHandlerFiles(IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
	...
	private void updateSplashHandlerModel(IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
		// Get the splash handler type
		String splashHandlerType = getSplashHandlerType();
	...
	private void runRemoveSplashAction(IMonitorModelBase model, IProgressMonitor monitor) throws CoreException {
	...
	private void runUpdateSplashAction(IMonitorModelBase model, IProgressMonitor monitor, String splashHandlerType) throws CoreException {
	...
	private IMonitorExtension createExtension(IMonitorModelBase model) throws CoreException {
	...
	private IMonitorElement createExtensionContent(IMonitorExtension extension) throws CoreException {
		...
		IMonitorElement child = createElement(element, IProductConstants.WINDOW_IMAGES, getWindowImagesString());
		if (child != null)
			element.add(child);
	...
	private IMonitorElement createElement(IMonitorElement parent, String name, String value) throws CoreException {
		IMonitorElement element = null;
	...
	private String getURL(String location) {
		...
		IProject project = MDEPlugin.getWorkspace().getRoot().getProject(projectName);
		if (project.exists()) {
	...
	private void modifyExistingFile(IFile file, IProgressMonitor monitor) throws CoreException {
		IStatus status = MDEPlugin.getWorkspace().validateEdit(new IFile[] {file}, getShell());
		...
		ModelModification mod = new ModelModification(file) {
			protected void modifyModel(IBaseModel model, IProgressMonitor monitor) throws CoreException {
				if (!(model instanceof IMonitorModelBase))
					return;
				IMonitorExtension extension = findProductExtension((IMonitorModelBase) model);
				if (extension == null)
					insertNewExtension((IMonitorModelBase) model);
				else
					modifyExistingExtension(extension);
				// Update the splash handler.  Update plug-in model and copy files
				updateSplashHandler((IMonitorModelBase) model, monitor);
				// Update splash progress.  Update plug-in model and copy files
				updateSplashProgress((IMonitorModelBase) model, monitor);
			}
		};
		PDEModelUtility.modifyModel(mod, monitor);
	}

	private IMonitorExtension findProductExtension(IMonitorModelBase model) {
	...
	private void insertNewExtension(IMonitorModelBase model) throws CoreException {
		IMonitorExtension extension = createExtension(model);
		model.getMonitorBase().add(extension);
	}

	private void modifyExistingExtension(IMonitorExtension extension) throws CoreException {
		if (extension.getChildCount() == 0) {
			insertNewProductElement(extension);
			return;
		}

		MonitorElementNode element = (MonitorElementNode) extension.getChildren()[0];
	...
	private void synchronizeChild(IMonitorElement element, String propertyName, String value) throws CoreException {
		IMonitorElement child = null;
		IMonitorObject[] children = element.getChildren();
		for (int i = 0; i < children.length; i++) {
			IMonitorElement candidate = (IMonitorElement) children[i];
			if (candidate.getName().equals("property")) { //$NON-NLS-1$
				IMonitorAttribute attr = candidate.getAttribute("name"); //$NON-NLS-1$
	...
	private void insertNewProductElement(IMonitorExtension extension) throws CoreException {
		IMonitorElement element = createExtensionContent(extension);
