...
import com.siteview.mde.internal.ui.wizards.product.UpdateSplashHandlerAction;
...
public class TemplateFileGenerator implements IVariableProvider {
	...
	// TODO: MP: SPLASH: Merge this utility back with template (maybe as an abstract base class?) - Extracted from com.siteview.mde.ui.templates.AbstractTemplateSection
	 ...
	 * @param model
	 */
	public TemplateFileGenerator(IProject project, IMonitorModelBase model, String pluginID, String targetPackage, String targetClass, String template) {
		fProject = project;
		fModel = model;
		fPluginID = pluginID;
	...
	public void generateFiles(IProgressMonitor monitor) throws CoreException {
		...
		Bundle templateBundle = Platform.getBundle("com.siteview.mde.ui.templates"); //$NON-NLS-1$
	...
	public URL getTemplateLocation() {
		Bundle bundle = Platform.getBundle("com.siteview.mde.ui.templates"); //$NON-NLS-1$
		if (bundle == null) {
			return null;
		}
	...
	private IFolder generateJavaSourceFolder(IFolder sourceFolder, IProgressMonitor monitor) throws CoreException {
		...
		if (packageName == null)
			packageName = fModel.getMonitorBase().getId();
	...
	private String getKeyValue(String key) {
		...
		if (key.equals(KEY_PLUGIN_CLASS) && fModel instanceof IMonitorModel) {
			IMonitor plugin = (IMonitor) fModel.getMonitorBase();
			return plugin.getClassName();
		}

		// TODO: MP: SPLASH: Broken and not needed, underlying model does not have class, id, translated name parameters defined
		if (key.equals(KEY_ACTIVATOR_SIMPLE) && fModel instanceof IMonitorModel) {
			IMonitor plugin = (IMonitor) fModel.getMonitorBase();
		...
		if (key.equals(KEY_PACKAGE_NAME) && fModel instanceof IMonitorModel) {
			return fPackage;
			// Old implementation, does not work
