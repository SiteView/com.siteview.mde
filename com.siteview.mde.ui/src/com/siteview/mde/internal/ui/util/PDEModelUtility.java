...
import com.siteview.mde.internal.core.bundle.BundleFragmentModel;
import com.siteview.mde.internal.core.bundle.BundlePluginModel;
...
import com.siteview.mde.internal.core.text.bundle.BundleTextChangeListener;
...
import com.siteview.mde.internal.ui.editor.schema.SchemaInputContext;
...
public class PDEModelUtility {
	 ...
	 */
	public static void connect(MDEFormEditor editor) {
	 ...
	 */
	public static void disconnect(MDEFormEditor editor) {
		IProject project = editor.getCommonProject();
		if (project == null) {
	...
	public static ManifestEditor getOpenManifestEditor(IProject project) {
		return (ManifestEditor) getOpenEditor(project, IMDEUIConstants.MANIFEST_EDITOR_ID);
	}
	...
	public static BuildEditor getOpenBuildPropertiesEditor(IProject project) {
		return (BuildEditor) getOpenEditor(project, IMDEUIConstants.BUILD_EDITOR_ID);
	}
	...
	public static SiteEditor getOpenUpdateSiteEditor(IProject project) {
		return (SiteEditor) getOpenEditor(project, IMDEUIConstants.SITE_EDITOR_ID);
	}

	private static MDEFormEditor getOpenEditor(IProject project, String editorId) {
		ArrayList list = (ArrayList) fOpenPDEEditors.get(project);
		if (list == null)
			return null;
		for (int i = 0; i < list.size(); i++) {
			MDEFormEditor editor = (MDEFormEditor) list.get(i);
	...
	public static SchemaEditor getOpenSchemaEditor(IFile file) {
		return (SchemaEditor) getOpenEditor(IMDEUIConstants.SCHEMA_EDITOR_ID, SchemaInputContext.CONTEXT_ID, file);
	}

	private static MDEFormEditor getOpenEditor(String editorID, String inputContextID, IFile file) {
	 ...
	 */
	public static IMonitorModelBase getActivePluginModel() {
		IEditorPart editor = MDEPlugin.getActivePage().getActiveEditor();
		if (editor instanceof ManifestEditor) {
			...
			if (model instanceof IMonitorModelBase)
				return (IMonitorModelBase) model;
		}
		return null;
	}
	...
	public static IEditingModel getOpenModel(IDocument doc) {
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i < list.size(); i++) {
				MDEFormEditor e = (MDEFormEditor) list.get(i);
				IMonitorModelBase model = (IMonitorModelBase) e.getAggregateModel();
...
//					return (IEditingModel)bModel;
	...
	public static void modifyModel(final ModelModification modification, final IProgressMonitor monitor) {
		...
		MDEFormEditor editor = getOpenEditor(modification);
	...
	public static TextFileChange[] changesForModelModication(final ModelModification modification, final IProgressMonitor monitor) {
		final MDEFormEditor editor = getOpenEditor(modification);
		if (editor != null) {
	...
	private static void modifyEditorModel(final ModelModification mod, final MDEFormEditor editor, final IBaseModel model, final IProgressMonitor monitor) {
	...
	private static MDEFormEditor getOpenEditor(ModelModification modification) {
		...
		if (name.equals(ICoreConstants.PLUGIN_FILENAME_DESCRIPTOR) || name.equals(ICoreConstants.FRAGMENT_FILENAME_DESCRIPTOR) || name.equals(ICoreConstants.MANIFEST_FILENAME)) {
			...
			MDEFormEditor openEditor = getOpenBuildPropertiesEditor(project);
			if (openEditor == null)
	...
	private static IBaseModel getModelFromEditor(MDEFormEditor openEditor, ModelModification modification) {
		if (openEditor == null)
			return null;
		String name = modification.getFile().getName();
	...
	private static AbstractEditingModel prepareAbstractEditingModel(IFile file, IDocument doc, boolean generateEditNames) {
		...
		if (filename.equals(ICoreConstants.MANIFEST_FILENAME))
			...
			model = new MonitorModel(doc, true);
		else if (filename.endsWith(F_PROPERTIES))
