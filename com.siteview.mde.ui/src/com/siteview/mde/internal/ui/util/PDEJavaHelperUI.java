...
import com.siteview.mde.internal.ui.editor.contentassist.*;
import com.siteview.mde.internal.ui.editor.contentassist.display.JavaDocCommentReader;
...
public class PDEJavaHelperUI {
	...
	public static String selectType(IResource resource, int scope) {
		...
		try {
			SelectionDialog dialog = JavaUI.createTypeDialog(MDEPlugin.getActiveWorkbenchShell(), PlatformUI.getWorkbench().getProgressService(), PDEJavaHelper.getSearchScope(project), scope, false, ""); //$NON-NLS-1$
	...
	public static String selectType(IResource resource, int scope, String filter, String superTypeName) {
		...
		try {
			...
			SelectionDialog dialog = JavaUI.createTypeDialog(MDEPlugin.getActiveWorkbenchShell(), PlatformUI.getWorkbench().getProgressService(), searchScope, scope, false, filter);
			dialog.setTitle(MDEUIMessages.ClassAttributeRow_dialogTitle);
			if (dialog.open() == Window.OK) {
				IType type = (IType) dialog.getResult()[0];
				return type.getFullyQualifiedName('$');
	...
	public static TypeFieldAssistDisposer addTypeFieldAssistToText(Text text, IProject project, int searchScope) {
		...
		if (activeBindings.length == 0)
			...
			description = NLS.bind(MDEUIMessages.PDEJavaHelper_msgContentAssistAvailableWithKeyBinding, activeBindings[0].format());
