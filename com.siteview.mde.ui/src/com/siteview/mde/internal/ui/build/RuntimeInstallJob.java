...
public class RuntimeInstallJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
		try {
			...
			monitor.beginTask(MDEUIMessages.RuntimeInstallJob_Job_name_installing, 12 + (2 * fInfo.items.length));
			...
			if (profileRegistry == null) {
				return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), MDEUIMessages.RuntimeInstallJob_ErrorCouldntOpenProfile);
			}
			...
			for (int i = 0; i < fInfo.items.length; i++) {
				...
				monitor.subTask(NLS.bind(MDEUIMessages.RuntimeInstallJob_Creating_installable_unit, fInfo.items[i].toString()));

				//Get the installable unit from the repo
				String id = null;
				String version = null;
				if (fInfo.items[i] instanceof IMonitorModelBase) {
					id = ((IMonitorModelBase) fInfo.items[i]).getMonitorBase().getId();
					version = ((IMonitorModelBase) fInfo.items[i]).getMonitorBase().getVersion();
				...
				if (id == null && version == null) {
					return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), NLS.bind(MDEUIMessages.RuntimeInstallJob_ErrorCouldNotGetIdOrVersion, fInfo.items[i].toString()));
				}
				...
				if (queryMatches.isEmpty()) {
					return new Status(IStatus.ERROR, MDEPlugin.getPluginId(), NLS.bind(MDEUIMessages.RuntimeInstallJob_ErrorCouldNotFindUnitInRepo, new String[] {id, version}));
				}
	...
	private IInstallableUnitPatch createInstallableUnitPatch(IInstallableUnit existingIU, Version newVersion, IProfile profile, IProgressMonitor monitor) {
		...
		iuPatchDescription.setProperty(IInstallableUnit.PROP_NAME, NLS.bind(MDEUIMessages.RuntimeInstallJob_installPatchName, id));
		iuPatchDescription.setProperty(IInstallableUnit.PROP_DESCRIPTION, MDEUIMessages.RuntimeInstallJob_installPatchDescription);
