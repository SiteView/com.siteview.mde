...
public class ExternalizeStringsWizardPage extends UserInputWizardPage {
	...
	static {
		...
		TABLE_COLUMNS[VALUE] = MDEUIMessages.ExternalizeStringsWizardPage_value;
		TABLE_COLUMNS[KEY] = MDEUIMessages.ExternalizeStringsWizardPage_subKey;
	}
	...
	protected ExternalizeStringsWizardPage(ModelChangeTable changeTable) {
		super(PAGE_NAME);
		setTitle(MDEUIMessages.ExternalizeStringsWizardPage_pageTitle);
		setDescription(MDEUIMessages.ExternalizeStringsWizardPage_pageDescription);
		...
		fModifyListener = new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				...
				if (StringHelper.isValidLocalization(localization)) {
					...
					setErrorMessage(MDEUIMessages.ExternalizeStringsWizardPage_badLocalizationError);
				}
			}
		};
	...
	private void createInputContents(Composite composite) {
		...
		label.setText(MDEUIMessages.ExternalizeStringsWizardPage_resourcelabel);
		...
		fSelectAll.setText(MDEUIMessages.ExternalizeStringsWizardPage_selectAllButton);
		...
		fDeselectAll.setText(MDEUIMessages.ExternalizeStringsWizardPage_deselectAllButton);
		...
		project.setText(MDEUIMessages.ExternalizeStringsWizardPage_projectLabel);
		fProjectLabel = new Label(infoComposite, SWT.NONE);
		gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.horizontalIndent = 10;
		fProjectLabel.setLayoutData(gd);
		fProjectLabel.setText(MDEUIMessages.ExternalizeStringsWizardPage_noUnderlyingResource);

		Label properties = new Label(infoComposite, SWT.NONE);
		properties.setText(MDEUIMessages.ExternalizeStringsWizardPage_localizationLabel);
		fLocalizationText = new Text(infoComposite, SWT.BORDER);
		gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.horizontalIndent = 10;
		fLocalizationText.setLayoutData(gd);
		fLocalizationText.setText(MDEUIMessages.ExternalizeStringsWizardPage_noUnderlyingResource);
		fLocalizationText.addModifyListener(fModifyListener);

		fInputViewer.setInput(MDEPlugin.getDefault());
	}
	...
	private void updateSourceViewer(ITextFileBufferManager manager, IFile sourceFile) {
		...
		IMonitorModelBase model = ((ModelChange) item.getParentItem().getData()).getParentModel();
	...
	private void updatePropertiesLabel(IMonitorModelBase model) {
	...
	private void validateKey(String key, ModelChangeElement element) {
		...
		if (key.equals(fPreErrorKey)) {
			...
			error = getErrorMessage(MDEUIMessages.ExternalizeStringsWizardPage_keyEmptyError, oldKey);
		} else if (key.charAt(0) == '#' || key.charAt(0) == '!' || key.charAt(0) == '%') {
			error = getErrorMessage(MDEUIMessages.ExternalizeStringsWizardPage_keyCommentError, oldKey);
		} else if (key.indexOf(':') != -1 || key.indexOf('=') != -1 || key.indexOf(' ') != -1) {
			error = getErrorMessage(MDEUIMessages.ExternalizeStringsWizardPage_keyError, oldKey);
		} else if ((!key.equals(oldKey) || fPreErrorKey != null) && properties.containsKey(key)) {
			error = getErrorMessage(MDEUIMessages.ExternalizeStringsWizardPage_keyDuplicateError, oldKey);
		}
	...
	private String getErrorMessage(String error, String suggestion) {
		...
		if (suggestion != null) {
			sb.append(MDEUIMessages.ExternalizeStringsWizardPage_keySuggested);
