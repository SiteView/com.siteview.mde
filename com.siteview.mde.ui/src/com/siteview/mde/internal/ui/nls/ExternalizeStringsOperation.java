...
public class ExternalizeStringsOperation extends WorkspaceModifyOperation {
	...
	protected void execute(IProgressMonitor monitor) throws CoreException, InvocationTargetException, InterruptedException {
		for (int i = 0; i < fChangeFiles.length; i++) {
			if (fChangeFiles[i] instanceof ModelChangeFile) {
				...
				CompositeChange pluginChange = getChangeForPlugin(changeFile.getModel().getParentModel().getMonitorBase().getId());
				...
				if (!pFile.exists() && !fFileChanges.containsKey(pFile)) {
					...
					fileChange.addTextEditGroup(new TextEditGroup(MDEUIMessages.ExternalizeStringsOperation_editNames_addComment, edit));
				}
	...
	private CompositeChange getChangeForPlugin(String pluginName) {
		...
		CompositeChange result = new CompositeChange(NLS.bind(MDEUIMessages.ExternalizeStringsOperation_pluginChangeName, pluginName));
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (changeElement.isExternalized()) {
					...
					uChange.addTextEditGroup(new TextEditGroup(NLS.bind(MDEUIMessages.ExternalizeStringsOperation_editNames_replaceText, changeElement.getKey()), uEdit));
					InsertEdit pEdit = getPropertiesInsertEdit(pDoc, changeElement);
					pChange.getEdit().addChild(pEdit);
					pChange.addTextEditGroup(new TextEditGroup(NLS.bind(MDEUIMessages.ExternalizeStringsOperation_editNames_insertProperty, changeElement.getKey()), pEdit));
				}
			}
		} catch (MalformedTreeException e) {
		}
	...
	private void addBundleLocalization(ModelChange change, IProgressMonitor mon, CompositeChange parent) {
		IMonitorModelBase base = change.getParentModel();
