...
public class NLSFragmentGenerator {
				...
				public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					...
					try {
						...
						Display.getDefault().syncExec(new Runnable() {
							public void run() {
								MDEPlugin.logException(ex, ex.getMessage(), MDEUIMessages.InternationalizeWizard_NLSFragmentGenerator_errorMessage);
							}
						});
					}
				}
		...
		for (Iterator iter = plugins.iterator(); iter.hasNext();) {
			...
			for (Iterator it = locales.iterator(); it.hasNext();) {
				...
				if (project.exists() && !overwrites.containsKey(project.getName())) {
					boolean overwrite = MessageDialog.openConfirm(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.InternationalizeWizard_NLSFragmentGenerator_overwriteTitle, NLS.bind(MDEUIMessages.InternationalizeWizard_NLSFragmentGenerator_overwriteMessage, pluginName(plugin, locale)));
	 ...
	 * @throws InterruptedException 
	 */
	private void createNLFragment(final IMonitorModelBase plugin, final IProject project, final Locale locale) throws CoreException, InvocationTargetException, InterruptedException {
	...
	private String pluginName(IMonitorModelBase plugin, Locale locale) {
		return template.replaceAll(quote(PLUGIN_NAME_MACRO), plugin.getMonitorBase().getId()).replaceAll(quote(LOCALE_NAME_MACRO), locale.toString());
	}
	 ...
	 * @throws IOException 
	 */
	private void createLocaleSpecificPropertiesFile(final IProject fragmentProject, IMonitorModelBase plugin, final Locale locale) throws CoreException, IOException {
