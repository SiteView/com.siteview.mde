...
import com.siteview.mde.core.IModelProviderListener;
...
public class InternationalizeWizardPluginPage extends InternationalizationWizardPage implements IModelProviderListener {
	...
	protected IMonitorModelBase[] fModels = new IMonitorModelBase[0];
	...
	public InternationalizeWizardPluginPage(InternationalizeModelTable modelTable, String pageName) {
		...
		super(pageName);
		setTitle(MDEUIMessages.InternationalizeWizard_PluginPage_pageTitle);
		setDescription(MDEUIMessages.InternationalizeWizard_PluginPage_pageDescription);

		MDEPlugin.getDefault().getLabelProvider().connect(this);
		MDECore.getDefault().getModelManager().getExternalModelManager().addModelProviderListener(this);
	...
	private void addFilter() {
		fFilter = new AvailableFilter(fSelected, MDEPlugin.getDefault().getLabelProvider());
	...
	private Composite createAvailableList(Composite parent) {
		Composite container = createViewerContainer(parent, MDEUIMessages.InternationalizeWizard_PluginPage_availableList);
		fAvailableViewer = createTableViewer(container, new ContentProvider(), MDECore.getDefault().getModelManager());
		return container;
	}

	protected Composite createInternationalizeList(Composite parent) {
		Composite container = createViewerContainer(parent, MDEUIMessages.InternationalizeWizard_PluginPage_internationalizeList);
		fSelectedViewer = createTableViewer(container, new SelectedContentProvider(), MDECore.getDefault().getModelManager().getExternalModelManager());
		return container;
	}
	...
	private IMonitorModelBase findModel(String id) {
		for (int i = 0; i < fModels.length; i++) {
			String modelId = fModels[i].getMonitorBase().getId();
	...
	private IFragmentModel[] findFragments(IMonitor plugin) {
	...
	protected void addPluginAndDependencies(IMonitorModelBase model, ArrayList selected, boolean addFragments) {

		boolean containsVariable = false;
		if (!selected.contains(model)) {
			selected.add(model);
			boolean hasextensibleAPI = ClasspathUtilCore.hasExtensibleAPI(model);
			if (!addFragments && !hasextensibleAPI && model instanceof IMonitorModel) {
	...
	protected void addDependencies(IMonitorModelBase model, ArrayList selected, boolean addFragments) {

		IMonitorImport[] required = model.getMonitorBase().getImports();
		if (required.length > 0) {
			for (int i = 0; i < required.length; i++) {
				IMonitorModelBase found = findModel(required[i].getId());
				if (found != null) {
		...
		if (addFragments) {
			if (model instanceof IMonitorModel) {
				IFragmentModel[] fragments = findFragments(((IMonitorModel) model).getMonitor());
				for (int i = 0; i < fragments.length; i++) {
					addPluginAndDependencies(fragments[i], selected, addFragments);
				}
			} else {
				IFragment fragment = ((IFragmentModel) model).getFragment();
				IMonitorModelBase found = findModel(fragment.getPluginId());
				if (found != null) {
					addPluginAndDependencies(found, selected, addFragments);
				}
			}
	...
	private Composite createButtonArea(Composite parent) {
		...
		fAddButton.setText(MDEUIMessages.ImportWizard_DetailedPage_add);
		...
		fAddAllButton.setText(MDEUIMessages.ImportWizard_DetailedPage_addAll);
		...
		fRemoveButton.setText(MDEUIMessages.ImportWizard_DetailedPage_remove);
		...
		fRemoveAllButton.setText(MDEUIMessages.ImportWizard_DetailedPage_removeAll);
	...
	private Composite createScrollArea(Composite parent) {
		Group container = createFilterContainer(parent, MDEUIMessages.InternationalizeWizard_PluginPage_filter, MDEUIMessages.ImportWizard_DetailedPage_search);
	...
	protected void pageChanged(boolean doAddEnablement, boolean doRemoveEnablement) {
		if (fTemplateText.getText().length() == 0) {
			setErrorMessage(MDEUIMessages.InternationalizeWizard_PluginPage_templateError);
		} else if (fSelectedViewer.getTable().getItemCount() == 0) {
			setErrorMessage(MDEUIMessages.InternationalizeWizard_PluginPage_selectionError);
		} else {
	...
	private void updateCount() {
		fCountLabel.setText(NLS.bind(MDEUIMessages.ImportWizard_DetailedPage_count, (new String[] {new Integer(fSelectedViewer.getTable().getItemCount()).toString(), new Integer(fAvailableViewer.getTable().getItemCount() + fSelectedViewer.getTable().getItemCount()).toString()})));
	...
	public void dispose() {
		...
		MDECore.getDefault().getModelManager().getExternalModelManager().removeModelProviderListener(this);
	}
