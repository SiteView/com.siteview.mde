		...
		try {
			...
			if (!changeTable.isEmpty()) {
				...
				if (!fSkipMessageDialog)
					MessageDialog.openInformation(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), MDEUIMessages.GetNonExternalizedStringsAction_allExternalizedTitle, MDEUIMessages.GetNonExternalizedStringsAction_allExternalizedMessage);
			}
		}
	}
