...
public class MonitorContentMergeViewer extends TextMergeViewer {

	private IColorManager fColorManager;
	private ArrayList fPropertyChangedListeners;

	public MonitorContentMergeViewer(Composite parent, CompareConfiguration config) {
		super(parent, config);
	}
	...
	protected void configureTextViewer(final TextViewer textViewer) {
		if (textViewer instanceof SourceViewer) {
			...
			MDEPlugin.getDefault().getPreferenceStore().addPropertyChangeListener(propertyChangedListener);
			...
			Font font = JFaceResources.getFont(MonitorContentMergeViewer.class.getName());
			if (font != null)
	...
	public String getTitle() {
		return MDEUIMessages.PluginContentMergeViewer_title;
	}
	...
	protected void handleDispose(DisposeEvent event) {
		...
		if (fPropertyChangedListeners != null) {
			...
			while (iter.hasNext())
				MDEPlugin.getDefault().getPreferenceStore().removePropertyChangeListener((IPropertyChangeListener) iter.next());
