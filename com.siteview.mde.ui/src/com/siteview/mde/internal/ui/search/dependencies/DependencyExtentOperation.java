...
public class DependencyExtentOperation {
	...
	public void execute(IProgressMonitor monitor) {
		IMonitorModelBase[] plugins = PluginJavaSearchUtil.getPluginImports(fImportID);
		monitor.beginTask(MDEUIMessages.DependencyExtentOperation_searching + " " + fImportID + "...", 10); //$NON-NLS-1$//$NON-NLS-2$ 
	...
	private void checkForJavaDependencies(IMonitorModelBase[] models, IProgressMonitor monitor) {
		try {
			...
			for (int i = 0; i < packageFragments.length; i++) {
				...
				monitor.subTask(MDEUIMessages.DependencyExtentOperation_inspecting + " " + pkgFragment.getElementName()); //$NON-NLS-1$ 
