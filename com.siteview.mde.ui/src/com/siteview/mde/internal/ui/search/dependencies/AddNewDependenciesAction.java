...
public class AddNewDependenciesAction extends Action {
	...
	public void run() {
		Job job = new WorkspaceJob(MDEUIMessages.DependencyManagementSection_jobName) {
			...
			public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
				try {
					...
					if (!op.foundNewDependencies())
						Display.getDefault().asyncExec(new Runnable() {
							public void run() {
								MessageDialog.openInformation(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.AddNewDependenciesAction_title, MDEUIMessages.AddNewDependenciesAction_notFound);
							}
						});
