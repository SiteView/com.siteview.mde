...
public class SearchResult extends AbstractTextSearchResult implements IEditorMatchAdapter {
	...
	public String getLabel() {
		...
		return fQuery.getLabel() + " - " + numMatches + " " + (numMatches == 1 ? MDEUIMessages.SearchResult_match : MDEUIMessages.SearchResult_matches); //$NON-NLS-1$ //$NON-NLS-2$  
	}
	...
	public ImageDescriptor getImageDescriptor() {
		return MDEPluginImages.DESC_PSEARCH_OBJ;
	}
	...
	public boolean isShownInEditor(Match match, IEditorPart editor) {
		...
		if (element instanceof IMonitorObject)
			return isMatchContained(editor, (IMonitorObject) element);
		return false;
	}
	...
	public Match[] computeContainedMatches(AbstractTextSearchResult result, IEditorPart editor) {
		...
		for (int i = 0; i < objects.length; i++) {
			if (objects[i] instanceof IMonitorObject) {
				IMonitorObject object = (IMonitorObject) objects[i];
	...
	protected boolean isMatchContained(IEditorPart editor, IMonitorObject object) {
