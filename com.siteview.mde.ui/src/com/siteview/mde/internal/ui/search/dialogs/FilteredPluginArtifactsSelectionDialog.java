 ...
 *******************************************************************************/
package com.siteview.mde.internal.ui.search.dialogs;
...
public class FilteredPluginArtifactsSelectionDialog extends FilteredItemsSelectionDialog {
	...
	private static final String DIALOG_SETTINGS = "com.siteview.mde.ui.dialogs.FilteredPluginArtifactsSelectionDialog"; //$NON-NLS-1$
	...
	private class DetailedLabelProvider extends LabelProvider {
		...
		public Image getImage(Object element) {
			if (element instanceof IMonitorModelBase) {
				...
				IMonitorModelBase base = getModel(id, version);
	...
	private class ExtensionsAction extends Action {
		...
		public ExtensionsAction() {
			super(MDEUIMessages.FilteredPluginArtifactsSelectionDialog_showExtensions, IAction.AS_CHECK_BOX);
			setChecked(true);
		}
	...
	private class ExtensionPointsAction extends Action {
		...
		public ExtensionPointsAction() {
			super(MDEUIMessages.FilteredPluginArtifactsSelectionDialog_showExtensionPoints, IAction.AS_CHECK_BOX);
			setChecked(true);
		}
	...
	private class ExportedPackagesAction extends Action {
		...
		public ExportedPackagesAction() {
			super(MDEUIMessages.FilteredPluginArtifactsSelectionDialog_showExportedPackages, IAction.AS_CHECK_BOX);
			setChecked(true);
		}
	...
	private class FeaturesAction extends Action {
		...
		public FeaturesAction() {
			super(MDEUIMessages.FilteredPluginArtifactsSelectionDialog_showFeatures, IAction.AS_CHECK_BOX);
			setChecked(true);
		}
	...
	public FilteredPluginArtifactsSelectionDialog(Shell shell) {
		...
		setTitle(MDEUIMessages.FilteredPluginArtifactsSelectionDialog_title);
		setMessage(MDEUIMessages.FilteredPluginArtifactsSelectionDialog_message);
	...
	protected void fillContentProvider(AbstractContentProvider contentProvider, ItemsFilter itemsFilter, IProgressMonitor progressMonitor) throws CoreException {
		...
		IMonitorModelBase[] models = manager.getActiveModels(true);

		FeatureModelManager fManager = MDECore.getDefault().getFeatureModelManager();
		IFeatureModel[] fModels = fManager.getModels();

		progressMonitor.beginTask(MDEUIMessages.FilteredPluginArtifactsSelectionDialog_searching, models.length + fModels.length);
	...
	protected IStatus validateItem(Object item) {
		return new Status(IStatus.OK, "com.siteview.mde.ui", 0, "", null); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	private class PluginSearchSelectionHistory extends SelectionHistory {
		...
		protected Object restoreItemFromMemento(IMemento memento) {
			...
			switch (type) {
				case TYPE_PLUGIN :
					IMonitorModelBase model = getModel(memento);
					if (model == null)
						return null;
					return model;
		...
		protected IMonitorModelBase getModel(IMemento memento) {
	...
	private class PluginSearchComparator implements Comparator {
		...
		private int compareSimilarObjects(Object o1, Object o2) {
			if (o1 instanceof IMonitorModelBase && o2 instanceof IMonitorModelBase) {
				...
				IMonitorExtension ipe2 = (IMonitorExtension) o2;
				int comparePointsResult = ipe1.getPoint().compareTo(ipe2.getPoint());
				if (comparePointsResult == 0)
					return comparePlugins(ipe1.getMonitorBase(), ipe2.getMonitorBase());
				// else
	...
	private IMonitorModelBase getModel(String id, String version) {
		ModelEntry entry = MonitorRegistry.findEntry(id);
		IMonitorModelBase[] models = entry.getActiveModels();
		for (int i = 0; i < models.length; i++) {
			IMonitorModelBase model = models[i];
