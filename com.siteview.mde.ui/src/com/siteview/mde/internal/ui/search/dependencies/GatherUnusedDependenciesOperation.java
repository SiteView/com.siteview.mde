...
public class GatherUnusedDependenciesOperation implements IRunnableWithProgress {
	...
	public GatherUnusedDependenciesOperation(IMonitorModelBase model) {
		fModel = model;
	}
	...
	public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
		...
		IMonitorImport[] imports = fModel.getMonitorBase().getImports();
	...
	private void updateMonitor(IProgressMonitor monitor, int size) {
		monitor.setTaskName(MDEUIMessages.UnusedDependencies_analyze + size + " " //$NON-NLS-1$
				+ MDEUIMessages.UnusedDependencies_unused + " " //$NON-NLS-1$
				+ (size == 1 ? MDEUIMessages.DependencyExtent_singular : MDEUIMessages.DependencyExtent_plural) + " " //$NON-NLS-1$
				+ MDEUIMessages.DependencyExtent_found);
	}

	private boolean isUnused(IMonitorImport plugin, SubProgressMonitor monitor) {
		IMonitorModelBase[] models = PluginJavaSearchUtil.getPluginImports(plugin);
	...
	public static void removeDependencies(IMonitorModelBase model, Object[] elements) {
		ImportPackageHeader pkgHeader = null;
		for (int i = 0; i < elements.length; i++) {
			if (elements[i] instanceof IMonitorImport)
				try {
					model.getMonitorBase().remove((IMonitorImport) elements[i]);
				} catch (CoreException e) {
				}
	...
	private void minimizeDependencies(HashMap usedPlugins, ArrayList usedPackages, IProgressMonitor monitor) {
		...
		while (!(plugins.isEmpty())) {
			...
			IMonitorModelBase base = MonitorRegistry.findModel(pluginId);
			if (base == null)
				continue;
