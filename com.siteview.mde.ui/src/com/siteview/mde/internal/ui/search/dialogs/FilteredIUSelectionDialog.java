...
import com.siteview.mde.internal.core.target.Messages;
...
//	private static final int TYPE_PACKAGE = 2;
	...
	private class IUWrapperLabelProvider extends LabelProvider implements DelegatingStyledCellLabelProvider.IStyledLabelProvider {
		...
		private MDELabelProvider labelProvider;

		public IUWrapperLabelProvider() {
			labelProvider = MDEPlugin.getDefault().getLabelProvider();
		...
		public Image getImage(Object element) {
			if (element instanceof IUPackage) {
				...
				if (QueryUtil.isGroup(iu))
					return labelProvider.get(MDEPluginImages.DESC_FEATURE_OBJ);
				return labelProvider.get(MDEPluginImages.DESC_PLUGIN_OBJ);
			}
			return null;
		}
	...
	public FilteredIUSelectionDialog(Shell shell, IQuery query) {
		super(shell, true);
		this.query = query;
		setTitle(MDEUIMessages.FilteredIUSelectionDialog_title);
		setMessage(MDEUIMessages.FilteredIUSelectionDialog_message);
	...
	protected Control createExtendedContentArea(Composite parent) {
		...
		fShowLatestVersionOnlyButton.setText(MDEUIMessages.FilteredIUSelectionDialog_showLatestVersionOnly);
