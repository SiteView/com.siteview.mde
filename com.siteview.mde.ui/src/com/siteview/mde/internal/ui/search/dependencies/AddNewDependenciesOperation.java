...
public class AddNewDependenciesOperation extends WorkspaceModifyOperation {
	...
	protected void execute(IProgressMonitor monitor) throws CoreException, InvocationTargetException, InterruptedException {
		...
		boolean useRequireBundle = new ProjectScope(fProject).getNode(MDECore.PLUGIN_ID).getBoolean(ICoreConstants.RESOLVE_WITH_REQUIRE_BUNDLE, true);
		...
		for (int j = 0; j < secDeps.length; j++) {
			try {
				...
				if (base != null) {
					...
					subMonitor.beginTask(NLS.bind(MDEUIMessages.AddNewDependenciesOperation_searchForDependency, pluginId), exported.length);
