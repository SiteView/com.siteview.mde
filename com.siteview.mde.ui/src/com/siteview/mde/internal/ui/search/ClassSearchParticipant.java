...
public class ClassSearchParticipant implements IQueryParticipant {
		...
		ModelModification mod = new ModelModification(project) {
			protected void modifyModel(IBaseModel model, IProgressMonitor monitor) throws CoreException {
				...
				if (model instanceof IMonitorModelBase) {
					IFile file = (IFile) ((IMonitorModelBase) model).getUnderlyingResource();
					SchemaRegistry registry = MDECore.getDefault().getSchemaRegistry();
					IMonitorBase pbase = ((IMonitorModelBase) model).getMonitorBase();
					IMonitorExtension[] extensions = pbase.getExtensions();
	...
	private void inspectExtension(ISchema schema, IMonitorParent parent, IFile file) {
		IMonitorObject[] children = parent.getChildren();

		if (parent instanceof MonitorElementNode && parent != null && parent.getParent() instanceof MonitorElementNode) {
			// check if this node corresponds to a Java type attribute which would have been defined has an element
			MonitorElementNode node = (MonitorElementNode) parent;
			MonitorElementNode parentNode = (MonitorElementNode) parent.getParent();
	...
	private void checkMatch(IMonitorAttribute attr, IFile file) {
		String value = null;
