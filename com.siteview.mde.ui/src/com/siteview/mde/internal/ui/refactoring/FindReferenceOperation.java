...
public class FindReferenceOperation implements IWorkspaceRunnable {
		...
		for (int i = 0; i < dependents.length; i++) {
			...
			for (int j = 0; j < requires.length; j++) {
				if (requires[j].getName().equals(oldId)) {
					CreateHeaderChangeOperation op = new CreateHeaderChangeOperation(MonitorRegistry.findModel(dependents[i]), Constants.REQUIRE_BUNDLE, oldId, fNewId);
	...
	private void findFragmentReferences(ArrayList changes, IProgressMonitor monitor) throws CoreException {
		...
		for (int i = 0; i < fragments.length; i++) {
			IMonitorModelBase base = MonitorRegistry.findModel(fragments[i]);
			...
			if (friends != null)
				for (int j = 0; j < friends.length; j++) {
					if (friends[j].equals(id)) {
						CreateHeaderChangeOperation op = new CreateHeaderChangeOperation(MonitorRegistry.findModel(pkgs[i].getExporter()), Constants.EXPORT_PACKAGE, id, fNewId);
						op.run(new SubProgressMonitor(monitor, 1));
						TextFileChange change = op.getChange();
						if (change != null)
							changes.add(change);
