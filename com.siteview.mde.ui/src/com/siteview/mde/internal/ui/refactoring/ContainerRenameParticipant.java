...
public class ContainerRenameParticipant extends PDERenameParticipant {
	...
	public String getName() {
		return MDEUIMessages.ContainerRenameParticipant_renameFolders;
	}
	...
	protected Change createManifestChange(IProgressMonitor monitor) throws CoreException {
		...
		if (manifest.exists()) {
			...
			try {
				...
				CompositeChange result = new CompositeChange(MDEUIMessages.ContainerRenameParticipant_renameBundleId);
				...
				if (bundle != null) {
					...
					if (header != null) {
						...
						FindReferenceOperation op = new FindReferenceOperation(MonitorRegistry.findModel(fProject).getBundleDescription(), newId);
