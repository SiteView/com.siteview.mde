...
public class RenamePluginProcessor extends RefactoringProcessor {
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor pm, CheckConditionsContext context) throws CoreException, OperationCanceledException {
		...
		if (res == null)
			...
			status.addFatalError(MDEUIMessages.RenamePluginProcessor_noManifestError);
		if (fInfo.isRenameProject()) {
			...
			if (newProject.exists() && !(res.getProject().equals(newProject)))
				status.addFatalError(MessageFormat.format(MDEUIMessages.RenameProjectChange_destinationExists, new String[] {newName}));
		}
		return status;
	}
	...
	public String getProcessorName() {
		return MDEUIMessages.RenamePluginProcessor_processorName;
	}
	...
	public RefactoringParticipant[] loadParticipants(RefactoringStatus status, SharableParticipants sharedParticipants) throws CoreException {
		if (fInfo.isRenameProject()) {
			...
			IParticipantDescriptorFilter filter = new IParticipantDescriptorFilter() {
				static final String PDE_CONTAINER_RENAME_PARTICIPANT = "com.siteview.mde.ui.manifestFolderRenameParticipant"; //$NON-NLS-1$
	...
	public Change createChange(IProgressMonitor pm) throws CoreException, OperationCanceledException {
		CompositeChange change = new CompositeChange(MessageFormat.format(MDEUIMessages.RenamePluginProcessor_changeTitle, new String[] {fInfo.getCurrentValue(), fInfo.getNewValue()}));
	...
	protected Change createProjectChange(IProgressMonitor monitor) {
		...
		if (project.getName().equals(newName))
			return null;
		descriptor.setDescription(MessageFormat.format(MDEUIMessages.RenamePluginProcessor_renameProjectDesc, new String[] {project.getName(), newName}));
	...
	protected Change[] createReferenceChanges(IProgressMonitor monitor) throws CoreException {
		IMonitorModelBase currentBase = fInfo.getBase();
		BundleDescription desc = currentBase.getBundleDescription();
		if (desc == null) {
			IMonitorModelBase savedBase = MonitorRegistry.findModel(currentBase.getUnderlyingResource().getProject());
