...
public class MessageLine extends CLabel {
	...
	private Image findImage(IStatus status) {
		...
		if (status.isOK()) {
			...
			return provider.get(MDEPluginImages.DESC_ERROR_ST_OBJ);
		} else if (status.matches(IStatus.WARNING)) {
			return provider.get(MDEPluginImages.DESC_WARNING_ST_OBJ);
		} else if (status.matches(IStatus.INFO)) {
			return provider.get(MDEPluginImages.DESC_INFO_ST_OBJ);
		}
		return null;
	}
