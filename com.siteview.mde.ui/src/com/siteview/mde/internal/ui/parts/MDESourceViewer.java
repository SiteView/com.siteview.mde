...
public class MDESourceViewer {
	...
	public MDESourceViewer(MDEFormPage page) {
	...
	public void createUIListeners() {
		...
		fViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				fPage.getMDEEditor().setSelection(event.getSelection());
			}
		});
		// Create focus listener
		fViewer.getTextWidget().addFocusListener(new FocusAdapter() {
			public void focusGained(FocusEvent e) {
				fPage.getMDEEditor().getContributor().updateSelectableActions(null);
			}
		});
	}
	...
	private void configureUIStyledText(GridData data, StyledText styledText) {
		// Configure the underlying styled text widget
		styledText.setMenu(fPage.getMDEEditor().getContextMenu());
		// Force borders
