...
import com.siteview.mde.internal.ui.wizards.WizardElement;
import com.siteview.mde.ui.IProvisionerWizard;
...
public class AddBundleContainerSelectionPage extends WizardSelectionPage {
	...
	private static ITargetPlatformService getTargetPlatformService() throws CoreException {
		if (fTargetService == null) {
			fTargetService = (ITargetPlatformService) MDECore.getDefault().acquireService(ITargetPlatformService.class.getName());
			if (fTargetService == null) {
				throw new CoreException(new Status(IStatus.ERROR, MDECore.PLUGIN_ID, Messages.AddDirectoryContainerPage_9));
			}
		}
	...
	private List getStandardChoices() {
		...
		standardChoices.add(new AbstractBundleContainerNode(Messages.AddBundleContainerSelectionPage_6, Messages.AddBundleContainerSelectionPage_7, MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_PRODUCT_DEFINITION)) {
			public IWizard createWizard() {
				Wizard wizard = new Wizard() {
		...
		// Feature Containers
		standardChoices.add(new AbstractBundleContainerNode(Messages.AddBundleContainerSelectionPage_9, Messages.AddBundleContainerSelectionPage_10, MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_FEATURE_OBJ)) {
			public IWizard createWizard() {
				Wizard wizard = new Wizard() {
					public void addPages() {
						IDialogSettings settings = MDEPlugin.getDefault().getDialogSettings().getSection(SETTINGS_SECTION);
		...
		standardChoices.add(new AbstractBundleContainerNode(Messages.AddBundleContainerSelectionPage_8, Messages.AddBundleContainerSelectionPage_11, MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_REPOSITORY_OBJ)) {
			public IWizard createWizard() {
				Wizard wizard = new Wizard() {
					public void addPages() {
						IDialogSettings settings = MDEPlugin.getDefault().getDialogSettings().getSection(SETTINGS_SECTION);
	...
	private List getProvisionerExtensionChoices() {
		...
		IExtensionPoint point = registry.getExtensionPoint(MDEPlugin.getPluginId(), PROVISIONER_POINT);
		if (point == null)
			return list;
	...
	protected WizardElement createWizardElement(IConfigurationElement config) {
		...
		if (imageName != null) {
			...
			image = MDEPlugin.getDefault().getLabelProvider().getImageFromPlugin(pluginID, imageName);
		}
						...
						if (fWizard != null) {
							...
							for (int i = 0; i < dirs.length; i++) {
								if (dirs[i] == null || !dirs[i].isDirectory()) {
									ErrorDialog.openError(getShell(), Messages.AddBundleContainerSelectionPage_0, Messages.AddBundleContainerSelectionPage_5, new Status(IStatus.ERROR, MDEPlugin.getPluginId(), Messages.AddDirectoryContainerPage_6));
									return false;
								}
								try {
