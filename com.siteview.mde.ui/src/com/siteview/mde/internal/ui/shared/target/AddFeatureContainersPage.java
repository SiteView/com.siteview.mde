...
public class AddFeatureContainersPage extends EditDirectoryContainerPage {
	...
	private MultiStatus doLoadFeatures(List result, File path) {
		...
		return new MultiStatus(IMDEUIConstants.PLUGIN_ID, IStatus.OK, (IStatus[]) resultStatus.toArray(new IStatus[resultStatus.size()]), Messages.FeatureImportWizard_DetailedPage_problemsLoading, null);
	}
	...
	private IStatus doLoadFeature(File dir, File manifest, List result) {
		...
		try {
			...
			if (!model.isValid()) {
				status = new Status(IStatus.WARNING, IMDEUIConstants.PLUGIN_ID, IStatus.OK, NLS.bind(Messages.FeatureImportWizardPage_importHasInvalid, dir), null);
			}
		} catch (Exception e) {
			// Errors in the file
