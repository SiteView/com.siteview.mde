...
import com.siteview.mde.internal.ui.shared.target.TargetLocationsGroup.IUWrapper;
	...
	public Image getImage(Object element) {
		if (element instanceof IResolvedBundle) {
			...
			if (bundle.getStatus().getSeverity() == IStatus.ERROR && bundle.getStatus().getCode() == IResolvedBundle.STATUS_FEATURE_DOES_NOT_EXIST) {
				...
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_FEATURE_OBJ, flag);
			} else if (bundle.isFragment()) {
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_FRAGMENT_OBJ, flag);
			} else if (bundle.isSourceBundle()) {
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_PLUGIN_MF_OBJ, flag);
			} else {
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_PLUGIN_OBJ, flag);
			}
		} else if (element instanceof BundleInfo) {
			return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_PLUGIN_OBJ);
		} else if (element instanceof NameVersionDescriptor) {
			if (((NameVersionDescriptor) element).getType() == NameVersionDescriptor.TYPE_FEATURE) {
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_FEATURE_OBJ);
			...
			if (element instanceof FeatureBundleContainer) {
				...
				return MDEPlugin.getDefault().getLabelProvider().get(image, flag);
			} else if (element instanceof ProfileBundleContainer) {
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_PRODUCT_DEFINITION, flag);
			} else if (element instanceof IUBundleContainer) {
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_REPOSITORY_OBJ, flag);
			}
		} else if (element instanceof IUWrapper) {
			return getImage(((IUWrapper) element).getIU());
		} else if (element instanceof IInstallableUnit) {
			return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_NOREF_FEATURE_OBJ);
		}
		return null;
	}
