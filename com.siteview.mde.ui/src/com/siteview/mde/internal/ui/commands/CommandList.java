...
public class CommandList {
	...
	protected class CommandTreeLabelProvider extends LabelProvider {
		...
		public Image getImage(Object element) {
			...
			if (element instanceof Category)
				img = MDEPluginImages.DESC_COMGROUP_OBJ.createImage();
			else if (element instanceof Command) {
				ImageDescriptor desc = fComImgServ.getImageDescriptor(((Command) element).getId());
				if (desc == null) {
					if (fDefaultImage == null)
						fDefaultImage = MDEPluginImages.DESC_BUILD_VAR_OBJ.createImage();
	...
	private class CollapseAction extends Action {
		public CollapseAction() {
			super(MDEUIMessages.CommandList_collapseAll0, IAction.AS_PUSH_BUTTON);
			setImageDescriptor(MDEPluginImages.DESC_COLLAPSE_ALL);
			setToolTipText(MDEUIMessages.CommandList_collapseAll0);
		}
	...
	private void createFilterText(Composite parent) {
		...
		final Image hoverImg = MDEPluginImages.DESC_CLEAR.createImage();
		clearButton.setImage(hoverImg);
		clearButton.setToolTipText(MDEUIMessages.CommandList_clearTooltip);
