...
public class CommandDetails {
	...
	private void createCommandDetails(Composite parent) {
		...
		section.setText(MDEUIMessages.CommandDetails_groupName);
	...
	private void createBasicInfo(Composite parent) {
		...
		fToolkit.createLabel(comp, MDEUIMessages.CommandDetails_id);
		fComIDT = fToolkit.createText(comp, MDEUIMessages.CommandDetails_noComSelected, SWT.BORDER);
	...
	private void createParameters(Composite parent) {
		...
		fParamLabel = fToolkit.createLabel(comp, MDEUIMessages.CommandDetails_noParameters);
	...
	private void createLinks(Composite parent) {
		...
		final Image execImage = MDEPluginImages.DESC_RUN_EXC.createImage();
		fExecLink.setImage(execImage);
		fExecLink.setText(MDEUIMessages.CommandDetails_executeText);
		fExecLink.setToolTipText(MDEUIMessages.CommandDetails_execute);
		...
		final Image clipImage = MDEPluginImages.DESC_CLIPBOARD.createImage();
		fCopyLink.setImage(clipImage);
		fCopyLink.setText(MDEUIMessages.CommandDetails_copyToClipboard);
		fCopyLink.setToolTipText(MDEUIMessages.CommandDetails_copytooltip);
	...
	private void createPreviewLabelComp(Composite parent) {
		...
		fToolkit.createLabel(preLabelComp, MDEUIMessages.CommandDetails_preview, SWT.NONE).setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		fSurroundCopyText = fToolkit.createButton(preLabelComp, MDEUIMessages.CommandDetails_includeMarkup, SWT.CHECK);
		fSurroundCopyText.setToolTipText(MDEUIMessages.CommandDetails_markupTooltip);
		...
		public void linkActivated(HyperlinkEvent e) {
			...
			try {
				...
				if (resultString != null) {
					MessageDialog.openInformation(fComIDT.getShell(), MDEUIMessages.CommandDetails_commandResult, resultString);
				}
			} catch (CommandException ex) {
				MessageDialog.openError(fComIDT.getShell(), MDEUIMessages.CommandDetails_execError, ex.toString());
			}
		}
	}
	...
	private class ObjectParameterControl implements ModifyListener {
		...
		private void validate() {
			...
			if (error == null)
				...
				fCCP.setMessage(NLS.bind(MDEUIMessages.CommandDetails_paramValueMessage, fParameter.getName(), error), IMessageProvider.WARNING);
		}
	}
	...
	private void populateParams(Command command, Object selectedObject) throws NotDefinedException {
		...
		if (parameters == null || parameters.length == 0) {
			fParamLabel.setText(MDEUIMessages.CommandDetails_noParameters);
		} else {
			fParamLabel.setText(MDEUIMessages.CommandDetails_numParams);
			...
			for (int i = 0; i < parameters.length; i++) {
				...
				if (!parameter.isOptional())
					nameText += '*';
				fToolkit.createLabel(paramLine, NLS.bind(MDEUIMessages.CommandDetails_param, nameText));
				...
				if (parameterValues != null) {
					...
					Button clearButton = fToolkit.createButton(paramLine, MDEUIMessages.CommandDetails_clear, SWT.PUSH);
	...
	private void resetAllFields() {
		...
		fComIDT.setText(MDEUIMessages.CommandDetails_noComSelected);
		fParamLabel.setText(MDEUIMessages.CommandDetails_noParameters);
