...
public class PDECompilersConfigurationBlock {
	...
	/**
	 * Returns a new {@link Key} for the {@link MDEPlugin} preference store
	 * @param key
	 * @return the new {@link Key} for the {@link MDEPlugin} preference store
	 */
	protected final static Key getPDEPrefKey(String key) {
		return new Key(MDE.PLUGIN_ID, key);
	}
	...
	private static String[] SEVERITIES = {MDEUIMessages.PDECompilersConfigurationBlock_error, MDEUIMessages.PDECompilersConfigurationBlock_warning, MDEUIMessages.PDECompilersConfigurationBlock_ignore};
	...
	public PDECompilersConfigurationBlock(IProject project, IWorkbenchPreferenceContainer container) {
		...
		MDECore.getDefault().getPreferencesManager();
	}
	...
	public Control createControl(Composite parent) {
		...
		//plugins page
		createPage(CompilerFlags.PLUGIN_FLAGS, main, MDEUIMessages.CompilersConfigurationBlock_plugins, MDEUIMessages.CompilersConfigurationBlock_label);

		if (fProject == null) {
			//the sharing property page does not use these two tabs
			//schema page
			createPage(CompilerFlags.SCHEMA_FLAGS, main, MDEUIMessages.CompilersConfigurationBlock_schemas, MDEUIMessages.CompilersConfigurationBlock_altlabel);

			//features / sites page
			createPage(CompilerFlags.FEATURE_FLAGS, main, MDEUIMessages.CompilersConfigurationBlock_features, MDEUIMessages.CompilersConfigurationBlock_label);
		}
	...
	private Composite createPage(int kind, Composite folder, String name, String description) {
		...
		switch (kind) {
			case CompilerFlags.PLUGIN_FLAGS : {
				...
				// General
				Composite client = createExpansibleComposite(sbody, MDEUIMessages.PDECompilersConfigurationBlock_general);
				initializeComboControls(client, new String[] {MDEUIMessages.compilers_p_no_required_att, MDEUIMessages.CompilersConfigurationTab_incompatEnv, MDEUIMessages.compilers_p_exported_pkgs}, new Key[] {KEY_P_NO_REQUIRED_ATT, KEY_P_INCOMPATIBLE_ENV, KEY_P_MISSING_EXPORT_PKGS}, CompilerFlags.PLUGIN_FLAGS);
				//build.properties
				client = createExpansibleComposite(sbody, MDEUIMessages.PDECompilersConfigurationBlock_build);
				initializeComboControls(client, new String[] {MDEUIMessages.PDECompilersConfigurationBlock_1, MDEUIMessages.PDECompilersConfigurationBlock_2, MDEUIMessages.PDECompilersConfigurationBlock_0, MDEUIMessages.PDECompilersConfigurationBlock_3, MDEUIMessages.PDECompilersConfigurationBlock_4, MDEUIMessages.PDECompilersConfigurationBlock_5, MDEUIMessages.PDECompilersConfigurationBlock_6, MDEUIMessages.PDECompilersConfigurationBlock_7, MDEUIMessages.CompilersConfigurationTab_buildPropertiesErrors}, new Key[] {KEY_P_BUILD_SOURCE_LIBRARY, KEY_P_BUILD_OUTPUT_LIBRARY, KEY_P_BUILD_MISSING_OUTPUT, KEY_P_BUILD_BIN_INCLUDES, KEY_P_BUILD_SRC_INCLUDES, KEY_P_BUILD_JAVA_COMPLIANCE, KEY_P_BUILD_JAVA_COMPILER, KEY_P_BUILD_ENCODINGS, KEY_P_BUILD}, CompilerFlags.PLUGIN_FLAGS);
				// Versioning
				client = createExpansibleComposite(sbody, MDEUIMessages.PDECompilersConfigurationBlock_versioning);
				initializeComboControls(client, new String[] {MDEUIMessages.compilers_p_missing_require_bundle, MDEUIMessages.compilers_p_missing_exp_pkg, MDEUIMessages.compilers_p_missing_imp_pkg}, new Key[] {KEY_P_VERSION_REQ_BUNDLE, KEY_P_VERSION_EXP_PKG, KEY_P_VERSION_IMP_PKG}, CompilerFlags.PLUGIN_FLAGS);
				// Usage
				client = createExpansibleComposite(sbody, MDEUIMessages.PDECompilersConfigurationBlock_usage);
				initializeComboControls(client, new String[] {MDEUIMessages.compilers_p_unresolved_import, MDEUIMessages.compilers_p_unresolved_ex_points, MDEUIMessages.compilers_p_internal, MDEUIMessages.compilers_p_deprecated, MDEUIMessages.compilers_p_not_externalized_att}, new Key[] {KEY_P_UNRESOLVED_IMPORTS, KEY_P_UNRESOLVED_EX_POINTS, KEY_P_INTERNAL, KEY_P_DEPRECATED, KEY_P_NOT_EXTERNALIZED}, CompilerFlags.PLUGIN_FLAGS);
				// References
				client = createExpansibleComposite(sbody, MDEUIMessages.PDECompilersConfigurationBlock_references);
				initializeComboControls(client, new String[] {MDEUIMessages.compilers_p_unknown_element, MDEUIMessages.compilers_p_unknown_attribute, MDEUIMessages.compilers_p_unknown_class, MDEUIMessages.compilers_p_discouraged_class, MDEUIMessages.compilers_p_unknown_resource, MDEUIMessages.compilers_p_unknown_identifier}, new Key[] {KEY_P_UNKNOWN_ELEMENT, KEY_P_UNKNOWN_ATTRIBUTE, KEY_P_UNKNOWN_CLASS, KEY_P_DISCOURAGED_CLASS, KEY_P_UNKNOWN_RESOURCE, KEY_P_UNKNOWN_IDENTIFIER,}, CompilerFlags.PLUGIN_FLAGS);

				break;
			}
			case CompilerFlags.SCHEMA_FLAGS : {
				createCheckControl(parent, MDEUIMessages.compilers_s_create_docs, KEY_S_CREATE_DOCS, CompilerFlags.SCHEMA_FLAGS);
				Composite comp = SWTFactory.createComposite(parent, 2, 2, GridData.FILL_HORIZONTAL, 0, 0);
				createTextControl(comp, MDEUIMessages.compilers_s_doc_folder, KEY_S_DOC_FOLDER, CompilerFlags.SCHEMA_FLAGS);
				SWTFactory.createVerticalSpacer(parent, 1);
				initializeComboControls(parent, new String[] {MDEUIMessages.compilers_s_open_tags}, new Key[] {KEY_S_OPEN_TAGS}, CompilerFlags.SCHEMA_FLAGS);
				break;
			}
			case CompilerFlags.FEATURE_FLAGS : {
				initializeComboControls(parent, new String[] {MDEUIMessages.compilers_f_unresolved_features, MDEUIMessages.compilers_f_unresolved_plugins}, new Key[] {KEY_F_UNRESOLVED_FEATURES, KEY_F_UNRESOLVED_PLUGINS}, CompilerFlags.FEATURE_FLAGS);
				break;
			}
		}
		return page;
			...
			try {
				...
				if (changes.size() > 0) {
					if (fRebuildcount < 1) {
						...
						String message = MDEUIMessages.PDECompilersConfigurationBlock_settings_changed_all;
						if (fProject != null) {
							message = MessageFormat.format(MDEUIMessages.PDECompilersConfigurationBlock_setting_changed_project, new String[] {fProject.getName()});
						}
						if (MessageDialog.openQuestion(fParent.getShell(), MDEUIMessages.PDECompilersConfigurationBlock_settings_changed, message)) {
							doFullBuild();
						}
					}
				}
	...
	private void persistExpansionState() {
		IDialogSettings settings = MDEPlugin.getDefault().getDialogSettings().addNewSection(SETTINGS);
	...
	private void restoreExpansionState() {
		IDialogSettings settings = MDEPlugin.getDefault().getDialogSettings().getSection(SETTINGS);
		if (settings != null) {
			for (int i = 0; i < fExpComps.size(); i++) {
	...
	private void addBuilder(Control control) {
		...
		for (Iterator iter = fControlMap.keySet().iterator(); iter.hasNext();) {
			...
			if (controls.contains(control)) {
				switch (key.intValue()) {
					case CompilerFlags.PLUGIN_FLAGS : {
						fBuilders.add(MDE.MANIFEST_BUILDER_ID);
						break;
					}
					case CompilerFlags.SCHEMA_FLAGS : {
						fBuilders.add(MDE.SCHEMA_BUILDER_ID);
						break;
					}
					case CompilerFlags.FEATURE_FLAGS :
					case CompilerFlags.SITE_FLAGS : {
						fBuilders.add(MDE.FEATURE_BUILDER_ID);
						break;
					}
				}
				return;
		...
		Job buildJob = new Job(MDEUIMessages.CompilersConfigurationBlock_building) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					...
					if (fProject == null) {
						projects = MDEPlugin.getWorkspace().getRoot().getProjects();
					} else {
					...
					for (int i = 0; i < projects.length; i++) {
						...
						if (!projectToBuild.isOpen())
							continue;
						if (projectToBuild.hasNature(MDE.PLUGIN_NATURE)) {
							if (fBuilders.contains(MDE.MANIFEST_BUILDER_ID))
								projectToBuild.build(IncrementalProjectBuilder.FULL_BUILD, MDE.MANIFEST_BUILDER_ID, null, new SubProgressMonitor(monitor, 1));
							else
								monitor.worked(1);
							if (fBuilders.contains(MDE.SCHEMA_BUILDER_ID))
								projectToBuild.build(IncrementalProjectBuilder.FULL_BUILD, MDE.SCHEMA_BUILDER_ID, null, new SubProgressMonitor(monitor, 1));
							else
								monitor.worked(1);
						} else if (projectToBuild.hasNature(MDE.FEATURE_NATURE)) {
							if (fBuilders.contains(MDE.FEATURE_BUILDER_ID))
								projectToBuild.build(IncrementalProjectBuilder.FULL_BUILD, MDE.FEATURE_BUILDER_ID, null, new SubProgressMonitor(monitor, 2));
						} else {
							monitor.worked(2);
						}
					}
