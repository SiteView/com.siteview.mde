...
public class ProjectSelectionDialog extends SelectionStatusDialog {
			...
			if (element instanceof IJavaModel) {
				...
				try {
					...
					for (int i = 0; i < projects.length; i++) {
						if (projects[i].getProject().hasNature(MDE.PLUGIN_NATURE)) {
							set.add(projects[i]);
						}
					}
	...
	public ProjectSelectionDialog(Shell parentShell, Set projectsWithSpecifics) {
		super(parentShell);
		setTitle(MDEUIMessages.ProjectSelectionDialog_title);
		setMessage(MDEUIMessages.ProjectSelectionDialog_message);
	...
	protected Control createDialogArea(Composite parent) {
		...
		Button checkbox = SWTFactory.createCheckButton(composite, MDEUIMessages.ProjectSelectionDialog_settingsTitle, null, false, 1);
	...
	private void doSelectionChanged(Object[] objects) {
		if (objects.length != 1) {
			updateStatus(new Status(IStatus.ERROR, MDEPlugin.getPluginId(), "")); //$NON-NLS-1$
			setSelectionResult(null);
		} else {
			updateStatus(new Status(IStatus.OK, MDEPlugin.getPluginId(), "")); //$NON-NLS-1$
	...
	protected void updateFilter(boolean selected) {
		...
		MDEPlugin.getDefault().getDialogSettings().put(DIALOG_SETTINGS_SHOW_ALL, !selected);
	}
