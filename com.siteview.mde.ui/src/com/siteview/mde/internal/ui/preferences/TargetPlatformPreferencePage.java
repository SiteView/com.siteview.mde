...
import com.siteview.mde.internal.ui.shared.target.StyledBundleLabelProvider;
import com.siteview.mde.internal.ui.util.SWTUtil;
import com.siteview.mde.internal.ui.util.SharedLabelProvider;
import com.siteview.mde.internal.ui.wizards.target.*;
...
public class TargetPlatformPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	private class TargetLabelProvider extends StyledCellLabelProvider {
		...
		public void update(ViewerCell cell) {
			...
			if (targetDef.equals(fActiveTarget)) {
				name = name + MDEUIMessages.TargetPlatformPreferencePage2_1;
			}
		...
		private Image getImage(ITargetDefinition target) {
			...
			if (fRunningHost != null && fRunningHost.isContentEquivalent(target)) {
				return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_PRODUCT_BRANDING, flag);
			}
			return MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_TARGET_DEFINITION, flag);
		}
	...
	private void createTargetProfilesGroup(Composite container) {
		...
		fReloadButton = SWTFactory.createPushButton(buttonComposite, MDEUIMessages.TargetPlatformPreferencePage2_16, null);
		...
		fAddButton = SWTFactory.createPushButton(buttonComposite, MDEUIMessages.TargetPlatformPreferencePage2_3, null);
		...
		fEditButton = SWTFactory.createPushButton(buttonComposite, MDEUIMessages.TargetPlatformPreferencePage2_5, null);
		...
		fRemoveButton = SWTFactory.createPushButton(buttonComposite, MDEUIMessages.TargetPlatformPreferencePage2_7, null);
		...
		fMoveButton = SWTFactory.createPushButton(buttonComposite, MDEUIMessages.TargetPlatformPreferencePage2_13, null);
		...
		if (!selection.isEmpty()) {
			...
			if (fActiveTarget.isResolved()) {
				...
				if (bundleStatus.getSeverity() == IStatus.ERROR) {
					ErrorDialog.openError(getShell(), MDEUIMessages.TargetPlatformPreferencePage2_14, MDEUIMessages.TargetPlatformPreferencePage2_15, bundleStatus, IStatus.ERROR);
				}
				...
				try {
					if (bundleStatus.getSeverity() != IStatus.ERROR && fActiveTarget.getHandle().equals(fPrevious) && ((TargetDefinition) fPrevious.getTargetDefinition()).isContentEquivalent(fActiveTarget)) {
						...
						if (!compare.isOK()) {
							MessageDialog.openInformation(getShell(), MDEUIMessages.TargetPlatformPreferencePage2_17, MDEUIMessages.TargetPlatformPreferencePage2_18);
							isOutOfSynch = true;
						}
					}
				} catch (CoreException e) {
	...
	private void handleAdd() {
		...
		wizard.setWindowTitle(MDEUIMessages.TargetPlatformPreferencePage2_4);
	...
	private void handleEdit() {
		if (!fTableViewer.getSelection().isEmpty()) {
			...
			wizard.setWindowTitle(MDEUIMessages.TargetPlatformPreferencePage2_6);
		...
		if (!selection.isEmpty()) {
			...
			if (isWorkspace) {
				...
				if (!MessageDialogWithToggle.ALWAYS.equalsIgnoreCase(choice)) {
					MessageDialogWithToggle dialog = MessageDialogWithToggle.openYesNoQuestion(getShell(), MDEUIMessages.TargetPlatformPreferencePage2_19, MDEUIMessages.TargetPlatformPreferencePage2_20, MDEUIMessages.TargetPlatformPreferencePage2_21, false, MDEPlugin.getDefault().getPreferenceStore(), IPreferenceConstants.PROP_PROMPT_REMOVE_TARGET);
	...
	private void handleMove() {
		...
		if (dialog.open() == Window.OK) {
			...
			IFile file = MDECore.getWorkspace().getRoot().getFile(newTargetLoc);
	...
	private ITargetPlatformService getTargetService() {
		return (ITargetPlatformService) MDECore.getDefault().acquireService(ITargetPlatformService.class.getName());
	}
	...
	public boolean performOk() {
		...
		try {
			...
			ErrorDialog.openError(getShell(), MDEUIMessages.TargetPlatformPreferencePage2_8, MDEUIMessages.TargetPlatformPreferencePage2_9, e.getStatus());
			return false;
		}
		...
		if (fMoved.size() > 0) {
			...
			while (iterator.hasNext()) {
				try {
					...
					IFile targetFile = MDECore.getWorkspace().getRoot().getFile((IPath) fMoved.get(target));
		...
		while (iterator.hasNext()) {
			...
			try {
				...
				ErrorDialog.openError(getShell(), MDEUIMessages.TargetPlatformPreferencePage2_8, MDEUIMessages.TargetPlatformPreferencePage2_11, e.getStatus());
				return false;
			}
		}
			...
			IJobChangeListener listener = new JobChangeAdapter() {
				public void done(IJobChangeEvent event) {
					if (event.getResult().getSeverity() == IStatus.OK) {
						if (fActiveTarget != null) {
							MDEPreferencesManager pref = new MDEPreferencesManager(MDEPlugin.getPluginId());
									...
									if (ORG_ECLIPSE_OSGI.equalsIgnoreCase(bundleInfo.getSymbolicName())) {
										...
										if (platformOsgiVersion.compareTo(bundleVersion) < 0) {
											Display.getDefault().syncExec(new Runnable() {
												public void run() {
													MessageDialog.openWarning(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.TargetPlatformPreferencePage2_28, MDEUIMessages.TargetPlatformPreferencePage2_10);
												}
											});
										}
										break;
	...
	private void runGC() {
		Job job = new Job(MDEUIMessages.TargetPlatformPreferencePage2_26) {
			protected IStatus run(IProgressMonitor monitor) {
				monitor.beginTask(MDEUIMessages.TargetPlatformPreferencePage2_27, IProgressMonitor.UNKNOWN);
