 ...
 *******************************************************************************/
package com.siteview.mde.internal.ui.views.target;
...
import com.siteview.mde.internal.ui.views.dependencies.DependenciesViewComparator;
...
public class StateViewPage extends Page implements IStateDeltaListener, IMonitorModelListener {
	...
	class DependencyGroup {
		...
		public String toString() {
			return (dependencies[0] instanceof BundleSpecification) ? MDEUIMessages.StateViewPage_requiredBundles : MDEUIMessages.StateViewPage_importedPackages;
		}
	}
	...
	class StateLabelProvider extends StyledCellLabelProvider implements ILabelProvider {
		...
		public void update(ViewerCell cell) {
			...
			if (element instanceof ImportPackageSpecification) {
				...
				if (isJREPackage(supplier)) {
					styledString.append(MDEUIMessages.StateViewPage_suppliedByJRE);
				} else {
					styledString.append(MDEUIMessages.StateViewPage_suppliedBy);
		...
		private void getElementString(Object element, StyledString styledString, boolean showLocation) {
			if (element instanceof BundleSpecification) {
				...
				styledString.append(' ').append(MDELabelProvider.formatVersion(version.toString())).toString();
		...
		public Image getImage(Object element) {
			...
			if (element instanceof BundleDescription) {
				...
				return (((BundleDescription) element).getHost() == null) ? fSharedProvider.get(MDEPluginImages.DESC_PLUGIN_OBJ, flags) : fSharedProvider.get(MDEPluginImages.DESC_FRAGMENT_OBJ, flags);
			}
			if (element instanceof ImportPackageSpecification)
				return JavaUI.getSharedImages().getImage(ISharedImages.IMG_OBJS_PACKAGE);
			if (element instanceof ResolverError) {
				if (((ResolverError) element).getType() == ResolverError.PLATFORM_FILTER)
					return fSharedProvider.get(MDEPluginImages.DESC_OPERATING_SYSTEM_OBJ);
	...
	protected void setActive(boolean active) {
		if (active) {
			State state = MDECore.getDefault().getModelManager().getState().getState();
			state.resolve(true);
	...
	public void makeContributions(IMenuManager menuManager, IToolBarManager toolBarManager, IStatusLineManager statusLineManager) {
		...
		Action filterResolved = new Action(MDEUIMessages.StateViewPage_showOnlyUnresolved_label, IAction.AS_CHECK_BOX) {
			public void run() {
		...
		Action filterLeaves = new Action(MDEUIMessages.StateViewPage_showLeaves, IAction.AS_CHECK_BOX) {
			public void run() {
	...
	private void fillContextMenu(IMenuManager menu) {
		...
		if (desc != null) {
			if (fOpenAction == null) {
				fOpenAction = new Action(MDEUIMessages.StateViewPage_openItem) {
					public void run() {
						handleDoubleClick();
					}
				};
