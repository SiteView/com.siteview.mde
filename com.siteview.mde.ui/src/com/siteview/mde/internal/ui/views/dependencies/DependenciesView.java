...
import com.siteview.mde.internal.ui.editor.monitor.LoopDialog;
...
import com.siteview.mde.internal.ui.views.target.StateViewPage;
...
public class DependenciesView extends PageBookView implements IPreferenceConstants, IHelpContextIds {
	...
	class ShowLoopsAction extends Action {
		...
		public ShowLoopsAction() {
			...
			setText(MDEUIMessages.DependenciesView_ShowLoopsAction_label);
			setDescription(MDEUIMessages.DependenciesView_ShowLoopsAction_description);
			setToolTipText(MDEUIMessages.DependenciesView_ShowLoopsAction_tooltip);
			setImageDescriptor(MDEPluginImages.DESC_DEP_LOOP);
			setDisabledImageDescriptor(MDEPluginImages.DESC_DEP_LOOP_DISABLED);
			setEnabled(false);
		}
		 ...
		 */
		public void run() {
			LoopDialog dialog = new LoopDialog(MDEPlugin.getActiveWorkbenchShell(), fLoops);
			dialog.open();
		}
	}
	...
	class ShowCalleesAction extends Action {
		...
		public ShowCalleesAction() {
			...
			setText(MDEUIMessages.DependenciesView_ShowCalleesAction_label);
			setDescription(MDEUIMessages.DependenciesView_ShowCalleesAction_description);
			setToolTipText(MDEUIMessages.DependenciesView_ShowCalleesAction_tooltip);
			setImageDescriptor(MDEPluginImages.DESC_CALLEES);
			setDisabledImageDescriptor(MDEPluginImages.DESC_CALLEES_DISABLED);
		}
	...
	class ShowCallersAction extends Action {
		public ShowCallersAction() {
			super("", AS_RADIO_BUTTON); //$NON-NLS-1$
			setText(MDEUIMessages.DependenciesView_ShowCallersAction_label);
			setDescription(MDEUIMessages.DependenciesView_ShowCallersAction_description);
			setToolTipText(MDEUIMessages.DependenciesView_ShowCallersAction_tooltip);
			setImageDescriptor(MDEPluginImages.DESC_CALLERS);
			setDisabledImageDescriptor(MDEPluginImages.DESC_CALLERS_DISABLED);
		}
	...
	class ShowListAction extends Action {
		public ShowListAction() {
			super("", AS_RADIO_BUTTON); //$NON-NLS-1$
			setText(MDEUIMessages.DependenciesView_ShowListAction_label);
			setDescription(MDEUIMessages.DependenciesView_ShowListAction_description);
			setToolTipText(MDEUIMessages.DependenciesView_ShowListAction_tooltip);
			setImageDescriptor(MDEPluginImages.DESC_FLAT_LAYOUT);
			setDisabledImageDescriptor(MDEPluginImages.DESC_FLAT_LAYOUT_DISABLED);
		}
	...
	class ShowTreeAction extends Action {
		...
		public ShowTreeAction() {
			super("", AS_RADIO_BUTTON); //$NON-NLS-1$
			setText(MDEUIMessages.DependenciesView_ShowTreeAction_label);
			setDescription(MDEUIMessages.DependenciesView_ShowTreeAction_description);
			setToolTipText(MDEUIMessages.DependenciesView_ShowTreeAction_tooltip);
			setImageDescriptor(MDEPluginImages.DESC_HIERARCHICAL_LAYOUT);
			setDisabledImageDescriptor(MDEPluginImages.DESC_HIERARCHICAL_LAYOUT_DISABLED);
		}
	...
	private MDEPreferencesManager fPreferences = MDEPlugin.getDefault().getPreferenceManager();
	...
	public void init(IViewSite site, IMemento memento) throws PartInitException {
		...
		if (id != null) {
			IMonitorModelBase plugin = MonitorRegistry.findModel(id);
			if (plugin != null) {
				fInput = plugin;
				addHistoryEntry(id);
				findLoops();
	...
	public void openTo(Object object) {
		if (object != null && !object.equals(fInput)) {
			if (object instanceof IMonitorModelBase) {
				String id = ((IMonitorModelBase) object).getMonitorBase().getId();
				addHistoryEntry(id);
			}
		}
		updateInput(object);
	...
	private void findLoops() {
		fLoops = NO_LOOPS;
		if (fInput != null && fInput instanceof IMonitorModel) {
			BusyIndicator.showWhile(MDEPlugin.getActiveWorkbenchShell().getDisplay(), new Runnable() {
				 ...
				 */
				public void run() {
					IMonitor plugin = ((IMonitorModel) fInput).getMonitor();
	...
	public void saveState(IMemento memento) {
		...
		if (fInput != null && fInput instanceof IMonitorModelBase) {
			String inputPluginId = ((IMonitorModelBase) fInput).getMonitorBase().getId();
	...
	void updateTitle(Object newInput) {
		...
		} else if (!newInput.equals(MDECore.getDefault().getModelManager())) {
			String name = MDEPlugin.getDefault().getLabelProvider().getText(newInput);
			String title;
			if (getCurrentContributingPart() == PART_CALLEES_TREE) {
				title = NLS.bind(MDEUIMessages.DependenciesView_callees_tree_title, name);
			} else if (getCurrentContributingPart() == PART_CALLEES_LIST) {
				title = NLS.bind(MDEUIMessages.DependenciesView_callees_list_title, name);
			} else if (getCurrentContributingPart() == PART_CALLERS_TREE) {
				title = NLS.bind(MDEUIMessages.DependenciesView_callers_tree_title, name);
			} else {
				title = NLS.bind(MDEUIMessages.DependenciesView_callers_list_title, name);
			}
			if (fLoops != NO_LOOPS) {
				title = title + " " + MDEUIMessages.DependenciesView_cycles_title; //$NON-NLS-1$
			}
			updateTitle(title);
		}
	}
	...
	private void updateHistoryEntries() {
		for (int i = fInputHistory.size() - 1; i >= 0; i--) {
			...
			if (MonitorRegistry.findModel(type) == null) {
	...
	public void gotoHistoryEntry(String entry) {
		if (fInputHistory.contains(entry)) {
			updateInput(MonitorRegistry.findModel(entry));
		}
	}
	...
	public String getInput() {
		if (fInput != null) {
			return ((IMonitorModelBase) fInput).getMonitorBase().getId();
		}
		return null;
	}
