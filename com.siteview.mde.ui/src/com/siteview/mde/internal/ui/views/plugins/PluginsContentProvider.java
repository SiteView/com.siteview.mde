...
public class PluginsContentProvider extends DefaultContentProvider implements ITreeContentProvider, IStructuredContentProvider {
	...
	public Object[] getChildren(Object parentElement) {
		if (parentElement instanceof IDeferredWorkbenchAdapter) {
			if (MDECore.getDefault().getModelManager().isInitialized())
		...
		if (parentElement instanceof IMonitorModelBase) {
			IMonitorModelBase model = (IMonitorModelBase) parentElement;
			if (model != null) {
				File file = new File(model.getInstallLocation());
				if (!file.isFile()) {
					FileAdapter adapter = new ModelFileAdapter(model, file, MDECore.getDefault().getSearchablePluginsManager());
	...
	public Object getParent(Object element) {
		if (element instanceof MonitorModelManager) {
			return null;
		}
