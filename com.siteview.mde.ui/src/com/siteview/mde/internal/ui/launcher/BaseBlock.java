...
public abstract class BaseBlock {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if (f.exists())
						...
						MessageDialog.openWarning(MDEPlugin.getActiveWorkbenchShell(), isFile() ? MDEUIMessages.BaseBlock_fileTitle : MDEUIMessages.BaseBlock_directoryTitle, isFile() ? MDEUIMessages.BaseBlock_fileNotFoundMessage : MDEUIMessages.BaseBlock_directoryNotFoundMessage);
				} catch (Exception ex) {
					MessageDialog.openWarning(MDEPlugin.getActiveWorkbenchShell(), isFile() ? MDEUIMessages.BaseBlock_fileTitle : MDEUIMessages.BaseBlock_directoryTitle, isFile() ? MDEUIMessages.BaseBlock_fileErrorMessage : MDEUIMessages.BaseBlock_directoryErrorMessage);
				}
			}
		});
	...
	protected void handleBrowseFileSystem() {
		...
		dialog.setText(MDEUIMessages.BaseBlock_dirSelection);
		dialog.setMessage(MDEUIMessages.BaseBlock_dirChoose);
	...
	protected void handleBrowseWorkspace() {
		ContainerSelectionDialog dialog = new ContainerSelectionDialog(fLocationText.getShell(), getContainer(), true, MDEUIMessages.BaseBlock_relative);
	...
	public String validate() {
		return (fLocationText.isEnabled() && getLocation().length() == 0) ? NLS.bind(MDEUIMessages.BaseBlock_errorMessage, getName()) : null;
	}
