		...
		if (isEnabled() || fInitDefaultCheckState) {
			...
			if (fInitDefaultCheckState) {
				...
				for (int i = 0; i < workspaceModels.length; i++) {
					String id = workspaceModels[i].getMonitorBase().getId();
					if (id != null) {
				...
				for (int i = 0; i < externalModels.length; i++) {
					...
					boolean masked = checkedWorkspace.contains(model.getMonitorBase().getId());
					if (masked) {
						wBuffer.add(model);
	...
	protected void addRequiredPlugins() {
		...
		try {
			...
			for (int i = 0; i < requiredIds.length; i++) {
				...
				IMonitorModelBase base = findPlugin(requiredIds[i]);
				if (base == null) {
					base = MonitorRegistry.findModel(requiredIds[i]);
					if (base != null) {
