...
import com.siteview.mde.internal.ui.wizards.tools.OrganizeManifestsProcessor;
import com.siteview.mde.launching.IPDELauncherConstants;
import com.siteview.mde.ui.launcher.MainTab;
...
public class LauncherUtilsStatusHandler implements IStatusHandler {
	...
	private Boolean generateConfigIni() {
		String message = MDEUIMessages.LauncherUtils_generateConfigIni;
		return Boolean.valueOf(generateDialog(message).intValue() == 0);
	}

	private Integer deleteWorkspace(String path) {
		return generateDialog(NLS.bind(MDEUIMessages.WorkbenchLauncherConfigurationDelegate_confirmDeleteWorkspace, path));
	}

	private Integer clearLog() {
		return generateDialog(MDEUIMessages.LauncherUtils_clearLogFile);
	}

	private void handleWorkspaceLocked(String workspace, ILaunchConfiguration launchConfig, String mode) {
		String message = NLS.bind(MDEMessages.LauncherUtils_cannotLaunchApplication, workspace);
		generateErrorDialog(MDEUIMessages.LauncherUtils_workspaceLocked, message, launchConfig, mode);
	}
	...
	public final static Shell getActiveShell() {
		IWorkbenchWindow window = MDEPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow();
		if (window == null) {
			IWorkbenchWindow[] windows = MDEPlugin.getDefault().getWorkbench().getWorkbenchWindows();
			if (windows.length > 0)
	...
	private static void generateErrorDialog(final String title, final String message, final ILaunchConfiguration launchConfig, final String mode) {
		getDisplay().syncExec(new Runnable() {
			public void run() {
				...
				MessageDialog dialog = new MessageDialog(parentShell, title, null, message, MessageDialog.ERROR, new String[] {MDEUIMessages.LauncherUtils_edit, IDialogConstants.OK_LABEL}, 1);
				int res = dialog.open();
				if (res == 0) {
