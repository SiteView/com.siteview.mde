...
import com.siteview.mde.launching.PDESourcePathProvider;
...
public class LaunchAction extends Action {
	...
	private ILaunchConfiguration refreshConfiguration(ILaunchConfigurationWorkingCopy wc) throws CoreException {
		...
		IMonitorModelBase[] models = getModels();
	...
	private void appendBundle(StringBuffer buffer, IMonitorModelBase model) {
		IPluginConfiguration configuration = (IPluginConfiguration) fPluginConfigurations.get(model.getMonitorBase().getId());
	...
	private IMonitorModelBase[] getModels() {
		...
		return (IMonitorModelBase[]) map.values().toArray(new IMonitorModelBase[map.size()]);
	}
	...
	private String getExpandedPath(String path) {
		if (path == null || path.length() == 0)
			return null;
		IResource resource = MDEPlugin.getWorkspace().getRoot().findMember(new Path(path));
		if (resource != null) {
	...
	private ILaunchConfiguration chooseConfiguration(ILaunchConfiguration[] configs) {
		...
		ElementListSelectionDialog dialog = new ElementListSelectionDialog(MDEPlugin.getActiveWorkbenchShell(), labelProvider);
		dialog.setElements(configs);
		dialog.setTitle(MDEUIMessages.RuntimeWorkbenchShortcut_title);
		if (fMode.equals(ILaunchManager.DEBUG_MODE)) {
			dialog.setMessage(MDEUIMessages.RuntimeWorkbenchShortcut_select_debug);
		} else {
			dialog.setMessage(MDEUIMessages.RuntimeWorkbenchShortcut_select_run);
		}
