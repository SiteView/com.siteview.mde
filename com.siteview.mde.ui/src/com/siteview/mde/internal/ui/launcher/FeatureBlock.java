...
import com.siteview.mde.internal.launching.launcher.LaunchValidationOperation;
...
import com.siteview.mde.internal.ui.shared.CachedCheckboxTreeViewer;
import com.siteview.mde.internal.ui.shared.FilteredCheckboxTree;
...
public class FeatureBlock {
	...
	class FeatureTreeLabelProvider extends StyledCellLabelProvider implements ILabelProvider {
		...
		MDELabelProvider pdeLabelProvider = new MDELabelProvider();
		...
		public Image getColumnImage(Object obj, int index) {
			...
			if (index == COLUMN_FEATURE_NAME) {
				if (obj instanceof FeatureLaunchModel) {
					...
					IMonitorModelBase pluginModelBase = ((PluginLaunchModel) obj).getPluginModelBase();
		...
		private StyledString getStyledText(Object element) {
			...
			if (element instanceof FeatureLaunchModel) {
				...
				styledString.append(" (", StyledString.QUALIFIER_STYLER); //$NON-NLS-1$
				String version = pluginLaunchModel.getPluginModelBase().getMonitorBase().getVersion();
				int index = version.indexOf('-');
				if (index > -1)
					version = version.substring(0, index);
				styledString.append(version, StyledString.QUALIFIER_STYLER);
	...
	class ButtonSelectionListener extends SelectionAdapter {
		...
		private void handleSelectFeatures() {
			...
			if (featureModels.size() == 0) {
				MessageDialog.openWarning(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.FeatureSelectionDialog_title, MDEUIMessages.FeatureBlock_AllFeatureSelected);
				return;
			}
			FeatureSelectionDialog dialog = new FeatureSelectionDialog(MDEPlugin.getActiveWorkbenchShell(), (IFeatureModel[]) featureModels.toArray(new IFeatureModel[featureModels.size()]), true);
			dialog.create();
		...
		private void handleAddPlugin() {
			...
			if (dialog.open() == Window.OK) {
				...
				for (int i = 0; i < models.length; i++) {
					PluginLaunchModel pluginLaunchModel = new PluginLaunchModel((IMonitorModelBase) models[i], IPDELauncherConstants.LOCATION_DEFAULT);
		...
		private void handleValidate() {
			...
			if (fDialog == null) {
				if (fOperation.hasErrors()) {
					...
					MessageDialog.openInformation(getShell(), MDEUIMessages.PluginStatusDialog_pluginValidation, NLS.bind(MDEUIMessages.AbstractLauncherToolbar_noSelection, fTab.getName().toLowerCase(Locale.ENGLISH)));
				} else {
					MessageDialog.openInformation(getShell(), MDEUIMessages.PluginStatusDialog_pluginValidation, MDEUIMessages.AbstractLauncherToolbar_noProblems);
				}
			} else {
				if (fOperation.getInput().size() > 0)
					...
					input.put(MDEUIMessages.AbstractLauncherToolbar_noProblems, Status.OK_STATUS);
		...
		private Shell getShell() {
			...
			return MDEPlugin.getActiveWorkbenchShell();
		}
	...
	class PluginLaunchModel {
		private IMonitorModelBase fPluginModelBase;
		private String fPluginResolution;

		public PluginLaunchModel(IMonitorModelBase pluginModelBase, String pluginResolution) {
			fPluginModelBase = pluginModelBase;
			fPluginResolution = pluginResolution;
		}

		public IMonitorModelBase getPluginModelBase() {
		...
		public String getPluginModelId() {
			return fPluginModelBase.getMonitorBase().getId();
		}

		public String getPluginModelVersion() {
			return fPluginModelBase.getMonitorBase().getVersion();
		}
	...
	public void createControl(Composite parent, int span, int indent) {
		...
		SWTFactory.createLabel(treeGroup, MDEUIMessages.FeatureBlock_FeatureGroupDescription, 2);
		createCheckBoxTree(treeGroup);
		createButtonContainer(treeGroup, 10);

		fFeatureWorkspaceButton = SWTFactory.createCheckButton(treeGroup, MDEUIMessages.FeatureBlock_UseWorkspaceFeatures, null, true, 2);
		...
		fAutoValidate = SWTFactory.createCheckButton(validatecomp, NLS.bind(MDEUIMessages.PluginsTabToolBar_auto_validate, fTab.getName().replaceAll("&", "").toLowerCase(Locale.ENGLISH)), null, false, 1); //$NON-NLS-1$ //$NON-NLS-2$
		...
		fValidateButton = SWTFactory.createPushButton(rightAlignComp, NLS.bind(MDEUIMessages.PluginsTabToolBar_validate, fTab.getName().replaceAll("&", "")), null); //$NON-NLS-1$//$NON-NLS-2$
	...
	private void createCheckBoxTree(Composite parent) {
		...
		column1.setText(MDEUIMessages.FeatureBlock_features);
		...
		column2.setText(MDEUIMessages.FeatureBlock_pluginResolution);
		...
		String[] items = new String[] {MDEUIMessages.FeatureBlock_default, MDEUIMessages.FeatureBlock_WorkspaceResolutionLabel, MDEUIMessages.FeatureBlock_ExternalResolutionLabel};
	...
	private void createButtonContainer(Composite parent, int vOffset) {
		...
		fSelectAllButton = SWTFactory.createPushButton(buttonComp, MDEUIMessages.AdvancedLauncherTab_selectAll, null);
		fSelectAllButton.addSelectionListener(fListener);
		fDeselectAllButton = SWTFactory.createPushButton(buttonComp, MDEUIMessages.AdvancedLauncherTab_deselectAll, null);
		fDeselectAllButton.addSelectionListener(fListener);
		fSelectFeaturesButton = SWTFactory.createPushButton(buttonComp, MDEUIMessages.FeatureBlock_SelectFeatures, null);
		fSelectFeaturesButton.addSelectionListener(fListener);
		fAddRequiredFeaturesButton = SWTFactory.createPushButton(buttonComp, MDEUIMessages.FeatureBlock_addRequiredFeatues, null);
		fAddRequiredFeaturesButton.addSelectionListener(fListener);
		fAddPluginButton = SWTFactory.createPushButton(buttonComp, NLS.bind(MDEUIMessages.FeatureBlock_AddPluginsLabel, fTab.getName().replaceAll("&", "")), null); //$NON-NLS-1$//$NON-NLS-2$
		fAddPluginButton.addSelectionListener(fListener);
		fRemovePluginButton = SWTFactory.createPushButton(buttonComp, NLS.bind(MDEUIMessages.FeatureBlock_RemovePluginsLabel, fTab.getName().replaceAll("&", "")), null); //$NON-NLS-1$//$NON-NLS-2$
		fRemovePluginButton.addSelectionListener(fListener);
		fRemovePluginButton.setEnabled(false);
		fDefaultsButton = SWTFactory.createPushButton(buttonComp, MDEUIMessages.AdvancedLauncherTab_defaults, null);
		fDefaultsButton.addSelectionListener(fListener);

		SWTFactory.createLabel(buttonComp, MDEUIMessages.FeatureBlock_defaultPluginResolution, 1);
		fWorkspacePluginButton = SWTFactory.createRadioButton(buttonComp, MDEUIMessages.FeatureBlock_workspaceBefore);
		fWorkspacePluginButton.addSelectionListener(fListener);
		fExternalPluginButton = SWTFactory.createRadioButton(buttonComp, MDEUIMessages.FeatureBlock_externalBefore);
		...
		fFilterButton = SWTFactory.createCheckButton(countComp, NLS.bind(MDEUIMessages.AdvancedLauncherTab_selectedBundles, ""), null, false, 1); //$NON-NLS-1$
		fFilterButton.addSelectionListener(fListener);

		fCounter = SWTFactory.createLabel(countComp, "", 1); //$NON-NLS-1$

		Image siteImage = MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_SITE_OBJ);
		fAdditionalPluginsParentElement = new NamedElement(NLS.bind(MDEUIMessages.FeatureBlock_AdditionalPluginsEntry, fTab.getName().replaceAll("&", "")), siteImage); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public void initializeFrom(ILaunchConfiguration config) throws CoreException {
		...
		MDEPreferencesManager prefs = new MDEPreferencesManager(IMDEUIConstants.PLUGIN_ID);
	...
	private void updateCounter() {
		if (fCounter != null) {
			...
			fCounter.setText(NLS.bind(MDEUIMessages.AbstractPluginBlock_counter, new Integer(checked), new Integer(total)));
		}
	}
