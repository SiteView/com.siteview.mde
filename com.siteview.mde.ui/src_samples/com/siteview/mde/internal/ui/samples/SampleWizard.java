...
public class SampleWizard extends Wizard implements INewWizard, IExecutableExtension {
	...
	private class ImportOverwriteQuery implements IOverwriteQuery {
		...
		private int openDialog(final String file) {
			...
			getShell().getDisplay().syncExec(new Runnable() {
				public void run() {
					String title = MDEUIMessages.SampleWizard_title;
					String msg = NLS.bind(MDEUIMessages.SampleWizard_overwrite, file);
	...
	public SampleWizard() {
		...
		samples = Platform.getExtensionRegistry().getConfigurationElementsFor("com.siteview.mde.ui.samples"); //$NON-NLS-1$
		namesPage = new ProjectNamesPage(this);
		lastPage = new ReviewPage(this);
		setNeedsProgressMonitor(true);
		setWindowTitle(MDEUIMessages.ShowSampleAction_title);
	}
