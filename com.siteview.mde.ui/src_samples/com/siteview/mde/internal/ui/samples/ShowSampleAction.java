 ...
 *******************************************************************************/
package com.siteview.mde.internal.ui.samples;
...
public class ShowSampleAction extends Action implements IIntroAction {
	...
	private void switchToSampleStandby(SampleWizard wizard) {
		...
		url.append("pluginId=com.siteview.mde.ui"); //$NON-NLS-1$
		url.append("&"); //$NON-NLS-1$
		url.append("partId=com.siteview.mde.ui.sampleStandbyPart"); //$NON-NLS-1$
		url.append("&"); //$NON-NLS-1$
	...
	private void ensureProperContext(SampleWizard wizard) {
		...
		if (perspId != null) {
			try {
				...
				PlatformUI.getWorkbench().showPerspective(perspId, MDEPlugin.getActiveWorkbenchWindow());
				wizard.selectReveal(MDEPlugin.getActiveWorkbenchShell());
	...
	private boolean ensureSampleFeaturePresent() {
		...
		if (MessageDialog.openQuestion(MDEPlugin.getActiveWorkbenchShell(), MDEUIMessages.ShowSampleAction_msgTitle, MDEUIMessages.ShowSampleAction_msgDesc)) {
	...
	private boolean downloadFeature() {
		IRunnableWithProgress op = new IRunnableWithProgress() {
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				try {
					SubMonitor sub = SubMonitor.convert(monitor, MDEUIMessages.ShowSampleAction_installing, 100);
	...
	void applyConfiguration() throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, IMDEUIConstants.PLUGIN_ID, "Unexpected failure applying configuration", e)); //$NON-NLS-1$
		} finally {
