...
public class ReviewPage extends WizardPage {
	...
	public ReviewPage(SampleWizard wizard) {
		...
		this.wizard = wizard;
		setTitle(MDEUIMessages.ReviewPage_title);
		setDescription(MDEUIMessages.ReviewPage_desc);
	}
	...
	private void updateContent() {
		...
		if (selection != null) {
			...
			if (desc.length == 1)
				buf.append(NLS.bind(MDEUIMessages.ReviewPage_descContent, (new String[] {selection.getAttribute("name"), desc[0].getValue()}))); //$NON-NLS-1$ 
			else
				buf.append(NLS.bind(MDEUIMessages.ReviewPage_content, selection.getAttribute("name"))); //$NON-NLS-1$ 
		} else {
			setMessage(MDEUIMessages.ReviewPage_noSampleFound, IMessageProvider.WARNING);
		}
		buf.append("</form>"); //$NON-NLS-1$
