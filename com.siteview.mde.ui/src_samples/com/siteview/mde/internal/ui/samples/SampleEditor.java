...
public class SampleEditor extends EditorPart {
	...
	public void createPartControl(Composite parent) {
		...
		String content = NLS.bind(MDEUIMessages.SampleEditor_desc, (desc != null ? desc : "")); //$NON-NLS-1$ 
		...
		buf.append(MDEUIMessages.SampleEditor_content);
		...
		instText.setImage("run", MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_RUN_EXC)); //$NON-NLS-1$
		instText.setImage("debug", MDEPlugin.getDefault().getLabelProvider().get(MDEPluginImages.DESC_DEBUG_EXC)); //$NON-NLS-1$
	...
	public void dispose() {
		if (inputFileListener != null) {
			MDEPlugin.getWorkspace().removeResourceChangeListener(inputFileListener);
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		...
		MDEPlugin.getWorkspace().addResourceChangeListener(inputFileListener);
	}
