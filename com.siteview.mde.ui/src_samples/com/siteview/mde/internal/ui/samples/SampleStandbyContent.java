...
public class SampleStandbyContent implements IStandbyContentPart {
	...
	public void createPartControl(Composite parent, FormToolkit toolkit) {
		...
		buf.append(MDEUIMessages.SampleStandbyContent_content);
		...
		instText.setImage("run", MDEPlugin.getDefault().getLabelProvider().get( //$NON-NLS-1$
				MDEPluginImages.DESC_RUN_EXC));
		instText.setImage("debug", MDEPlugin.getDefault().getLabelProvider() //$NON-NLS-1$
				.get(MDEPluginImages.DESC_DEBUG_EXC));
	...
	private ISetSelectionTarget findTarget() {
		...
		if (id == null)
			return null;
		IViewPart view = MDEPlugin.getActivePage().findView(id);
	...
	public void setInput(Object input) {
		...
		IConfigurationElement[] samples = Platform.getExtensionRegistry().getConfigurationElementsFor("com.siteview.mde.ui.samples"); //$NON-NLS-1$
	...
	private void update(IConfigurationElement sample) {
		...
		if (descConfig != null && descConfig.length == 1) {
			...
			String content = NLS.bind(MDEUIMessages.SampleStandbyContent_desc, (desc != null ? desc : "")); //$NON-NLS-1$ 
